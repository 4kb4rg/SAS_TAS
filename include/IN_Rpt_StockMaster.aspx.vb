...
Public Class IN_Rpt_StockMaster : inherits page
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        strSearch =  "AND ItemType = '" & objInventory.EnumInventoryItemType.Stock & "' AND itm.Status like  '" & IIF(Not strStatus = "", strStatus, "%" ) &"' "
        ...
        Dim Brs, myRow As DataRow
        Dim i,j as int16
    
        myDS = ObjRptDs.Clone()
        
        myDS.Tables(0).Columns("SellFixedPrice").DataType = System.Type.GetType("System.String")
        
        For Each Brs In objRptDs.Tables(0).Rows
            myRow = myDS.Tables(0).NewRow()
            For j  = 0 To objRptDs.Tables(0).Columns.Count - 1
                        If (objRptDs.Tables(0).Columns(j).ColumnName.Trim.ToUpper = "SELLFIXEDPRICE") THEN       
                            myRow(j) = objGlobal.GetIDDecimalSeparator(Brs(j))
                        Else
                            myRow(j) = Brs(j)
                        End if
            Next j
        ...
        rdCrystalViewer.Load(objMapPath & "Web\EN\IN\Reports\Crystal\In_Rpt_StockMaster.rpt", OpenReportMethod.OpenReportByTempCopy)
        rdCrystalViewer.SetDataSource(myDS.Tables(0))
        ...
        Else
			...
			If strExportToExcel = "0" Then
                    DiskOpts.DiskFileName = objMapPath & "web\ftp\IN_Rpt_StockMaster.pdf"
			else
			        DiskOpts.DiskFileName = objMapPath & "web\ftp\IN_Rpt_StockMaster.xls"
			end if
			...
			If strExportToExcel = "0" Then
				Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/IN_Rpt_StockMaster.pdf"">")
			else
				Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/IN_Rpt_StockMaster.xls"">")
			end if
        End If
