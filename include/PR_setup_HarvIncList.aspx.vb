...
Public Class PR_setup_HarvIncList : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "HI.HarvIncCode"
            End If
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "PR_CLSSETUP_HARVINCENTIVE_SEARCH"
        Dim strSrchHarvIncCode As String
        ...
        strSrchHarvIncCode = IIF(txtHarvIncCode.Text = "", "", txtHarvIncCode.Text)
        ...
        strParam = strLocation & "|" & strSrchHarvIncCode & "|" & _
                   strSrchDesc & "|" & _
                   strSrchStatus & "|" & _
                   strSrchLastUpdate & "|" & _
        ...
        Try
            intErrNo = objPRSetup.mtdGetHarvIncentive(strOpCd_GET, strParam, objHarvIncDs, False)
        ...
        If objHarvIncDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objHarvIncDs.Tables(0).Rows.Count - 1
                objHarvIncDs.Tables(0).Rows(intCnt).Item("HarvIncCode") = Trim(objHarvIncDs.Tables(0).Rows(intCnt).Item("HarvIncCode"))
                objHarvIncDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objHarvIncDs.Tables(0).Rows(intCnt).Item("Description"))
                objHarvIncDs.Tables(0).Rows(intCnt).Item("Status") = Trim(objHarvIncDs.Tables(0).Rows(intCnt).Item("Status"))
                objHarvIncDs.Tables(0).Rows(intCnt).Item("UpdateID") = Trim(objHarvIncDs.Tables(0).Rows(intCnt).Item("UpdateID"))
                objHarvIncDs.Tables(0).Rows(intCnt).Item("UserName") = Trim(objHarvIncDs.Tables(0).Rows(intCnt).Item("UserName"))
            Next
        End If
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        Dim strOpCd As String = "PR_CLSSETUP_HARVINCENTIVE_STATUS_UPD"
        Dim strParam As String = ""
        Dim CPCell As TableCell = e.Item.Cells(0)
        Dim strSelectedHarvIncCode As String 
        Dim intErrNo As Integer

        strSelectedHarvIncCode = CPCell.Text
        strParam = strSelectedHarvIncCode & "|" & objPRSetup.EnumHarvIncentiveStatus.Deleted
        ...
        Try
            intErrNo = objPRSetup.mtdUpdHarvIncentive(strOpCd, _
                                                      ...
                                                      True)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_SETUP_HARVINCENTIVE_DEL&errmesg=" & lblErrMessage.Text & "&redirect=hr/setup/PR_setup_HarvIncList.aspx")
        End Try
    ...
    Sub NewHarvIncBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("PR_setup_HarvIncdet.aspx")
    End Sub
