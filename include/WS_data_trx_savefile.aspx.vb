...
Public Class WS_data_trx_savefile : Inherits page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumWSAccessRights.WSDownload), intWSAR) = False Then
    ...
    Sub SaveFile()
        ...
        Dim strOpCode As String = "WS_CLSDATA_MTHENDTRX_DOWNLOAD"
        Dim strFtpPath As String
        Dim strXmlPath As String = ""
        Dim strXmlFileName As String = objGlobal.mtdGenerateDownloadFile(objGlobal.EnumMthEnd.WSMthEnd, False)
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=WSDATA_TRX_GET_FTPPATH&errmesg=&redirect=")
        End Try

        Try
            strParam = strParam & strOpCode & "|" & objWsData.mtdGetDownloadTable(objWsData.EnumDownloadType.WSMthEndTrx)
            intErrNo = objWsData.mtdDownloadRef(strCompany, _
                                                ...
                                                objDownloadDs)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=WSDATA_TRX_DOWNLOAD_REF&errmesg=&redirect=")
        End Try
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=WSDATA_TRX_ENCRYPT_REF&errmesg=&redirect=")
        End Try

        Try
            strParam = objGlobal.EnumModule.Workshop & "|" & objAdmShare.EnumMthEndPost.Yes
            intErrNo = objAdmShare.mtdUpdMonthEndPosted(strCompany, _
                                                        ...
                                                        intPosted)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=WSDATA_TRX_POSTED_UPD&errmesg=&redirect=")
        End Try

        If intPosted = 2 Then
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=WSDATA_TRX_NOT_CLOSED&errmesg=&redirect=")
        Else
