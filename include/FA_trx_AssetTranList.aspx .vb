...
Public Class FA_trx_AssetTranList : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            If SortExpression.Text = "" Then
                SortExpression.Text = "AssetTranID"
            End If
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.Text = "ASSET TRANSFER LISTING " 'UCase(GetCaption(objLangCap.EnumLangCap.AssetWO))
        lblTxID.Text = "Asset Transfer ID"
        ...
        dgTxList.Columns(2).HeaderText = "To. Loc"
    End Sub
   ...
   End Sub
    ...
    Protected Function LoadData() As DataSet
        Dim strOppCode_Get As String = "FA_CLSTRX_ASSETTRAN_GET"
        ...
        strSearch = "AND FA.LOCCODE = '" & strLocation & "'"
        strSearch = strSearch & "AND FA.AccMonth = '" & srchAccMonth.Text & "'"
        strSearch = strSearch & " AND FA.AccYear = '" & srchAccYear.Text & "'"
        If Trim(srchTxID.Text) <> "" Then
            strSearch = strSearch & " AND FA.AssetTranID = '" & srchTxID.Text & "'"
        End If
        If Trim(srchAssetCode.Text) <> "" Then
            strSearch = strSearch & " AND  LN.Description = '" & srchAssetCode.Text & "'"
        End If
        If srchStatusList.SelectedItem.Value <> 5 Then
            strSearch = strSearch & " AND  FA.Status = '" & srchStatusList.SelectedItem.Value & "'"
        End If
        If Trim(srchUpdateBy.Text) <> "" Then
            strSearch = strSearch & " AND  LN.UpdateID = '" & srchUpdateBy.Text & "'"
        End If

        strOrder = "ORDER BY " & SortExpression.Text & "  " & SortCol.Text

        strParamValue = strSearch & "|" & strOrder

        Try
            intErrNo = objFATrx.mtdGetDataCommon(strOppCode_Get, strParamName, strParamValue, objTxDs)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=FA_CLSTRX_ASSETTRANLIST_GET&errmesg=" & lblErrMessage.Text & "&redirect=FA/trx/FA_trx_AssetTranList.aspx")
        End Try

        For intCnt = 0 To objTxDs.Tables(0).Rows.Count - 1
            objTxDs.Tables(0).Rows(intCnt).Item("AssetTranID") = objTxDs.Tables(0).Rows(intCnt).Item("AssetTranID").Trim()
