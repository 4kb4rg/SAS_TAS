...
Public Class PR_setup_PrmBrondolList_Estate : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            If SortExpression.Text = "" Then
                SortExpression.Text = "PRBrondolCode"
            End If
    ...
    Protected Function LoadData() As DataSet
        ...
        If objBankDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objBankDs.Tables(0).Rows.Count - 1
                objBankDs.Tables(0).Rows(intCnt).Item("PRBrondolCode") = Trim(objBankDs.Tables(0).Rows(intCnt).Item("PRBrondolCode"))
                objBankDs.Tables(0).Rows(intCnt).Item("PrmBdl") = Trim(objBankDs.Tables(0).Rows(intCnt).Item("PrmBdl"))
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal e As DataGridCommandEventArgs)
        Dim strOpCd_UPD As String = "PR_PR_STP_PREMI_BERONDOL_LIST_DEL"
        Dim int As Integer = e.Item.ItemIndex
        Dim GolCell As TableCell = e.Item.Cells(0)
        Dim strGolId As String
        Dim strStatus As String
        Dim intErrNo As Integer
        Dim EditText As Label


        strGolId = GolCell.Text
        EditText = dgLine.Items.Item(int).FindControl("Status")
        strStatus = IIf(EditText.Text = "Active", "2", "1")

        ParamNama = "IG|ST|UD|UI"
        ParamValue = strGolId & "|" & strStatus & "|" & DateTime.Now & "|" & strUserId

        Try
            intErrNo = ObjOK.mtdInsertDataCommon(strOpCd_UPD, ParamNama, ParamValue)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_PR_STP_PREMI_BERONDOL_LIST_DEL&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
    ...
    Sub NewAstekBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PR_setup_PrmBrondolDet_Estate.aspx")
    End Sub
