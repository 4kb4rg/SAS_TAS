...
Public Class PR_Setup_Angsuran : Inherits Page
    ...
    Protected WithEvents srchAngsuranCode as TextBox
    ...
    Dim strOppCd_GET As String = "PR_CLSSETUP_ANGSURAN_LIST_GET"
    Dim strOppCd_ADD As String = "HR_CLSSETUP_ICTYPE_LIST_ADD"
    Dim strOppCd_UPD As String = "HR_CLSSETUP_ICTYPE_LIST_UPD"
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumHRAccessRights.HRICType), intHRAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")

        Else

            If SortExpression.text = "" Then
                SortExpression.text = "AngsuranCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objHR.mtdGetICTypeStatus(objHR.EnumICTypeStatus.Active), objHR.EnumICTypeStatus.Active))
        StatusList.Items.Add(New ListItem(objHR.mtdGetICTypeStatus(objHR.EnumICTypeStatus.Deleted), objHR.EnumICTypeStatus.Deleted))

    End Sub 

    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objHR.mtdGetICTypeStatus(objHR.EnumICTypeStatus.All), objHR.EnumICTypeStatus.All))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetICTypeStatus(objHR.EnumICTypeStatus.Active), objHR.EnumICTypeStatus.Active))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetICTypeStatus(objHR.EnumICTypeStatus.Deleted), objHR.EnumICTypeStatus.Deleted))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND IC.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objHR.EnumICTypeStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchAngsuranCode.text = "" Then
            SearchStr =  SearchStr & " AND IC.AngsuranCode like '" & srchAngsuranCode.text &"%' "
        End If
        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND IC.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objHR.mtdGetMasterList(strOppCd_GET, strParam, objHR.EnumHRMasterType.ICType, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_ICTYPE_GET_LIST&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        Return objDataSet
    End Function
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objHR.EnumICTypeStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("AngsuranCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("AngsuranCode")
        AngsuranCode = EditText.Text
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          strUserId, _
                                          strParam, _
                                          objHR.EnumHRMasterType.ICType, _
                                          blnDupKey, _
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_UPDATE_ICTYPE&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_ICType.aspx")
        End Try
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objHR.EnumICTypeStatus.Active, _
                        objHR.EnumICTypeStatus.Deleted, _
                        objHR.EnumICTypeStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  AngsuranCode & "|" & _
                    Description & "|" & _
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_DELETE_ICTYPE&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_ICType.aspx")
        End Try
