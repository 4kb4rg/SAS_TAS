...
Public Class IN_ProdType : Inherits Page
    ...
    Protected WithEvents srchProdTypeCode as TextBox
    ...
    Dim strOppCd_GET As String = "IN_CLSSETUP_PRODTYPE_LIST_GET"
    ...
    Dim ProdTypeCodeTag As String
    Dim ProdTypeDescTag As String
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumINAccessRights.INProductSetup), intINAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")
            
        Else
            onload_GetLangCap()
            If SortExpression.text = "" Then
                SortExpression.text = "ProdTypeCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.text = UCase(GetCaption(objLangCap.EnumLangCap.ProdType))
        lblProdTypeCode.text = GetCaption(objLangCap.EnumLangCap.ProdType) & lblCode.text
        lblDescription.text = GetCaption(objLangCap.EnumLangCap.ProdTypeDesc)

        strValidateCode = lblPleaseEnter.text & lblProdTypeCode.text & "."
        strValidateDesc = lblPleaseEnter.text & lblDescription.text & "."

        EventData.Columns(0).HeaderText = lblProdTypeCode.text
        EventData.Columns(1).HeaderText = lblDescription.text

        DocTitleTag = lblTitle.text & lblList.text
        ProdTypeCodeTag = lblProdTypeCode.text
        ProdTypeDescTag = lblDescription.text
    End Sub
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_SETUP_PRODTYPE_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodtype.aspx")
        End Try
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductTypeStatus(objIN.EnumProductTypeStatus.Active), objIN.EnumProductTypeStatus.Active))
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductTypeStatus(objIN.EnumProductTypeStatus.Deleted), objIN.EnumProductTypeStatus.Deleted))

    End Sub 

    Sub BindSearchList() 
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductTypeStatus(objIN.EnumProductTypeStatus.Active), objIN.EnumProductTypeStatus.Active))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductTypeStatus(objIN.EnumProductTypeStatus.Deleted), objIN.EnumProductTypeStatus.Deleted))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductTypeStatus(objIN.EnumProductTypeStatus.All), objIN.EnumProductTypeStatus.All))
    End Sub 
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND PType.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductTypeStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "

        If NOT srchProdTypeCode.text = "" Then
            SearchStr =  SearchStr & " AND PType.ProdTypeCode like '" & srchProdTypeCode.text &"%' "
        End If
        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND PType.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objIN.mtdGetMasterList(strOppCd_GET, strParam, objIN.EnumInventoryMasterType.ProductType, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_PRODTYPE&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodtype.aspx")
        End Try

        If objDataSet.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objDataSet.Tables(0).Rows.Count - 1
                objDataSet.Tables(0).Rows(intCnt).Item("ProdTypeCode") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("ProdTypeCode"))
...
Sub btnPreview_Click (sender As Object, e As ImageClickEventArgs)
    ...
    Dim strProdTypeCode As String
    Dim strDescription As String
    Dim strUpdateBy As String
    Dim strSortExp As String
    Dim strSortCol As String

    strStatus = IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductTypeStatus.All, srchStatusList.selectedItem.Value, "")
    strProdTypeCode = srchProdTypeCode.text
    strDescription = srchDesc.text
    strUpdateBy =  srchUpdateBy.text
    strSortExp = sortexpression.text
    strSortCol = sortcol.text

    Response.Write("<Script Language=""JavaScript"">window.open(""../reports/IN_Rpt_ProdTypeList.aspx?strStatus=" & strStatus & _
                   "&strProdTypeCode=" & strProdTypeCode & _
                   ...
                   "&ProdTypeCodeTag=" & ProdTypeCodeTag & _
                   "&ProdTypeDescTag=" & ProdTypeDescTag & _
...
End Sub
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objIN.EnumProductTypeStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("ProdTypeCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("ProdTypeCode")
        ProdTypeCode = EditText.Text
        ...
        intErrNo = objIN.mtdUpdMasterList(strOppCd_ADD, _
                                                ...
                                                strUserId, _
                                                strParam, _
                                                objIN.EnumInventoryMasterType.ProductType, _
                                                blnDupKey, _
                                                blnUpdate.text)
        Catch Exp As System.Exception
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objIN.EnumProductTypeStatus.Active, _
                        objIN.EnumProductTypeStatus.Deleted, _
                        objIN.EnumProductTypeStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  ProdTypeCode & "|" & _
                    Description & "|" & _
