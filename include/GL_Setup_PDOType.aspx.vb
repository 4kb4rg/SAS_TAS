...
Public Class GL_Setup_PDOType : Inherits Page
    ...
    Dim strOppCd_ADD As String = "GL_STP_PDO_HEADER_ADD"
    Dim strOppCd_DEL As String = "GL_STP_PDO_HEADER_DEL"
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.Text = "GROUP PERMINTAAN DANA OPERASIONAL (PDO)"
        lblVehTypeCode.Text = "Group PDO"
    ...
    Sub BindSearchList()
        srchStatus.Items.Add(New ListItem(objGLSetup.mtdGetVehTypeStatus(objGLSetup.EnumVehTypeStatus.All), objGLSetup.EnumVehTypeStatus.All))
        srchStatus.Items.Add(New ListItem(objGLSetup.mtdGetVehTypeStatus(objGLSetup.EnumVehTypeStatus.Active), objGLSetup.EnumVehTypeStatus.Active))
        srchStatus.Items.Add(New ListItem(objGLSetup.mtdGetVehTypeStatus(objGLSetup.EnumVehTypeStatus.Deleted), objGLSetup.EnumVehTypeStatus.Deleted))
    ...
    Function GetAutoNum(ByVal pBulan As String, ByVal pTahun As String, ByVal pLocCode As String) As String
        Dim strOpCd_AutoNum As String = "GL_STP_PDO_DETAIL_AUTONUM"
        Dim intErrNo As Integer
        ...
        Catch Exp As System.Exception
            Response.Write("/include/mesg/ErrorMessage.aspx?errcode=GL_STP_PDO_DETAIL_AUTONUM &errmesg=" & Exp.Message & "&redirect=HR/trx/HR_trx_EmployeeList_Estate.aspx")
        End Try

        If objNumDs.Tables(0).Rows.Count = 1 Then
            nNOUrut = objNumDs.Tables(0).Rows(0).Item("PdoGrpID")
        Else
            nNOUrut = 0
        End If
    ...
    Protected Function LoadData() As DataSet
        Dim strOppCd_GET As String = "GL_STP_PDO_HEADER_LIST"
        Dim strParam As String
        Dim SearchStr As String
        Dim sortItem As String
        Dim objTransDs As New Object()

        'SearchStr = " AND veh.Status like '" & IIf(srchStatus.SelectedItem.Value <> objGLSetup.EnumVehTypeStatus.All, srchStatus.SelectedItem.Value, "%") & "' "

        'If Not srchVehTypeCode.Text = "" Then
        '    SearchStr = SearchStr & " AND veh.VehTypeCode like '" & srchVehTypeCode.Text & "%'"
        'End If
        'If Not srchDescription.Text = "" Then
        '    SearchStr = SearchStr & " AND veh.Description like '" & srchDescription.Text & "%'"
        'End If
    ...
    Sub btnPreview_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        strStatus = IIf(Not srchStatus.SelectedItem.Value = objGLSetup.EnumVehTypeStatus.All, srchStatus.SelectedItem.Value, "")
        strVehTypeCode = srchVehTypeCode.Text
        strDescription = srchDescription.Text
        strUpdateBy = srchUpdBy.Text
        strSortExp = SortExpression.Text
        strSortCol = sortcol.Text

        Response.Write("<Script Language=""JavaScript"">window.open(""../reports/GL_Rpt_VehicleType.aspx?strVehTypeCodeTag=" & strVehTypeCodeTag & _
                    "&strDescTag=" & strDescTag & "&strTitleTag=" & strTitleTag & _
                    "&strStatus=" & strStatus & _
                    "&strVehTypeCode=" & strVehTypeCode & "&strDescription=" & strDescription & _
    ...
    Sub DEDR_Edit(ByVal Sender As Object, ByVal e As DataGridCommandEventArgs)
        ...
        Dim validateUrut As RequiredFieldValidator
        Dim chk As CheckBox
        blnUpdate.Text = True
        ...
        Select Case CInt(txtEditText.Text) = objGLSetup.EnumVehTypeStatus.Active
            Case True
                ...
                txtEditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("PdoGrpID")
            ...
            Case False
                ...
                txtEditText.Enabled = False
                txtUrut = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("Urut")
        ...
        validateUrut = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("validateUrut")
    ...
    Sub DEDR_Update(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        Dim txtEditText As TextBox
        Dim ddlList As DropDownList
        Dim strPDOCode As String
        Dim strDescription As String
        Dim strUrut As String = "0"
        Dim strStatus As String
        ...
        'txtEditText.ReadOnly = True
        txtEditText = E.Item.FindControl("PdoGrpID")
        strPDOCode = txtEditText.Text
        txtEditText = E.Item.FindControl("Description")
        strDescription = txtEditText.Text
        txtEditText = E.Item.FindControl("Urut")
        strUrut = txtEditText.Text

        ParamNama = "PDO|DESC|URUT|LOCCODE|STATUS|CRDATE|UPDATE|UID"
        ParamValue = strPDOCode & "|" & _
                     strDescription & "|" & _
                     strUrut & "|" & _
                     strLocation & "|" & _
                     "1" & "|" & _
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        strStatus = IIf(txtEditText.Text = objGLSetup.EnumVehTypeStatus.Active, _
                        objGLSetup.EnumVehTypeStatus.Deleted, _
                        objGLSetup.EnumVehTypeStatus.Active)

        ParamNama = "PDO|LOCCODE|UID|UDATE"
        ParamValue = strPDOCode & "|" & _
                     strLocation & "|" & _
                     strUserId & "|" & _
                     Date.Now()
    ...
    Sub DEDR_Add(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        strNewIDFormat = "PDO" & strLocation & StrNoTransF

        newRow = dataSet.Tables(0).NewRow()
        newRow.Item("PdoGrpID") = strNewIDFormat
        ...
        validateUrut = EventData.Items.Item(CInt(EventData.EditItemIndex)).FindControl("validateUrut")

        validateCode.ErrorMessage = strValidateCode
        validateDesc.ErrorMessage = strValidateDesc
        validateUrut.ErrorMessage = strValidateUrut
