...
Public Class IN_Rpt_StockIssueDet : inherits page
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Select Case CInt(strIssueType)
            Case objIN.EnumStockIssueType.OwnUse
                strOpCd_Get = "IN_CLSTRX_STOCKISSUE_DETAILS_GET_REPORT" & "|" & "StockIssue"
                strOpCd_GetLine = "IN_CLSTRX_STOCKISSUE_LINE_GET_REPORT" & "|" & "StockIssueLn"
                strReportName = "IN_Rpt_StockIssueDet_OwnUse.rpt"
            Case objIN.EnumStockIssueType.StaffPayroll, objIN.EnumStockIssueType.StaffDN
                strOpCd_Get = "IN_CLSTRX_STOCKISSUE_DETAILS_GET_REPORT_STAFF" & "|" & "StockIssue"
                strOpCd_GetLine = "IN_CLSTRX_STOCKISSUE_LINE_GET_REPORT_STAFF" & "|" & "StockIssueLn"
                strReportName = "IN_Rpt_StockIssueDet_Staff.rpt"
            Case objIN.EnumStockIssueType.External
                strOpCd_Get = "IN_CLSTRX_STOCKISSUE_DETAILS_GET_REPORT_EXTERNAL" & "|" & "StockIssue"
                strOpCd_GetLine = "IN_CLSTRX_STOCKISSUE_LINE_GET_REPORT_EXTERNAL" & "|" & "StockIssueLn"
                If strLocation = "OLN" Then
                    strReportName = "IN_Rpt_StockIssueDet_External.rpt"
                Else
                    strReportName = "IN_Rpt_StockIssueList_External_Olein.rpt"
                End If
            Case objIn.EnumStockIssueType.Nursery
                strOpCd_Get = "IN_CLSTRX_STOCKISSUE_DETAILS_GET_REPORT_NURSERY " & "|" & "StockIssue"
                strOpCd_GetLine = "IN_CLSTRX_STOCKISSUE_LINE_GET_REPORT_NURSERY" & "|" & "StockIssueLn"
                strReportName = "IN_Rpt_StockIssueDet_Nursery.rpt"
        End Select
        ...
        Try
            intErrNo = objIN.mtdGetStockIssueReport(strOpCodes, _
                                                    strParam, _
                                                    strCompany, _
                                                    strLocation, _
                                                    strUserId, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_RPT_STOCKISSUE_GET&errmesg=" & lblErrMesage.Text & "&redirect=")        
        End Try

        If objRptDs.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
                If batchPrint = "yes" Then
                    objRptDs.Tables(0).Rows(intCnt).Item("PrintStatus") = objIN.mtdGetStockIssueStatus(CInt(objRptDs.Tables(0).Rows(intCnt).Item("Status")))
                    ...
                    For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
                        For intCnt2=0 To UBound(arrReprintedID)
                            If Trim(objRptDs.Tables(0).Rows(intCnt).Item("StockIssueId")) = Trim(arrReprintedID(intCnt2)) Then
                                objRptDs.Tables(0).Rows(intCnt).Item("PrintStatus") = objIN.mtdGetStockIssueStatus(CInt(objRptDs.Tables(0).Rows(intCnt).Item("Status"))) & " (re-printed)"                                
                            End If
                        Next
                    Next
                End If
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_RPT_STOCKISSUE_GET_MAPPATH&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\IN_Rpt_StockIssueDet.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/IN_Rpt_StockIssueDet.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        If (Trim(strIssueType) = objIN.EnumStockIssueType.OwnUse) OR (Trim(strIssueType) = objIN.EnumStockIssueType.Nursery) Then
        ...
        ElseIf Trim(strIssueType) = objIn.EnumStockIssueType.External Then
            Response.Write("hai")
            Dim paramField7 As New ParameterField()
            Dim paramField8 As New ParameterField()
            ...
            paramField7 = paramFields.Item("strDateFrom")
            paramField8 = paramFields.Item("strDateTo")

            crParameterValues7 = paramField7.CurrentValues
            crParameterValues8 = paramField8.CurrentValues

            ParamDiscreteValue7.Value = strDateFrom
            ParamDiscreteValue8.Value = strDateTo
