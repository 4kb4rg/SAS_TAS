...
Public Class NU_trx_SeedTransplantDetails : Inherits Page
    ...
    Protected WithEvents lblToBatchNoErr As Label
    ...
    Protected WithEvents ddlToBatchNo As DropDownList
    ...
    Dim strOppCd_GET As String = "NU_CLSTRX_SEEDTRANSPLANT_GET"
    Dim strOppCd_ADD As String = "NU_CLSTRX_SEEDTRANSPLANT_ADD"
    Dim strOppCd_UPD As String = "NU_CLSTRX_SEEDTRANSPLANT_UPD"
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumNUAccessRights.NUTransplanting), intNUAR) = False Then
        ...
        Else
            ...
            If Not Page.IsPostBack Then
                ...
                If Not lblTxID.Text = "" Then
                    ...
                    DisableControl()
                    If lblStatus.Text = objNUTrx.mtdGetSeedTransplantStatus(objNUTrx.EnumSeedTransplantStatus.Deleted) Then
                        Delete.Visible = False
                    ElseIf lblStatus.Text = objNUTrx.mtdGetSeedTransplantStatus(objNUTrx.EnumSeedTransplantStatus.Closed) Then
                        Delete.Visible = False
                    Else
                        Delete.Visible = True
    ...
    Sub onload_GetLangCap()
        ...
        lblToBatchNoErr.Text = "Please select " & GetCaption(objLangCap.EnumLangCap.BatchNo) & "."
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_GET_LANGCAP&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedTransplantDetails.aspx")
        End Try
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objNUTrx.mtdGetSeedTransplant(strCompany, _
                                                    strLocation, _
                                                    strUserId, _
                                                    strAccMonth, _
                                                    strAccYear, _
                                                    ...
                                                    objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_GET&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedTransplantList.aspx")
        End Try
    ...
    Sub DisplayData()
        ...
        If dsTx.Tables(0).Rows.Count > 0 Then
            lblTxID.Text = Trim(dsTx.Tables(0).Rows(0).Item("TransplantID"))
            txtDate.Text = objGlobal.GetShortDate(strDateFormat, Trim(dsTx.Tables(0).Rows(0).Item("TransplantDate")))
            ...
            lblStatus.Text = objNUTrx.mtdGetSeedTransplantStatus(Trim(dsTx.Tables(0).Rows(0).Item("Status")))
            ...
            lblCostSeeds.Text = objGlobal.GetIDDecimalSeparator(FormatNumber(objNUTrx.mtdGetCostSeedsTransPlanting(strOpCdStckTxLine_CostGET, _
                                                        strCompany, _
                                                        strLocation, _
                                                        strUserId, _
                                                        strAccMonth, _
            ...
            BindToBatchNo(Trim(dsTx.Tables(0).Rows(0).Item("BlkCode")), Trim(dsTx.Tables(0).Rows(0).Item("ToMainBatchNo")))
    ...
    Sub BindBlkCode(ByVal pv_strBlkCode As String)
        ...
        SearchStr = " AND blk.BlkCode IN (SELECT BlkCode FROM NU_NURSERYBATCH WHERE NurseryStage='2' AND Status='1' AND LocCode='" & strLocation & "') "
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_BIND_BLOCK_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedTransplantDetails.aspx")
        End Try
        ...
        If intSelectedIndex = 0 And Not lblTxID.Text = "" Then
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_BIND_BLOCK_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedTransplantDetails.aspx")
            End Try
        End If
    ...
    Sub BindBatchNo(ByVal pv_strBlkCode As String, ByVal pv_strBatchNo As String)
        ...
        If Not pv_strBlkCode = "" Then
            strParam = pv_strBlkCode & "|" & _
                        ...
                        strLocation & "|" & _
                        "NB.BatchNo" & "|" & _
                        cstr(objNUSetup.EnumNurseryStage.PreNursery)

            Try
                intErrNo = objNUSetup.mtdGetNurseryBatch(strOpCode, strParam, dsForDropDown)
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_BIND_BATCHNO_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedTransplantDetails.aspx")
            End Try
            ...
            If intSelectedIndex = -1 And Not lblTxID.Text = "" Then
                ...
                Catch Exp As System.Exception
                    Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_BIND_BATCHNO_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedTransplantDetails.aspx")
                End Try
            End If
    ...
    Sub BindToBatchNo(ByVal pv_strBlkCode As String, ByVal pv_strBatchNo As String)
        Dim strOpCode As String = "NU_CLSSETUP_NURSERYBATCH_LIST_GET"
        Dim strParam As String
        Dim dsForDropDown As DataSet
        Dim dsForInactiveItem As DataSet
        ...
        If Not pv_strBlkCode = "" Then
            strParam = "" & "|" & _
                        ...
                        strLocation & "|" & _
                        "NB.BatchNo" & "|" & _
                        CStr(objNUSetup.EnumNurseryStage.MainNursery)

            Try
                intErrNo = objNUSetup.mtdGetNurseryBatch(strOpCode, strParam, dsForDropDown)
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_BIND_TOBATCHNO_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedTransplantDetails.aspx")
            End Try
            ...
            ddlToBatchNo.DataSource = dsForDropDown.Tables(0)
            ddlToBatchNo.DataValueField = "BatchNo"
            ddlToBatchNo.DataTextField = "BatchNo"
            ddlToBatchNo.DataBind()
            ...
            If intSelectedIndex = -1 And Not lblTxID.Text = "" Then
                ...
                Try
                    ...
                    If dsForInactiveItem.Tables(0).Rows.Count > 0 Then  
                        ddlToBatchNo.Items.Add(New ListItem(Trim(dsForInactiveItem.Tables(0).Rows(0).Item("BatchNo")) & _
                         " (" & objNUSetup.mtdGetNurseryBatchStatus(Trim(dsForInactiveItem.Tables(0).Rows(0).Item("Status"))) & ") ", Trim(dsForInactiveItem.Tables(0).Rows(0).Item("BatchNo"))))
                        intSelectedIndex = ddlToBatchNo.Items.Count - 1
                    Else 
                        intSelectedIndex = 0
                    End If

                Catch Exp As System.Exception
                    Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANT_DET_BIND_TOBATCHNO_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedTransplantDetails.aspx")
                End Try
            End If

            ddlToBatchNo.SelectedIndex = intSelectedIndex
        Else
            ddlToBatchNo.Items.Add(New ListItem(" "))
        End If
    End Sub
    ...
    Sub UpdateData(ByVal strAction As String)
        ...
        If Not strDate = "" Then
            Select Case strAction
                Case "Confirm"
                    strStatus = objNUTrx.EnumSeedTransplantStatus.Confirmed
                Case "Delete"
                    strStatus = objNUTrx.EnumSeedTransplantStatus.Deleted
            End Select

            strNewIDFormat = "STP" & "/" & strCompany & "/" & strLocation & "/" & strAccYear & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/"
            ...
            strParam = lblTxID.Text & "|" & _
                        ...
                        strDate & "|" & _
                        txtQty.Text & "|" & _
                        ddlToBatchNo.SelectedItem.Value & "|" & _
                        Trim(txtAddNote.Value) & "|" & _
                        Trim(strNewIDFormat)

            Try
                intErrNo = objNUTrx.mtdUpdSeedTransplant(strOppCd_ADD, _
                                                    strOppCd_UPD, _
                                                    strOppCd_GET, _
                                                    strLocation, _
                                                    strUserId, _
                                                    ...
                                                    intError, _
                                                    lblOper.Text, _
                                                    objGlobal.mtdGetDocPrefix(objGlobal.EnumDocType.SeedTransplant), _
                                                    TxID, _
                                                    blnDeleteErr)
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDTRANSPLANTDETAILS_UPDATE&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedTransplantDetails.aspx")
            End Try
            ...
            Else
                If blnDeleteErr = False Then
                    ...
                    '/* UPDATE NURSERY BATCH INTO MAIN NURSERY */
                    strOpCode = "NU_CLSSETUP_NURSERYBATCH_LIST_UPD_TRANSPLANT"
                    strParamName = "UPDATESTR"
                    strParamValue = Trim(lblTxID.Text)
                    ...
                    Response.Redirect("NU_trx_SeedTransplantList.aspx")
                End If
            End If
        End If
    ...
    Sub btnConfirm_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        If ddlBatchNo.SelectedItem.Value = "" Then
            ...
            Exit Sub
        End If
        'If ddlToBatchNo.Items.Count = 0 Then
        '    lblToBatchNoErr.Visible = True
        '    Exit Sub
        'End If
        UpdateData("Confirm")
    End Sub
    ...
    Sub CallFillBatchNo(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        BindToBatchNo(ddlBlkCode.SelectedItem.Value, "")
        ...
        Dim strOpCode As String
        Dim dsNUTP As Object

        '/* GET NURSERY BATCH QTY */
        strOpCode = "NU_CLSSETUP_NURSERYBATCH_LIST_GET_QTY"
        strParamName = "UPDATESTR"
        strParamValue = Trim(ddlBatchNo.SelectedItem.Value)

        Try
            intErrNo = objNUTrx.mtdGetDataCommon(strOpCode, _
                                                strParamName, _
                                                strParamValue, _
                                                dsNUTP)
        Catch Exp As System.Exception
        ...
        If dsNUTP.Tables(0).Rows.Count > 0 Then
            txtQty.Text = Round(dsNUTP.Tables(0).Rows(0).Item("QtyAkhir"), 0)
        Else
            txtQty.Text = 0
        End If
        ...
        'txtQty.Text = cDbl(objNUTrx.mtdGetQtySeedsTransPlanting(strOpCdStckTxLine_QtyGET, _
        '                                                strCompany, _
        '                                                strLocation, _
        '                                                strUserId, _
        '                                                strAccMonth, _
