...
Public Class PM_Rpt_StorageTypeMasterList : inherits page
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd As String = "PM_CLSSETUP_STORAGETYPE_GET"
        ...
        strSearch =  " AND ST.Status like '" & IIF(Not strStatus = "", strStatus, "%" ) & "' "
        ...
        Try
            intErrNo = objPM.mtdGetStorageType(strOpCd, strParam, objRptDs)
        ...
        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objPM.mtdGetStorageTypeStatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
            objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate") = objGlobal.GetLongDate(objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate"))
        Next

        rdCrystalViewer.Load(objMapPath & "Web\EN\PM\Reports\Crystal\PM_Rpt_StorageTypeMasterList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\PM_Rpt_StorageTypeMasterList.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/PM_Rpt_StorageTypeMasterList.pdf"">")
        End If
