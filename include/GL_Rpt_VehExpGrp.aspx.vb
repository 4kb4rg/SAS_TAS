...
Public Class GL_Rpt_VehExpGrp : inherits page
    ...
    Public Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strVehExpGrpCode = Trim(Request.QueryString("strVehExpGrpCode"))
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd As String = "GL_CLSSETUP_VEHSUBGROUP_LIST_SEARCH"
        ...
        strSearch =  " AND Acc.Status like '" & IIF(Not strStatus = "", strStatus, "%" ) &"' "

        If Not strVehExpGrpCode = "" Then
            strSearch = strSearch & " AND Acc.VehExpGrpCode like '" & strVehExpGrpCode & "%'"
        End If
        If Not strDescription = "" Then
            strSearch = strSearch & " AND Acc.Description like '" & strDescription & "%'"
        End If
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=GL_SETUP_RPT_VEHEXPGRPLIST_GET&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try

        Try
            intErrNo = objAdmin.mtdGetBasePath(objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=GL_SETUP_RPT_VEHEXPGRPLIST_GET_MAPPATH&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try

        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("VehExpGrpCode") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("VehExpGrpCode"))
        ...
        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objGLSetup.mtdGetVehicleExpenseGrpStatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
            objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate") = objGlobal.GetLongDate(objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate"))
        Next


        rdCrystalViewer.Load(objMapPath & "Web\EN\GL\Reports\Crystal\GL_Rpt_VehExpGrp.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\GL_Rpt_VehExpGrp.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/GL_Rpt_VehExpGrp.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        paramField3 = ParamFields.Item("strVehExpGrpCodeTag")
        ...
        ParamDiscreteValue3.value = strVehExpGrpcodeTag
