...
Public Class PU_DAInternal : Inherits Page
    ...
    Dim strOppCd_GET As String = "PU_CLSTRX_DISPATCHADVICEINTERNAL_LIST_GET"
    ...
    Sub BindGrid()
        ...
        For intCnt = 0 To dgStockTransfer.Items.Count - 1
            ...
            If CInt(lbl.Text) = objPUTrx.EnumDAInternalStatus.Active Then
                btn.Attributes("onclick") = "javascript:return ConfirmAction('delete');"
            ElseIf CInt(lbl.Text) = objPUTrx.EnumDAInternalStatus.Deleted Then
    ...
    End Sub
    Sub BindSearchList()
        srchStatusList.Items.Add(New ListItem(objPUTrx.mtdGetDAInternalStatus(objPUTrx.EnumDAInternalStatus.All), objPUTrx.EnumDAInternalStatus.All))
        srchStatusList.Items.Add(New ListItem(objPUTrx.mtdGetDAInternalStatus(objPUTrx.EnumDAInternalStatus.Active), objPUTrx.EnumDAInternalStatus.Active))
        srchStatusList.Items.Add(New ListItem(objPUTrx.mtdGetDAInternalStatus(objPUTrx.EnumDAInternalStatus.Confirmed), objPUTrx.EnumDAInternalStatus.Confirmed))
        srchStatusList.Items.Add(New ListItem(objPUTrx.mtdGetDAInternalStatus(objPUTrx.EnumDAInternalStatus.Cancelled), objPUTrx.EnumDAInternalStatus.Cancelled))
        srchStatusList.Items.Add(New ListItem(objPUTrx.mtdGetDAInternalStatus(objPUTrx.EnumDAInternalStatus.Deleted), objPUTrx.EnumDAInternalStatus.Deleted))
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objPUTrx.mtdGetDAInternalList(strOppCd_GET, strParam, objDataSet)
        Catch Exp As System.Exception
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        Dim strOpCode As String = "IN_CLSTRX_DISPADVINTERNAL_LIST_UPD"
        Dim strParamName As String = ""
        Dim strParamValue As String = ""

        strParamName = "LOCCODE|DISPADVID|STATUS|UPDATEID"

        If strstatus = objPUTrx.EnumDAInternalStatus.Active Then

            strParamValue = strLocation & "|" & Trim(strTxID) & _
                            "|" & objPUTrx.EnumDAInternalStatus.Deleted & "|" & strUserId

        Else

            strParamValue = strLocation & "|" & Trim(strTxID) & _
                            "|" & objPUTrx.EnumDAInternalStatus.Active & "|" & strUserId
    ...
    Sub NewINDABtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PU_Trx_DAInternalDet.aspx?DAType=" & objPUTrx.EnumDAType.Stock)
    End Sub

    Sub NewDCDABtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PU_Trx_DAInternalDet.aspx?DAType=" & objPUTrx.EnumDAType.DirectCharge)
    End Sub


    Sub NewFADABtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PU_Trx_DAInternalDet.aspx?DAType=" & objPUTrx.EnumDAType.FixedAsset)
    End Sub

    Sub NewNUDABtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PU_Trx_DAInternalDet.aspx?DAType=" & objPUTrx.EnumDAType.Nursery)
    End Sub

    Sub BindDAType()
        ddlDAType.Items.Clear()
        ddlDAType.Items.Add(New ListItem(objPUTrx.mtdGetDAType(objPUTrx.EnumDAType.All), objPUTrx.EnumDAType.All))
        ...
        ddlDAType.Items.Add(New ListItem(objPUTrx.mtdGetDAType(objPUTrx.EnumDAType.Nursery), objPUTrx.EnumDAType.Nursery))
    End Sub
End Class
