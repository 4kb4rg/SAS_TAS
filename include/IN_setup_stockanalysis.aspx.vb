...
Public Class IN_StockAnalysis : Inherits Page
    ...
    Protected WithEvents srchStockAnaCode as TextBox
    ...
    Dim strOppCd_GET As String = "IN_CLSSETUP_STOCKANA_LIST_GET"
    Dim strOppCd_ADD As String = "IN_CLSSETUP_STOCKANA_LIST_ADD"
    Dim strOppCd_UPD As String = "IN_CLSSETUP_STOCKANA_LIST_UPD"
    ...
    Dim StockAnaCodeTag As String
    Dim StockAnaDescTag As String
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumINAccessRights.INAnalisisStok), intINAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")
        Else
            onload_GetLangCap()
            If SortExpression.text = "" Then
                SortExpression.text = "StockAnalysisCode"
            End If
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.text = UCase(GetCaption(objLangCap.EnumLangCap.StockAnalysis))
        lblStockAnaCode.text = GetCaption(objLangCap.EnumLangCap.StockAnalysis) & lblCode.text
        lblDescription.text = GetCaption(objLangCap.EnumLangCap.StockAnalysisDesc)

        strValidateCode = lblPleaseEnter.text & lblStockAnaCode.text & "."
        strValidateDesc = lblPleaseEnter.text & lblDescription.text & "."

        EventData.Columns(0).HeaderText = lblStockAnaCode.text
        EventData.Columns(1).HeaderText = lblDescription.text

        DocTitleTag = lblTitle.text & lblList.text
        StockAnaCodeTag = lblStockAnaCode.text
        StockAnaDescTag = lblDescription.text
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_SETUP_STOCKANALYSIS_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_StockAnalysis.aspx")
        End Try
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objIN.mtdGetStockAnalysisStatus(objIN.EnumStockAnalysisStatus.Active), objIN.EnumStockAnalysisStatus.Active))
        StatusList.Items.Add(New ListItem(objIN.mtdGetStockAnalysisStatus(objIN.EnumStockAnalysisStatus.Deleted), objIN.EnumStockAnalysisStatus.Deleted))

    End Sub 

    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objIN.mtdGetStockAnalysisStatus(objIN.EnumStockAnalysisStatus.Active), objIN.EnumStockAnalysisStatus.Active))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetStockAnalysisStatus(objIN.EnumStockAnalysisStatus.Deleted), objIN.EnumStockAnalysisStatus.Deleted))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetStockAnalysisStatus(objIN.EnumStockAnalysisStatus.All), objIN.EnumStockAnalysisStatus.All))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND StockAna.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objIN.EnumStockAnalysisStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchStockAnaCode.text = "" Then
            SearchStr = SearchStr & " AND StockAna.StockAnalysisCode like '" & srchStockAnaCode.text &"%' "
        End If

        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND StockAna.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        strParam =  sortitem & "|" & _
                    SearchStr
        Try
        intErrNo = objIN.mtdGetMasterList(strOppCd_GET, strParam, objIN.EnumInventoryMasterType.StockAnalysis, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_STOCKANALYSIS&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_stockanalysis.aspx")
        End Try

        If objDataSet.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objDataSet.Tables(0).Rows.Count - 1
                objDataSet.Tables(0).Rows(intCnt).Item("StockAnalysisCode") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("StockAnalysisCode"))
...
Sub btnPreview_Click (sender As Object, e As ImageClickEventArgs)
    ...
    Dim strStockAnaCode As String
    Dim strDescription As String
    Dim strUpdateBy As String
    Dim strSortExp As String
    Dim strSortCol As String

    strStatus = IIF(Not srchStatusList.selectedItem.Value = objIN.EnumStockAnalysisStatus.All, srchStatusList.selectedItem.Value, "")
    strStockAnaCode = srchStockAnaCode.text
    strDescription = srchDesc.text
    strUpdateBy =  srchUpdateBy.text
    strSortExp = sortexpression.text
    strSortCol = sortcol.text

    Response.Write("<Script Language=""JavaScript"">window.open(""../reports/IN_Rpt_StockAnalysis.aspx?strStatus=" & strStatus & _
                   "&strStockAnaCode=" & strStockAnaCode & _
                   ...
                   "&StockAnaCodeTag=" & StockAnaCodeTag & _
                   "&StockAnaDescTag=" & StockAnaDescTag & _
...
End Sub
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objIN.EnumStockAnalysisStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("StockAnalysisCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Dim list As Dropdownlist
        Dim lblMsg As label
        Dim StockAnalysisCode As String
        Dim Description As String
        Dim blnDupKey As Boolean = False
        Dim Status As String
        Dim CreateDate As String
 
        EditText = E.Item.FindControl("StockAnalysisCode")
        StockAnalysisCode = EditText.Text
        ...
        intErrNo = objIN.mtdUpdMasterList(strOppCd_ADD, _
                                                ...
                                                strUserId, _
                                                strParam, _
                                                objIN.EnumInventoryMasterType.StockAnalysis, _
                                                blnDupKey, _
                                                blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=UPDATE_STOCKANALYSIS&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_stockanalysis.aspx")
        End Try
    ...
    Sub DEDR_Cancel(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        Trace.Warn(EventData.PageCount)
        EventData.EditItemIndex = -1
        BindGrid()
    End Sub
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        Status = IIf(EditText.Text = objIN.EnumStockAnalysisStatus.Active, _
                        objIN.EnumStockAnalysisStatus.Deleted, _
                        objIN.EnumStockAnalysisStatus.Active)
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam = StockAnalysisCode & "|" & _
                    Description & "|" & _
