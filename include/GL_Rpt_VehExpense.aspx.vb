...
Public Class GL_Rpt_VehExpense : inherits page
    ...
    Public Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strVehExpGrpCodeTag = Trim(Request.QueryString("strVehExpGrpCodeTag"))
        strVehExpCodeTag = Trim(Request.QueryString("strVehExpCodeTag"))  
        ...
        strVehExpCode = Trim(Request.QueryString("strVehExpCode"))
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Try
            intErrNo = objGLSetup.mtdGetVehExpCode(strOpCd, strParam, objRptDs, False)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=GL_SETUP_RPT_VEHICLEEXPENSE_GET&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try

        Try
            intErrNo = objAdmin.mtdGetBasePath(objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=GL_SETUP_RPT_VEHICLEEXPENSE_GET_MAPPATH&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try

        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("VehExpenseCode") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("VehExpenseCode"))
            ...
            objRptDs.Tables(0).Rows(intCnt).Item("VehExpenseCode") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("VehExpenseCode"))
            objRptDs.Tables(0).Rows(intCnt).Item("RunCostInd") = objGLSetup.mtdGetVehRuningCost(Trim(objRptDs.Tables(0).Rows(intCnt).Item("RunCostInd")))

        
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objGLSetup.mtdGetVehicleExpenseStatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
            objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate") = objGlobal.GetLongDate(objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate"))
        Next

        rdCrystalViewer.Load(objMapPath & "Web\EN\GL\Reports\Crystal\GL_Rpt_VehExpense.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\GL_Rpt_VehExpense.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/GL_Rpt_VehExpense.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        paramField3 = ParamFields.Item("strVehExpCodeTag")
        paramField4 = ParamFields.Item("strDescTag")
        paramField5 = ParamFields.Item("strTitleTag")
        paramField6 = ParamFields.Item("strVehExpGrpCodeTag")
        ...
        ParamDiscreteValue3.value = strVehExpCodeTag
        ParamDiscreteValue4.value = strDescTag
        ParamDiscreteValue5.value = UCase(strTitleTag)
        ParamDiscreteValue6.value = strVehExpGrpCodeTag
