...
Public Class IN_StockMaster_RequestDetailAdd : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            If Not IsPostBack Then
                If Not Session("DOCID") = "" And ItemCode = "" Then
                    LblNoDoc.Text = Session("DOCID")
                    DisplayData_ItemRequest(LblNoDoc.Text)
                    DisplayDropDown()
                    DisplayDropDown_ItemRequest()
                End If

                If Not ItemCode = "" And Session("DOCID") = "" Then
                    DisplayData()
                    DisplayDropDown()
    ...
    Protected Function LoadData() As DataSet
        ...
        strParam = ItemCode & "|" & objIN.EnumInventoryItemType.Stock & "','" & objIN.EnumInventoryItemType.WorkshopItem & "|" & Trim(strLocation)
    ...
    Sub DisplayData_ItemRequest(ByVal pKode As String)
        dsStockItem = LoadData_ItemRequest(pKode)
    ...
    Protected Function LoadData_ItemRequest(ByVal pKode As String) As DataSet
    ...
    Sub DisplayDropDown_ItemRequest()
        Dim strOppCd_ProdType_GET As String = "IN_CLSSETUP_PRODTYPE_LIST_GET"
        Dim strOppCd_ProdCat_GET As String = "IN_CLSSETUP_PRODCAT_LIST_GET"
        Dim strOppCd_ProdBrand_GET As String = "IN_CLSSETUP_PRODBRAND_LIST_GET"
        Dim strOppCd_ProdModel_GET As String = "IN_CLSSETUP_PRODMODEL_LIST_GET"
        ...
        BindDropList_ItemRequest(strOppCd_ProdType_GET, "ProdTypeCode", "ProdTypeCode", objIN.EnumInventoryMasterType.ProductType, _
                       objGlobal.EnumModule.Inventory, objIN.EnumProductTypeStatus.Active, lstProdType)
        BindDropList_ItemRequest(strOppCd_ProdCat_GET, "ProdCatCode", "ProdCatCode", objIN.EnumInventoryMasterType.ProductCategory, _
                        objGlobal.EnumModule.Inventory, objIN.EnumProductCategoryStatus.Active, lstProdCat)
        BindDropList_ItemRequest(strOppCd_ProdBrand_GET, "ProdBrandCode", "ProdBrandCode", objIN.EnumInventoryMasterType.ProductBrand, _
                        objGlobal.EnumModule.Inventory, objIN.EnumProductBrandStatus.Active, lstProdBrand)
        BindDropList_ItemRequest(strOppCd_ProdModel_GET, "ProdModelCode", "ProdModelCode", objIN.EnumInventoryMasterType.ProductModel, _
                        objGlobal.EnumModule.Inventory, objIN.EnumProductModelStatus.Active, lstProdModel)
        BindDropList_ItemRequest(strOppCd_ProdMaterial_GET, "ProdMatCode", "ProdMatCode", objIN.EnumInventoryMasterType.ProductMaterial, _
                        objGlobal.EnumModule.Inventory, objIN.EnumProductMaterialStatus.Active, lstProdMat)
        BindDropList_ItemRequest(strOppCd_UOM_GET, "UOMCode", "UOMCode", "0", _
                         objGlobal.EnumModule.Admin, objAdmin.EnumUOMStatus.Active, InventoryUOM)
    End Sub

    Sub BindDropList_ItemRequest(ByVal Oppcode As String, ByVal ItemKeyField As String, ByVal MasterKeyField As String, ByVal MasterType As Integer, _
        ...
        If SelectedIndex = -1 And Not LblNoDoc.Text = "" Then
    ...
    Sub UpdateData(ByVal strAction As String)
        ...
        Dim strOppCd_EXISTS As String = "IN_CLSSETUP_INVMASTER_DETAILS_REQUEST_DETAIL_GET"
        Dim strOppCd_APR As String = "IN_CLSSETUP_INVMASTER_DETAILS_REQUEST_APPROVED"
        ...
        ''CARI DATA APAKAAH SUDAH TERSIMPAN
        strParamName = "ITEMCODE"
        strParamValue = Trim(StckItemCode.Text)

        Try
            intErrNo = ObjOk.mtdGetDataCommon(strOppCd_EXISTS, _
                                                    strParamName, _
                                                    strParamValue, objDetail)

        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=UPDATE_STOCKITEM&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_StockMaster.aspx")
        End Try

        If objDetail.Tables(0).Rows(0).Item("ItemCode") = "" Then
            UserMsgBox(Me, "Simpan Data Gagal..!!!" & vbCrLf & "Lakukan Simpan Ulang")
            Exit Sub
        End If
        ''========================
        ...
        strParamName = "NODOC|STATUS|APPROVED|ITEMCODE|UPDDATE"
        ...
        Try
            intErrNo = objGLTrx.mtdInsertDataCommon(strOppCd_APR, _
                                                    strParamName, _
                                                    strParam)
