...
Public Class PR_Trx_ADList : Inherits Page

    Protected WithEvents dgLine As DataGrid
    Protected WithEvents lbDelete As LinkButton    
    Protected WithEvents lblTracker As Label
    Protected WithEvents lstDropList As DropDownList
    Protected WithEvents txtADTrxId As TextBox
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "AD.ADTrxId"
            End If
            ...
            lbl = dgLine.Items.Item(intCnt).FindControl("lblStatus") 
           	Select Case CInt(Trim(lbl.Text)) 
                Case objPRTrx.EnumADTrxStatus.Active
                    lbl = dgLine.Items.Item(intCnt).FindControl("lblCountCloseLn")
                    If lbl.Text = 0 Then
                    ...
                    End If
                Case objPRTrx.EnumADTrxStatus.Cancelled, objPRTrx.EnumADTrxStatus.Closed
                    lbButton = dgLine.Items.Item(intCnt).FindControl("lbDelete")
                    lbButton.Visible = False
            End Select
        Next
    ...
    Sub Link_Click(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        If Not E.CommandName.ToString = "Sort" And Not E.CommandName.ToString = "Delete" Then
            ...
            lblCodeValue = dgLine.Items.Item(intIndex).FindControl("lblADId")
            Response.Redirect("PR_Trx_ADDet.aspx?adid=" & lblCodeValue.Text)
        End If
    End Sub
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "PR_CLSTRX_ADTRX_SEARCH"
        Dim strSrchADTrxId As String
        ...
        strSrchADTrxId = IIF(txtADTrxId.Text = "", "", txtADTrxId.Text)
        strSrchDesc = IIF(txtDescription.Text = "", "", txtDescription.Text)
        strSrchEmp = IIF(txtEmpCode.Text = "", "", txtEmpCode.Text)
        ...
        strParam = strSrchADTrxId & "|" & _
                   ...
                   SortCol.Text & "|"
        Try
            intErrNo = objPRTrx.mtdGetADTrx(strOpCd_GET, _
                                            strCompany, _
                                            strLocation, _
                                            strUserId, _
                                            strAccMonth, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_TRX_ADTRX_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        If objADDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objADDs.Tables(0).Rows.Count - 1
                objADDs.Tables(0).Rows(intCnt).Item("ADTrxId") = Trim(objADDs.Tables(0).Rows(intCnt).Item("ADTrxId"))
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        Dim strOpCd As String = "PR_CLSTRX_ADTRX_STATUS_UPD"
        Dim strParam As String = ""
        Dim strSelectedADTrxId As String 
        Dim intErrNo As Integer
        Dim objADTrxId As String
        Dim lblAD As Label

        dgLine.EditItemIndex = CInt(E.Item.ItemIndex)
        lblAD = dgLine.Items.Item(CInt(E.Item.ItemIndex)).FindControl("lblADId")
        strSelectedADTrxId = lblAD.Text
        strParam = strSelectedADTrxId & "|" & objPRTrx.EnumADTrxStatus.Cancelled
        ...
        Try
            intErrNo = objPRTrx.mtdUpdADTrx(strOpCd, _
                                           ...
                                           objADTrxId)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_SETUP_ADTRX_CANCEL&errmesg=" & lblErrMessage.Text & "&redirect=pr/trx/PR_trx_ADList.aspx")
        End Try
    ...
    Sub NewADBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("PR_trx_ADDet.aspx")
    End Sub
