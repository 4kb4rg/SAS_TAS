...
#Region "COMPONENT"
    ...
    Sub DEDR_Update(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        sTrPrLnID = dgPRListing.Items.Item(E.Item.ItemIndex).Cells(0).Text
        sTrPRID = dgPRListing.Items.Item(E.Item.ItemIndex).Cells(1).Text
        strItemCode = dgPRListing.Items.Item(E.Item.ItemIndex).Cells(2).Text
    ...
    Sub BtnApproved_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        nQtyAPP = CType(dgItem.Cells(11).FindControl("lstQtyApp"), TextBox).Text

        strParamName = "BOR|INTLEVEL|ITEM|PRLNID|USRID|APPSTATUS|QTYAPP"
        strParamValue = StrPRID & "|" & intLevel & "|" & strItemCode & "|" & StrPRLNID & "|" & strUserId & "|" & 3 & "|" & nQtyAPP
        ...
        If bUpdate = True Then
            UserMsgBox(Me, "Approved Sucsess...!!!")
        End If
...
#Region "PROCEDURE"
    ...
    Sub PageConTrol()
        For intCnt = 0 To dgPRListing.Items.Count - 1
            ...
            '''==control Button Approved
            If CType(dgPRListing.Items(intCnt).FindControl("lblAppLevel"), Label).Text = 4 _
                   Or CType(dgPRListing.Items(intCnt).FindControl("lblNextApp_Level"), Label).Text <> intLevel Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).Enabled = False
            Else
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).Enabled = True
            End If

            If CType(dgPRListing.Items(intCnt).FindControl("lblStatusln"), Label).Text = 2 Or _
                     CType(dgPRListing.Items(intCnt).FindControl("lblStatusln"), Label).Text = 4 Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).Enabled = False

            End If

            'If CType(dgPRListing.Items(intCnt).FindControl("lblStatusln"), Label).Text = 2 _
            '        Or CType(dgPRListing.Items(intCnt).FindControl("lblStatusln"), Label).Text <> 3 Then
            '    CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).Enabled = True
            'End If

            '''==control Linked edit jika Level Approved Lebih tinggi dari level yang login
            ...
            '' control warna dan tombol cancel hanya di level 4 dan jika sudah approved level 4 akan tampil

            If CType(dgPRListing.Items(intCnt).FindControl("lblAppBy_Level"), Label).Text = 0 Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).BackColor = Drawing.Color.Yellow
            ElseIf CType(dgPRListing.Items(intCnt).FindControl("lblAppBy_Level"), Label).Text = 1 Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).BackColor = Drawing.Color.LightGreen
            ElseIf CType(dgPRListing.Items(intCnt).FindControl("lblAppBy_Level"), Label).Text = 2 Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).BackColor = Drawing.Color.Blue
            ElseIf CType(dgPRListing.Items(intCnt).FindControl("lblAppBy_Level"), Label).Text = 3 Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).BackColor = Drawing.Color.Red
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).ForeColor = Drawing.Color.White
            ElseIf CType(dgPRListing.Items(intCnt).FindControl("lblAppBy_Level"), Label).Text = 4 Then
                CType(dgPRListing.Items(intCnt).FindControl("BtnApproved"), Button).Text = "Completed"
                If intLevel = 4 Then
                    If CType(dgPRListing.Items(intCnt).FindControl("lblPOIDLast"), Label).Text = "" Or _
                                CType(dgPRListing.Items(intCnt).FindControl("lblStatusln"), Label).Text <> 3 Then
                        CType(dgPRListing.Items(intCnt).FindControl("BtnCancel"), Button).Visible = False
                    Else
                        CType(dgPRListing.Items(intCnt).FindControl("BtnCancel"), Button).Visible = True
                        CType(dgPRListing.Items(intCnt).FindControl("BtnCancel"), Button).BackColor = Drawing.Color.SpringGreen
                    End If
                End If
            End If
    ...
    Protected Function LoadData() As DataSet
        ...
        StatusLn = "1,4"
        strParamName = "LOCCODE|STATUS|PRLEVEL"
        strParamValue = strLocation & "|" & StatusLn & "|" & intLevel
    ...
    Protected Function LoadSrch() As DataSet
        ...
        strParamName = "LOCCODE|STATUS|PRLEVEL|ACCMONTH|ACCYEAR|PRID|STATUSLN"
        strParamValue = strLocation & "|" & 1 & "|" & intLevel & "|" & strAccMonth & "|" & strAccYear & "|" & srchPRID.Text & "|" & srchStatusLnList.SelectedItem.Value
