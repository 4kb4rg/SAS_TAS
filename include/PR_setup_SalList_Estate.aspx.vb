...
Public Class PR_setup_SalList_Estate : Inherits Page
    ...
    Protected WithEvents txtEmpCode As DropDownList
    ...
    Sub BindEmpType()
        ...
        txtEmpCode.DataSource = objEmpTypeDs.Tables(0)
        txtEmpCode.DataTextField = "Description"
        txtEmpCode.DataValueField = "EmpTyCode"
        txtEmpCode.DataBind()
        txtEmpCode.SelectedIndex = 0
    ...
    Protected Function LoadData() As DataSet
        ...
        If (srcpmonth.SelectedItem.Value.Trim = "") Or (srcpyear.SelectedItem.Value.Trim = "") Then
            strSearch = "AND A.LocCode='" & strLocation & "' AND A.Status like '" & IIf(Not ddlStatus.SelectedItem.Value = "0", ddlStatus.SelectedItem.Value, "%") & "' "
        Else
            strSearch = " AND A.LocCode='" & strLocation & "' AND ('" & srcpyear.SelectedItem.Value.Trim & srcpmonth.SelectedItem.Value.Trim & "' >= right(rtrim(periodestart),4)+left(rtrim(periodestart),2)) AND " & _
        ...
        If Not txtSalCode.Text = "" Then
            strSearch = strSearch & " AND A.SalaryCode like '%" & _
                        txtSalCode.Text & "%' "
        End If
        If Not txtEmpCode.SelectedItem.Value.Trim = "" Then
            strSearch = strSearch & " AND CodeEmpTy like '%" & txtEmpCode.SelectedItem.Value.Trim & "%' "
        End If
        ...
        If objBankDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objBankDs.Tables(0).Rows.Count - 1
                objBankDs.Tables(0).Rows(intCnt).Item("SalaryCode") = Trim(objBankDs.Tables(0).Rows(intCnt).Item("SalaryCode"))
                objBankDs.Tables(0).Rows(intCnt).Item("DescEmpty") = Trim(objBankDs.Tables(0).Rows(intCnt).Item("DescEmpty"))
                'objBankDs.Tables(0).Rows(intCnt).Item("DescJobCode") = Trim(objBankDs.Tables(0).Rows(intCnt).Item("DescJobCode"))
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal e As DataGridCommandEventArgs)
        Dim strOpCd_UPD As String = "PR_PR_STP_EMPSALARY_LIST_DEL"
        ...
        ParamNama = "SC|ST|UD|UI|Loc"
        ParamValue = strSalCd & "|" & strStatus & "|" & DateTime.Now & "|" & strUserId & "|" & strLocation
    ...
    Sub NewSalBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PR_setup_Saldet_Estate.aspx")
    End Sub

    Sub DEDR_Gen(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Dim strOpCd As String = "PR_PR_STP_EMPSALARY_GEN"
        Dim intErrNo As Integer
        ...
        Try
            ...
            lblErrMessage.Text = "Proses generate setting upah sukses"
            lblErrMessage.ForeColor = Drawing.Color.Blue
        Catch Exp As System.Exception
            lblErrMessage.Text = "Error proses generate setting upah"
            lblErrMessage.ForeColor = Drawing.Color.Red
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_PR_STP_EMPSALARY_GEN&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
