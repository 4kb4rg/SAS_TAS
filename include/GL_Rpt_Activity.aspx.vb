...
Public Class GL_Rpt_Activity : inherits page
    ...
    Public Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strActCodeTag = Trim(Request.QueryString("strActCodeTag"))  
        strDescTag = Trim(Request.QueryString("strDescTag")) 
        strTitleTag = Trim(Request.QueryString("strTitleTag"))
        strActGrpCodeTag = Trim(Request.QueryString("strActGrpCodeTag"))
        strStatus = Trim(Request.QueryString("strStatus"))
        strActCode = Trim(Request.QueryString("strActCode"))
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd As String = "GL_CLSSETUP_ACTIVITY_LIST_GET"
        ...
        Try
            intErrNo = objGLSetup.mtdGetActivity(strOpCd, strParam, objRptDs, False)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=GL_RPT_ACTIVITY_GET&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try

        Try
            intErrNo = objAdmin.mtdGetBasePath(objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=GL_RPT_ACTIVITY_GET_PATH&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try

        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("ActCode") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("ActCode"))
            objRptDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("Description"))
            objRptDs.Tables(0).Rows(intCnt).Item("ActGrpCode") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("ActGrpCode"))
        ...
        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objGLSetup.mtdGetActSatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
            objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate") = objGlobal.GetLongDate(objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate"))
        Next


        rdCrystalViewer.Load(objMapPath & "Web\EN\GL\Reports\Crystal\GL_Rpt_Activity.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\GL_Rpt_Activity.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/GL_Rpt_Activity.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        paramField3 = ParamFields.Item("strActCodeTag")
        paramField4 = ParamFields.Item("strDescTag")
        paramField5 = ParamFields.Item("strTitleTag")
        paramField6 = ParamFields.Item("strActGrpCodeTag")
        ...
        ParamDiscreteValue3.value = strActCodeTag
        ParamDiscreteValue4.value = strDescTag
        ParamDiscreteValue5.value = UCase(strTitleTag)
        ParamDiscreteValue6.value = strActGrpCodeTag
