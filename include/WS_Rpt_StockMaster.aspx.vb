...
Public Class WS_Rpt_StockMaster : inherits page
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd As String = "WS_CLSSETUP_INVMASTER_LIST_GET_FOR_REPORT"
        ...
        strSearch =  "AND ItemType = '" & objIN.EnumInventoryItemType.WorkshopItem & "' AND itm.Status like  '" & IIF(Not strStatus = "", strStatus, "%" ) &"' "
        ...
        Try
            intErrNo = objIN.mtdGetMasterList(strOpCd, _
                                                     strParam, _
        ...
        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objIN.mtdGetStockItemStatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
        Next

        rdCrystalViewer.Load(objMapPath & "Web\EN\WS\Reports\Crystal\Ws_Rpt_StockMaster.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\WS_Rpt_StockMaster.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/WS_Rpt_StockMaster.pdf"">")
        End If
