...
Public Class NU_trx_CullDetails : Inherits Page
    ...
    Protected WithEvents lblCullTypeCodeTag As Label
    Protected WithEvents lblCullTypeCodeErr As Label
    ...
    Protected WithEvents ddlCullTypeCode As DropDownList
    ...
    Dim strOppCd_GET As String = "NU_CLSTRX_CULL_GET"
    Dim strOppCd_ADD As String = "NU_CLSTRX_CULL_ADD"
    Dim strOppCd_UPD As String = "NU_CLSTRX_CULL_UPD"
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            lblCullTypeCodeErr.Visible = False
            ...
            If Not Page.IsPostBack Then
                ...
                If Not lblTxID.Text = "" Then
                    ...
                    DisableControl()
                    If lblStatus.Text = objNUTrx.mtdGetCullStatus(objNUTrx.EnumCullStatus.Deleted) Then
                        Delete.Visible = False
                    ElseIf lblStatus.Text = objNUTrx.mtdGetCullStatus(objNUTrx.EnumCullStatus.Closed) Then
                        Delete.Visible = False
                    Else
                        Delete.Visible = True
    ...
    Sub onload_GetLangCap()
        ...
        lblCullTypeCodeTag.Text = GetCaption(objLangCap.EnumLangCap.CullType)
        lblBlkCodeErr.Text = "Please select " & GetCaption(objLangCap.EnumLangCap.NurseryBlock) & "."
        lblBatchNoErr.Text = "Please select " & GetCaption(objLangCap.EnumLangCap.BatchNo) & "."
        lblCullTypeCodeErr.Text = "Please select " & GetCaption(objLangCap.EnumLangCap.CullType) & "."
    End Sub
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_GET_LANGCAP&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_CullDetails.aspx")
        End Try
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objNUTrx.mtdGetCull(strCompany, _
                                            ...
                                            objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_GET&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_CullList.aspx")
        End Try
    ...
    Sub DisableControl()
        ...
        ddlCullTypeCode.Enabled = strView
    ...
    Sub DisplayData()
        ...
        If dsTx.Tables(0).Rows.Count > 0 Then
            lblTxID.Text = Trim(dsTx.Tables(0).Rows(0).Item("CullID"))
            txtDate.Text = objGlobal.GetShortDate(strDateFormat, Trim(dsTx.Tables(0).Rows(0).Item("CullDate")))
            ...
            lblStatus.Text = objNUTrx.mtdGetCullStatus(Trim(dsTx.Tables(0).Rows(0).Item("Status")))
            ...
            BindCullTypeCode(Trim(dsTx.Tables(0).Rows(0).Item("CullTypeCode")))
    ...
    Sub BindBlkCode(ByVal pv_strBlkCode As String)
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_BIND_BLOCK_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_CullDetails.aspx")
        End Try
        ...
        If intSelectedIndex = 0 And Not lblTxID.Text = "" Then
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_BIND_BLOCK_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_CullDetails.aspx")
            End Try
        End If
    ...
    Sub BindBatchNo(ByVal pv_strBlkCode As String, ByVal pv_strBatchNo As String)
        ...
        If Not pv_strBlkCode = "" Then
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_BIND_BATCHNO_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_CullDetails.aspx")
            End Try
            ...
            ddlBatchNo.DataTextField = "BatchDescr"
            ddlBatchNo.DataBind()
            ...
            If intSelectedIndex = -1 And Not lblTxID.Text = "" Then
                ...
                Catch Exp As System.Exception
                    Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_BIND_BATCHNO_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_CullDetails.aspx")
                End Try
            End If
    ...
    Sub BindCullTypeCode(ByVal pv_strCullTypeCode As String)
        Dim strOpCode As String = "NU_CLSSETUP_CULLTYPE_LIST_GET"
        Dim strParam As String
        Dim dsForDropDown As DataSet
        Dim dsForInactiveItem As DataSet
        Dim intSelectedIndex As Integer = 0
        ...
        strParam = "||" & objNUSetup.EnumCullTypeStatus.Active & "||" & _
                    "CT.CullTypeCode ASC "

        Try
            intErrNo = objNUSetup.mtdGetCullType(strOpCode, strParam, dsForDropDown)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_BIND_CULLTYPE_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_CullDetails.aspx")
        End Try

        For intCnt = 0 To dsForDropDown.Tables(0).Rows.Count - 1
            dsForDropDown.Tables(0).Rows(intCnt).Item("CullTypeCode") = dsForDropDown.Tables(0).Rows(intCnt).Item("CullTypeCode").Trim()
            dsForDropDown.Tables(0).Rows(intCnt).Item("Description") = dsForDropDown.Tables(0).Rows(intCnt).Item("CullTypeCode").Trim() & " (" & dsForDropDown.Tables(0).Rows(intCnt).Item("Description").Trim() & ")"
            If dsForDropDown.Tables(0).Rows(intCnt).Item("CullTypeCode") = Trim(pv_strCullTypeCode) Then
                intSelectedIndex = intCnt + 1
            End If
        Next intCnt
        ...
        dr = dsForDropDown.Tables(0).NewRow()
        dr("CullTypeCode") = ""
        dr("Description") = "Select " & lblCullTypeCodeTag.Text
        dsForDropDown.Tables(0).Rows.InsertAt(dr, 0)

        ddlCullTypeCode.DataSource = dsForDropDown.Tables(0)
        ddlCullTypeCode.DataValueField = "CullTypeCode"
        ddlCullTypeCode.DataTextField = "Description"
        ddlCullTypeCode.DataBind()

        If intSelectedIndex = 0 And Not lblTxID.Text = "" Then
            strParam = pv_strCullTypeCode & "||||" & _
                        "CT.CullTypeCode ASC "

            Try
                intErrNo = objNUSetup.mtdGetCullType(strOpCode, strParam, dsForInactiveItem)
                If dsForInactiveItem.Tables(0).Rows.Count > 0 Then  
                    ddlCullTypeCode.Items.Add(New ListItem(Trim(dsForInactiveItem.Tables(0).Rows(0).Item("CullTypeCode")) & _
                     " (" & objNUSetup.mtdGetCullTypeStatus(Trim(dsForInactiveItem.Tables(0).Rows(0).Item("Status"))) & ") ", Trim(dsForInactiveItem.Tables(0).Rows(0).Item("CullTypeCode"))))
                    intSelectedIndex = ddlCullTypeCode.Items.Count - 1
                Else 
                    intSelectedIndex = 0
                End If

            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULL_DET_BIND_CULLTYPE_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_CullDetails.aspx")
            End Try
        End If

        ddlCullTypeCode.SelectedIndex = intSelectedIndex
    End Sub
    ...
    Sub UpdateData(ByVal strAction As String)
        ...
        If Not strDate = "" Then
            Select Case strAction
                Case "Confirm"
                    strStatus = objNUTrx.EnumCullStatus.Confirmed
                Case "Delete"
                    strStatus = objNUTrx.EnumCullStatus.Deleted
            End Select

            strNewIDFormat = "SCU" & "/" & strCompany & "/" & strLocation & "/" & strAccYear & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/"

            strParam = lblTxID.Text & "|" & _
                        ddlBlkCode.SelectedItem.Value & "|" & _
                        ddlBatchNo.SelectedItem.Value & "|" & _
                        strDate & "|" & _
                        ddlCullTypeCode.SelectedItem.Value & "|" & _
                        txtQty.Text & "|" & _
                        Trim(txtAddNote.Value) & "|" & _
                        Trim(strNewIDFormat)

            Try
                intErrNo = objNUTrx.mtdUpdCull(strOppCd_ADD, _
                                                    strOppCd_UPD, _
                                                    strOppCd_GET, _
                                                    strLocation, _
                                                    strUserId, _
                                                    ...
                                                    intError, _
                                                    lblOper.Text, _
                                                    objGlobal.mtdGetDocPrefix(objGlobal.EnumDocType.Culling), _
                                                    TxID, _
                                                    blnDeleteErr, _
                                                    intConfigSetting)
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_CULLDETAILS_UPDATE&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_CullDetails.aspx")
            End Try
            ...
            Else
                If blnDeleteErr = False Then
                    Response.Redirect("NU_trx_CullList.aspx")
                End If
            End If
        End If
    ...
    Sub btnConfirm_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        If ddlBatchNo.SelectedItem.Value = "" Then
            ...
            Exit Sub
        End If
        If ddlCullTypeCode.SelectedItem.Value = "" Then
            lblCullTypeCodeErr.Visible = True
            Exit Sub
        End If
        UpdateData("Confirm")
    End Sub
