...
Public Class HR_Setup_Level : Inherits Page
    ...
    Protected WithEvents srchLevelCode as TextBox
    ...
    Dim strOppCd_GET As String = "HR_CLSSETUP_LEVEL_LIST_GET"
    Dim strOppCd_ADD As String = "HR_CLSSETUP_LEVEL_LIST_ADD"
    Dim strOppCd_UPD As String = "HR_CLSSETUP_LEVEL_LIST_UPD"
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumHRAccessRights.HRLevel), intHRAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")

        Else
            onload_GetLangCap()
            If SortExpression.text = "" Then
                SortExpression.text = "LevelCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objHR.mtdGetLevelStatus(objHR.EnumLevelStatus.Active), objHR.EnumLevelStatus.Active))
        StatusList.Items.Add(New ListItem(objHR.mtdGetLevelStatus(objHR.EnumLevelStatus.Deleted), objHR.EnumLevelStatus.Deleted))

    End Sub 

    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objHR.mtdGetLevelStatus(objHR.EnumLevelStatus.All), objHR.EnumLevelStatus.All))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetLevelStatus(objHR.EnumLevelStatus.Active), objHR.EnumLevelStatus.Active))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetLevelStatus(objHR.EnumLevelStatus.Deleted), objHR.EnumLevelStatus.Deleted))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND LVL.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objHR.EnumLevelStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchLevelCode.text = "" Then
            SearchStr =  SearchStr & " AND LVL.LevelCode like '" & srchLevelCode.text &"%' "
        End If
        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND LVL.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objHR.mtdGetMasterList(strOppCd_GET, strParam, objHR.EnumHRMasterType.Level, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_LEVEL_GET_LIST&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        Return objDataSet
    End Function
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objHR.EnumLevelStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("LevelCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("LevelCode")
        LevelCode = EditText.Text
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          strUserId, _
                                          strParam, _
                                          objHR.EnumHRMasterType.Level, _
                                          blnDupKey, _
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_UPDATE_LEVEL&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Level.aspx")
        End Try
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objHR.EnumLevelStatus.Active, _
                        objHR.EnumLevelStatus.Deleted, _
                        objHR.EnumLevelStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  LevelCode & "|" & _
                    Description & "|" & _
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_DELETE_LVEL&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Level.aspx")
        End Try
    ...
    Sub onload_GetLangCap()
        ...
        lblTitle.text = UCase(GetCaption(objLangCap.EnumLangCap.Level))
        lblLevel.text = GetCaption(objLangCap.EnumLangCap.Level)
        lblDesc.text = GetCaption(objLangCap.EnumLangCap.LevelDesc)

        strValidateCode = lblValidate.text & lblLevel.text & lblCode.text & "."
        strValidateDesc = lblValidate.text & lblDesc.text & "." 

        EventData.Columns(0).HeaderText = lblLevel.text & lblCode.text
    ...
    Sub GetEntireLangCap()
        ...
        Try
            intErrNo = objLangCap.mtdGetBussTerm(strOpCode_BussTerm, _
                                                 ...
                                                 strParam)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PU_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Level.aspx")
        End Try
