...
Public Class NU_trx_SeedPlantDetails : Inherits Page
    ...
    Dim strOppCd_GET As String = "NU_CLSTRX_SEEDPLANT_GET"
    Dim strOppCd_ADD As String = "NU_CLSTRX_SEEDPLANT_ADD"
    Dim strOppCd_UPD As String = "NU_CLSTRX_SEEDPLANT_UPD"
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumNUAccessRights.NUSeedPlant), intNUAR) = False Then
        ...
        Else
            ...
            If Not Page.IsPostBack Then
                ...
                If Not lblTxID.Text = "" Then
                    ...
                    DisableControl()
                    If lblStatus.Text = objNUTrx.mtdGetSeedPlantStatus(objNUTrx.EnumSeedPlantStatus.Deleted) Then
                        Delete.Visible = False
                    ElseIf lblStatus.Text = objNUTrx.mtdGetSeedPlantStatus(objNUTrx.EnumSeedPlantStatus.Closed) Then
                        Delete.Visible = False
                    Else
                        Delete.Visible = True
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANT_DET_GET_LANGCAP&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedPlantDetails.aspx")
        End Try
    End Sub
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objNUTrx.mtdGetSeedPlant(strCompany, _
                                                strLocation, _
                                                strUserId, _
                                                strAccMonth, _
                                                strAccYear, _
                                                ...
                                                objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANT_GET&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedPlantList.aspx")
        End Try
    ...
    Sub DisplayData()
        ...
        If dsTx.Tables(0).Rows.Count > 0 Then
            lblTxID.Text = Trim(dsTx.Tables(0).Rows(0).Item("PlantID"))
            txtDate.Text = objGlobal.GetShortDate(strDateFormat, Trim(dsTx.Tables(0).Rows(0).Item("PlantDate")))
            ...
            lblStatus.Text = objNUTrx.mtdGetSeedPlantStatus(Trim(dsTx.Tables(0).Rows(0).Item("Status")))
            ...
            lblCostSeeds.Text = objGlobal.GetIDDecimalSeparator(FormatNumber(objNUTrx.mtdGetCostSeedsPlanting(strOpCdStckTxLine_CostGET, _
                                                        strCompany, _
                                                        strLocation, _
                                                        strUserId, _
                                                        strAccMonth, _
    ...
    Sub BindBlkCode(ByVal pv_strBlkCode As String)
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANT_DET_BIND_BLOCK_DROPDOWNLIST&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedPlantDetails.aspx")
        End Try
        ...
        If intSelectedIndex = 0 And Not lblTxID.Text = "" Then
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANT_DET_BIND_BLOCK_DROPDOWNLIST_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedPlantDetails.aspx")
            End Try
        End If
    ...
    Sub BindBatchNo(ByVal pv_strBlkCode As String, ByVal pv_strBatchNo As String)
        ...
        If Not pv_strBlkCode = "" Then
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANT_DET_BIND_BATCHNO&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedPlantDetails.aspx")
            End Try
            ...
            If intSelectedIndex = -1 And Not lblTxID.Text = "" Then
                ...
                Catch Exp As System.Exception
                    Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANT_DET_BIND_BATCHNO_2&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedPlantDetails.aspx")
                End Try
            End If
    ...
    Sub UpdateData(ByVal strAction As String)
        ...
        If Not strDate = "" Then
            Select Case strAction
                Case "Confirm"
                    strStatus = objNUTrx.EnumSeedPlantStatus.Confirmed
                Case "Delete"
                    strStatus = objNUTrx.EnumSeedPlantStatus.Deleted
            End Select

            strNewIDFormat = "SPL" & "/" & strCompany & "/" & strLocation & "/" & strAccYear & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/"
            ...
            Try
                intErrNo = objNUTrx.mtdUpdSeedPlant(strOppCd_ADD, _
                                                    strOppCd_UPD, _
                                                    strOppCd_GET, _
                                                    strLocation, _
                                                    strUserId, _
                                                    ...
                                                    intError, _
                                                    lblOper.Text, _
                                                    objGlobal.mtdGetDocPrefix(objGlobal.EnumDocType.SeedPlant), _
                                                    TxID, _
                                                    blnDeleteErr)
            ...
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDPLANTDETAILS_UPDATE&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedPlantDetails.aspx")
            End Try
            ...
            Else
                'If blnDeleteErr = False Then
                    Response.Redirect("NU_trx_SeedPlantList.aspx")
                'End If
            End If
        End If
    ...
    Sub CallFillBatchNo(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'txtQty.Text = cDbl(objNUTrx.mtdGetQtySeedsPlanting(strOpCdStckTxLine_QtyGET, _
        '                                                strCompany, _
        '                                                strLocation, _
        '                                                strUserId, _
        '                                                strAccMonth, _
    ...
    Sub GetSeedsQtyCost()
        Dim strOpCode_NUSeedRcv As String = "NU_CLSTRX_SEEDS_GET_QTYCOST"
        Dim strParamName As String = ""
        Dim strParamValue As String = ""
        Dim dsDataNU As DataSet

        strParamName = "LOCCODE|BATCHNO"
        strParamValue = Trim(strLocation) & _
                        "|" & Trim(ddlBatchNo.SelectedItem.Value)

        Try
            intErrNo = objGLtrx.mtdGetDataCommon(strOpCode_NUSeedRcv, _
                                                    strParamName, _
                                                    strParamValue, _
                                                    dsDataNU)
        ...
        If dsDataNU.Tables(0).Rows.Count > 0 Then
            txtQty.Text = dsDataNU.Tables(0).Rows(0).Item("QtyRemain")
            lblCostSeeds.Text = objGlobal.GetIDDecimalSeparator(FormatNumber(dsDataNU.Tables(0).Rows(0).Item("Cost"), 0))
        End If
    End Sub
