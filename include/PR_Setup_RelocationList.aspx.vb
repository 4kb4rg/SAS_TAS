...
Public Class PR_Setup_RelocationList : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "RelocationCode"
            End If
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        Dim strOpCd As String = "PR_CLSSETUP_RELOCATION_STATUS_UPD"
        Dim strParam As String = ""
       ...
       strSelectedRelocationCode = CPCell.Text       
       strParam =  strSelectedRelocationCode & "|" & objPRSetup.EnumRelocationStatus.Deleted & "||"
     
        Try
            intErrNo = objPRSetup.mtdUpdRelocation(strOpCd, _
                                           strCompany, _
                                           strLocation, _
                                           strUserId, _
                                           strParam)
        ...
        Catch Exp As Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_SETUP_RELOCATION_DEL&errmesg=" & lblErrMessage.Text & "&redirect=hr/setup/PR_Setup_RelocationList.aspx")
        End Try
    ...
    Sub NewADBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("PR_Setup_RelocationDet.aspx")
    End Sub
    ...
    Sub BindGrid()        
        ...
        col.HeaderText = GetCaption(objLangCap.EnumLangCap.Relocation) & " Code"
        dgLine.DataBind()
        BindPageList()
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr = " AND R.Status like '" & IIf(ddlStatus.SelectedItem.Value <> objPRSetup.EnumRelocationStatus.All, ddlStatus.SelectedItem.Value, "%") & "' "

        If Not txtRelocationCode.Text = "" Then
            SearchStr = SearchStr & " AND R.RelocationCode like '" & txtRelocationCode.Text & "%' "
        End If
        If Not txtDescription.Text = "" Then
            SearchStr = SearchStr & " AND R.RelocationDesc like '" & txtDescription.Text & "%' "
        End If
        ...
        If objDataSet.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objDataSet.Tables(0).Rows.Count - 1
                objDataSet.Tables(0).Rows(intCnt).Item("RelocationCode") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("RelocationCode"))
                objDataSet.Tables(0).Rows(intCnt).Item("RelocationDesc") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("RelocationDesc"))
                objDataSet.Tables(0).Rows(intCnt).Item("Type") = objPRSetup.mtdGetRelocationType(Trim(objDataSet.Tables(0).Rows(intCnt).Item("Type")))                           
