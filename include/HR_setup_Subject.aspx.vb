...
Public Class HR_Setup_Subject : Inherits Page
    ...
    Protected WithEvents srchSubjectCode as TextBox
    ...
    Dim strOppCd_GET As String = "HR_CLSSETUP_SUBJECT_LIST_GET"
    Dim strOppCd_ADD As String = "HR_CLSSETUP_SUBJECT_LIST_ADD"
    Dim strOppCd_UPD As String = "HR_CLSSETUP_SUBJECT_LIST_UPD"
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumHRAccessRights.HRSubject), intHRAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")

        Else

            If SortExpression.text = "" Then
                SortExpression.text = "SubjectCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objHR.mtdGetSubjectStatus(objHR.EnumSubjectStatus.Active), objHR.EnumSubjectStatus.Active))
        StatusList.Items.Add(New ListItem(objHR.mtdGetSubjectStatus(objHR.EnumSubjectStatus.Deleted), objHR.EnumSubjectStatus.Deleted))

    End Sub 

    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objHR.mtdGetSubjectStatus(objHR.EnumSubjectStatus.All), objHR.EnumSubjectStatus.All))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetSubjectStatus(objHR.EnumSubjectStatus.Active), objHR.EnumSubjectStatus.Active))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetSubjectStatus(objHR.EnumSubjectStatus.Deleted), objHR.EnumSubjectStatus.Deleted))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND SU.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objHR.EnumSubjectStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchSubjectCode.text = "" Then
            SearchStr =  SearchStr & " AND SU.SubjectCode like '" & srchSubjectCode.text &"%' "
        End If
        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND SU.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objHR.mtdGetMasterList(strOppCd_GET, strParam, objHR.EnumHRMasterType.Subject, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SUBJECT_GET_LIST&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        Return objDataSet
    End Function
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objHR.EnumSubjectStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("SubjectCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("SubjectCode")
        SubjectCode = EditText.Text
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          strUserId, _
                                          strParam, _
                                          objHR.EnumHRMasterType.Subject, _
                                          blnDupKey, _
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_UPDATE_SUBJECT&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Subject.aspx")
        End Try
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objHR.EnumSubjectStatus.Active, _
                                     objHR.EnumSubjectStatus.Deleted, _
                                     objHR.EnumSubjectStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  SubjectCode & "|" & _
                    Description & "|" & _
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_DELETE_SUBJECT&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Subject.aspx")
        End Try
