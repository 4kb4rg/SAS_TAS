...
Public Class HR_setup_MPOBbonusList : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "B.BonusCode"
            End If
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "HR_CLSSETUP_MPOBBONUS_SEARCH"
        Dim strSrchBonusCode As String
        ...
        strSrchBonusCode = IIF(txtBonusCode.Text = "", "", txtBonusCode.Text)
        ...
        strParam = strSrchBonusCode & "|" & _
                   ...
                   SortCol.Text & "|"
        Try
            intErrNo = objHRSetup.mtdGetBonus(strOpCd_GET, strParam, objBonusDs, False)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_MPOBBONUS_GET&errmesg=" & Exp.ToString() & "&redirect=")
        End Try

        If objBonusDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objBonusDs.Tables(0).Rows.Count - 1
                objBonusDs.Tables(0).Rows(intCnt).Item("BonusCode") = objBonusDs.Tables(0).Rows(intCnt).Item("BonusCode").Trim()
                objBonusDs.Tables(0).Rows(intCnt).Item("Description") = objBonusDs.Tables(0).Rows(intCnt).Item("Description").Trim()
                objBonusDs.Tables(0).Rows(intCnt).Item("UserName") = objBonusDs.Tables(0).Rows(intCnt).Item("UserName").Trim()
            Next
        End If
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        Dim strOpCd As String = "HR_CLSSETUP_MPOBBONUS_STATUS_UPD"
        Dim strParam As String = ""
        Dim CPCell As TableCell = e.Item.Cells(0)
        Dim strBonusCode As String 
        Dim intErrNo As Integer

        strBonusCode = CPCell.Text
        strParam = strBonusCode & "|" & objHRSetup.EnumBonusStatus.Deleted
        ...
        Try
            intErrNo = objHRSetup.mtdUpdBonus(strOpCd, _
                                             ...
                                             True)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_MPOBBONUS_DEL&errmesg=" & Exp.ToString() & "&redirect=hr/setup/HR_setup_MPOBbonusList.aspx")
        End Try
    ...
    Sub NewBonusBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("HR_setup_MPOBbonusDet.aspx")
    End Sub
