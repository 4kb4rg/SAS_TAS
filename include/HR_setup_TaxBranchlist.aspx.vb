...
Public Class HR_setup_TaxBranchList : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "TB.TaxBranchCode"
            End If
    ...
    Sub BindGrid() 
        ...
        For intCnt = 0 To dgLine.Items.Count - 1
            ...
            Select Case CInt(Trim(lbl.Text))
                Case objHRSetup.EnumTaxBranchStatus.Active
                        lbButton = dgLine.Items.Item(intCnt).FindControl("lbDelete")
                        lbButton.Visible = True
                        lbButton.Attributes("onclick") = "javascript:return ConfirmAction('delete');"
                Case objHRSetup.EnumTaxBranchStatus.Deleted
                        lbButton = dgLine.Items.Item(intCnt).FindControl("lbDelete")
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "HR_CLSSETUP_TAXBRANCH_SEARCH"
        Dim strSrchTaxBranchCode As String
        ...
        strSrchTaxBranchCode = IIF(txtTaxBranchCode.Text = "", "", txtTaxBranchCode.Text)
        ...
        strParam = strSrchTaxBranchCode & "|" & _
                   strSrchDesc & "|" & _
                   strSrchStatus & "|" & _
                   strSrchLastUpdate & "|" & _
        ...
        Try
            intErrNo = objHRSetup.mtdGetTaxBranch(strOpCd_GET, strParam, objTaxBranchDs, False)
        ...
        If objTaxBranchDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objTaxBranchDs.Tables(0).Rows.Count - 1
                ...
                objTaxBranchDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objTaxBranchDs.Tables(0).Rows(intCnt).Item("Description"))
                objTaxBranchDs.Tables(0).Rows(intCnt).Item("Status") = Trim(objTaxBranchDs.Tables(0).Rows(intCnt).Item("Status"))
                objTaxBranchDs.Tables(0).Rows(intCnt).Item("UpdateID") = Trim(objTaxBranchDs.Tables(0).Rows(intCnt).Item("UpdateID"))
                objTaxBranchDs.Tables(0).Rows(intCnt).Item("UserName") = Trim(objTaxBranchDs.Tables(0).Rows(intCnt).Item("UserName"))
            Next
        End If

        Return objTaxBranchDs
    End Function
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        ...
        strSelectedTaxCode = CPCell.Text
        strParam = strSelectedTaxCode & Chr(9) & Chr(9) & Chr(9) & Chr(9) & objHRSetup.EnumTaxBranchStatus.Deleted
        ...
        Try
            intErrNo = objHRSetup.mtdUpdTaxBranch(strOpCd_ADD, _
                                                ...
                                                True)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_TAXBRANCH_DEL&errmesg=" & lblErrMessage.Text & "&redirect=hr/setup/HR_setup_TaxBranchList.aspx")
        End Try
    ...
    Sub NewTBBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("HR_setup_TaxBranchdet.aspx")
    End Sub
