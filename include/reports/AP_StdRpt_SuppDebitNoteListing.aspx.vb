...
Public Class AP_StdRpt_SuppDebitNoteListing : Inherits Page
    ...
    Protected WithEvents lblErrSuppDebitNoteRefDateFrom As Label
    ...
    Protected WithEvents txtSuppDebitNoteRefNo As TextBox
    ...
    Protected WithEvents txtSuppDebitNoteIDFrom As TextBox
    Protected WithEvents txtSuppDebitNoteIDTo As TextBox
    Protected WithEvents txtSuppDebitNoteRefDateFrom As TextBox
    Protected WithEvents txtSuppDebitNoteRefDateTo As TextBox
    ...
    Sub BindStatus()
        ...
        ddlStatus.Items.Add(New ListItem(objAP1.mtdGetDebitNoteStatus(objAP1.EnumInvoiceRcvStatus.Active), objAP1.EnumInvoiceRcvStatus.Active))
        ddlStatus.Items.Add(New ListItem(objAP1.mtdGetDebitNoteStatus(objAP1.EnumInvoiceRcvStatus.Confirmed), objAP1.EnumInvoiceRcvStatus.Confirmed))
        ddlStatus.Items.Add(New ListItem(objAP1.mtdGetDebitNoteStatus(objAP1.EnumInvoiceRcvStatus.Deleted), objAP1.EnumInvoiceRcvStatus.Deleted))
        ddlStatus.Items.Add(New ListItem(objAP1.mtdGetDebitNoteStatus(objAP1.EnumInvoiceRcvStatus.Cancelled), objAP1.EnumInvoiceRcvStatus.Cancelled))
        ddlStatus.Items.Add(New ListItem(objAP1.mtdGetDebitNoteStatus(objAP1.EnumInvoiceRcvStatus.WriteOff), objAP1.EnumInvoiceRcvStatus.WriteOff))
        ddlStatus.Items.Add(New ListItem(objAP1.mtdGetDebitNoteStatus(objAP1.EnumInvoiceRcvStatus.Closed), objAP1.EnumInvoiceRcvStatus.Closed))
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        strDateFrom = Trim(txtSuppDebitNoteRefDateFrom.Text)
        strDateTo = Trim(txtSuppDebitNoteRefDateTo.Text)
        ...
        If Not (strDateFrom = "" And strDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDateFrom, objDateFormat, objDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDateTo, objDateFormat, objDateTo) = True Then
                Response.Write("<Script Language=""JavaScript"">window.open(""AP_StdRpt_SuppDebitNoteListingPreview.aspx?CompName=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                            "&DDLAccMth=" & strddlAccMth & "&DDLAccYr=" & strddlAccYr & "&lblLocation=" & lblLocation.Text & _
                            "&txtSuppDebitNoteRefNo=" & Server.UrlEncode(Trim(txtSuppDebitNoteRefNo.Text)) & "&txtSuppDebitNoteRefDateFrom=" & objDateFrom & "&txtSuppDebitNoteRefDateTo=" & objDateTo & _
                            ...
                            "&txtSuppDebitNoteIDFrom=" & Server.UrlEncode(Trim(txtSuppDebitNoteIDFrom.Text)) & "&txtSuppDebitNoteIDTo=" & Server.UrlEncode(Trim(txtSuppDebitNoteIDTo.Text)) & _
