...
Public Class CT_StdRpt_PRList_Preview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Session("SS_PRNoFrom") = Request.QueryString("PRNoFrom")
        Session("SS_PRNoTo") = Request.QueryString("PRNoTo")
        Session("SS_PRStatus") = Request.QueryString("PRStatus")
        ...
        Session("SS_PRLnStatus") = Request.QueryString("PRLnStatus")
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR As String = "CT_STDRPT_PR_AND_PRLN_GET"

        Dim WildStr As String = "AND PR.PRID *= PRLn.PRID AND "

        Dim NormStr As String = "AND PR.PRID = PRLn.PRID AND ITM.ItemCode = PRLN.ItemCode AND "
        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = "(DateDiff(Day, '" & Session("SS_DATEFROM") & "', PR.CreateDate) >= 0) And (DateDiff(Day, '" & Session("SS_DATETO") & "', PR.CreateDate) <= 0) AND "
        End If

        If Not (Request.QueryString("PRNoFrom") = "" And Request.QueryString("PRNoTo") = "") Then
            SearchStr = SearchStr & "PR.PRID IN (SELECT SUBPR.PRID FROM IN_PR SUBPR WHERE SUBPR.PRID >= '" & Request.QueryString("PRNoFrom") & _
                        "' AND SUBPR.PRID <= '" & Request.QueryString("PRNoTo") & "') AND "
        End If

        If Request.QueryString("PRStatus") = "All" Then
            SearchStr = SearchStr & "PR.Status LIKE '%' AND "
        ElseIf Request.QueryString("PRStatus") = "Closed" Then
            SearchStr = SearchStr & "PR.Status = '" & objCTTrx.EnumPurReqStatus.Fulfilled & "' AND "
        ElseIf Request.QueryString("PRStatus") = "Outstanding" Then
            SearchStr = SearchStr & "PR.Status = '" & objCTTrx.EnumPurReqStatus.Confirmed & "' AND "
        End If

        If Not Request.QueryString("ProdType") = "" Then
            itmSearchStr = "ITM.ProdTypeCode LIKE '" & Request.QueryString("ProdType") & "' AND "
        Else
            itmSearchStr = "ITM.ProdTypeCode LIKE '%' AND "
        End If
        ...
        Else
            itmSearchStr = itmSearchStr & "ITM.ProdBrandCode LIKE '%' AND "
        End If
        ...
        Else
            itmSearchStr = itmSearchStr & "ITM.StockAnalysisCode LIKE '%' AND "
        End If

        If Not Request.QueryString("ItemCode") = "" Then
            SearchStr = SearchStr & "PRLN.ItemCode LIKE '" & Request.QueryString("ItemCode") & "' AND "
        Else
            SearchStr = SearchStr & "PRLN.ItemCode LIKE '%' AND "
        End If

        If Request.QueryString("PRLnStatus") = "All" Then
            SearchStr = SearchStr & "PRLN.Status LIKE '%' AND "
        ElseIf Request.QueryString("PRLnStatus") = "Closed" Then
            SearchStr = SearchStr & "PRLN.QtyOutstanding = 0 AND "
        ElseIf Request.QueryString("PRLnStatus") = "Outstanding" Then
            SearchStr = SearchStr & "PRLN.QtyOutstanding <> 0 AND "
        End If

        If Not SearchStr = "" Or Not itmSearchStr = "" Then
            ...
            strParam = strUserLoc & "|" & strAccMonth & "|" & strAccYear & "|" & objCTTrx.EnumPurReqDocType.CanteenPR & "|" & itmSearchStr & "|" & NormStr & SearchStr
        End If

        Try
            intErrNo = objCT.mtdGetReport_PurchaseRequestList(strOpCdPR, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_CT_PR_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        ...
        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\CT_StdRpt_PRList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\CT_StdRpt_PRList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/CT_StdRpt_PRList.pdf"">")
    End Sub
    ...
    Sub PassParam()
        ...
        paramField17 = paramFields.Item("ParamProdType")
        paramField18 = paramFields.Item("ParamProdBrand")
        paramField19 = paramFields.Item("ParamProdModel")
        paramField20 = paramFields.Item("ParamProdCat")
        paramField21 = paramFields.Item("ParamProdMat")
        paramField22 = paramFields.Item("ParamStkAna")
        paramField23 = paramFields.Item("lblProdTypeCode")
        paramField24 = paramFields.Item("lblProdBrandCode")
        paramField25 = paramFields.Item("lblProdModelCode")
        paramField26 = paramFields.Item("lblProdCatCode")
        paramField27 = paramFields.Item("lblProdMatCode")
        paramField28 = paramFields.Item("lblStkAnaCode")
        ...
        ParamDiscreteValue7.Value = Session("SS_PRNOFROM")
        ParamDiscreteValue8.Value = Session("SS_PRNOTO")
        ParamDiscreteValue9.Value = Session("SS_PRSTATUS")
        ParamDiscreteValue10.Value = Session("SS_PRLNSTATUS")
        ...
        ParamDiscreteValue15.Value = Session("SS_RPTID")
        ParamDiscreteValue16.Value = Session("SS_RPTNAME")
        ParamDiscreteValue17.Value = Session("SS_PRODTYPE")
        ParamDiscreteValue18.Value = Session("SS_PRODBRAND")
        ParamDiscreteValue19.Value = Session("SS_PRODMODEL")
        ParamDiscreteValue20.Value = Session("SS_PRODCAT")
        ParamDiscreteValue21.Value = Session("SS_PRODMAT")
        ParamDiscreteValue22.Value = Session("SS_STKANA")
        ParamDiscreteValue23.Value = Session("SS_LBLPRODTYPECODE")
        ParamDiscreteValue24.Value = Session("SS_LBLPRODBRANDCODE")
        ParamDiscreteValue25.Value = Session("SS_LBLPRODMODELCODE")
        ParamDiscreteValue26.Value = Session("SS_LBLPRODCATCODE")
        ParamDiscreteValue27.Value = Session("SS_LBLPRODMATCODE")
        ParamDiscreteValue28.Value = Session("SS_LBLSTKANACODE")
