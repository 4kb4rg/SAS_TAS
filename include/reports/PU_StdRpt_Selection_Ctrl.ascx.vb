...
Public Class PU_STDRPT_SELECTION_CTRL : Inherits UserControl
    ...
    Protected WithEvents TrLocList As HtmlTableRow

    Protected WithEvents TrCompList As HtmlTableRow
    Protected WithEvents cbCompAll As CheckBox
    Protected WithEvents cblCompList As CheckBoxList
    Protected WithEvents hidCompList As HtmlInputHidden
    Protected WithEvents lblCompListErr As Label
    ...
    Sub BindReportNameList()
        ...
        strParam = " WHERE ReportType = '" & Convert.ToString(objGlobal.EnumStdRptType.Purchasing) & "' AND Status = '" & Convert.ToString(objGlobal.EnumStdRptStatus.Active) & "'   AND Reportid IN (Select ReportID From SH_REPORT_USER Where UserId='" & strUserId & "') ORDER BY RptName"
        Try
...
'            Response.Redirect("../../en/reports/PU_StdRpt_POStmt.aspx?SelIndex=" & Convert.ToString(intSelectedIndex) & "&UserLoc=" & strUserLoc)
        ...
        ElseIf strSelectedIndex = "rptpu1000010" Then
            Response.Redirect("../../en/reports/PU_StdRpt_PORealization.aspx?SelIndex=" & Convert.ToString(intSelectedIndex) & "&UserLoc=" & strUserLoc)
        ...
        ElseIf strSelectedIndex = "rptpu1000019" Then
    ...
    Sub GetCompanyList()
        Dim strOpCd As String = "REPORT_CLSCOMP_COMPANY_LOCATION_LIST_GET"
    ...
    Sub CheckComp_Clicked(ByVal Sender As Object, ByVal E As EventArgs)
        Dim intCntCompAll As Integer = 0

        If cbCompAll.Checked Then
            For intCntCompAll = 0 To cblCompList.Items.Count - 1
                cblCompList.Items(intCntCompAll).Selected = True
            Next
        Else
            For intCntCompAll = 0 To cblCompList.Items.Count - 1
                cblCompList.Items(intCntCompAll).Selected = False
            Next
        End If
        CompCheck()
    End Sub

    Sub CompCheckList(ByVal Sender As Object, ByVal E As EventArgs)
        CompCheck()
    End Sub
    ...
    Sub CompCheck()
        Dim intCnt2 As Integer = 0
        Dim tempUserComp As String
        Dim txt As HtmlInputHidden
        Dim strOpCd As String
        Dim strParamName As String
        Dim strParamValue As String

        strOpCd = "REPORT_CLSCOMP_COMPANY_LOCATION_LIST_UPD"
        strParamName = "STATUS|COMPCODE"

        For intCnt2 = 0 To cblCompList.Items.Count - 1
            If cblCompList.Items(intCnt2).Selected Then
                strParamValue = "1" & "|" & Trim(cblCompList.Items(intCnt2).Text)
            Else
                strParamValue = "0" & "|" & Trim(cblCompList.Items(intCnt2).Text)
            End If
            ...
            Catch Exp As System.Exception
                Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=UPD_PICKED_COMP&errmesg=" & Exp.ToString() & "&redirect=")
            End Try
        Next
        'hidCompList.Value = tempUserComp
    End Sub
End Class
