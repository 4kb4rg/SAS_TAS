...
Public Class CT_StdRpt_StkReceiveList_Preview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCdStkRcv As String = "CT_STDRPT_STKRECEIVE_AND_STKRECEIVELN_GET"
        ...
        Dim WildStr As String = "AND STKRCV.CanteenRcvID *= STKRCVLn.CanteenRcvID AND "
        Dim NormStr As String = "AND STKRCV.CanteenRcvID = STKRCVLn.CanteenRcvID AND "

        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = "(DateDiff(Day, '" & Session("SS_DATEFROM") & "', STKRCV.CreateDate) >= 0) And (DateDiff(Day, '" & Session("SS_DATETO") & "', STKRCV.CreateDate) <= 0) AND "
        End If

        If Not (Request.QueryString("StkRcvIDFrom") = "" And Request.QueryString("StkRcvIDTo") = "") Then
            SearchStr = SearchStr & "STKRCV.CanteenRcvID IN (SELECT SUBSTK.CanteenRcvID FROM CT_CANTEENRCV SUBSTK WHERE SUBSTK.CanteenRcvID >= '" & Request.QueryString("StkRcvIDFrom") & _
                        "' AND SUBSTK.CanteenRcvID <= '" & Request.QueryString("StkRcvIDTo") & "') AND "
        End If

        If Not Request.QueryString("StkRefNo") = "" Then
            SearchStr = SearchStr & "STKRCV.CanteenRefNo LIKE '" & Request.QueryString("StkRefNo") & "' AND "
        Else
            SearchStr = SearchStr & "STKRCV.CanteenRefNo LIKE '%' AND "
        End If

        If Not (Request.QueryString("PRIDFrom") = "" And Request.QueryString("PRIDTo") = "") Then
            SearchStr = SearchStr & "STKRCVLn.DocID IN (SELECT SUBSTK.DocID FROM CT_CANTEENRCVLN SUBSTK WHERE SUBSTK.DocID >= '" & Request.QueryString("PRIDFrom") & _
                        "' AND SUBSTK.DocID <= '" & Request.QueryString("PRIDTo") & "') AND "
        End If

        If Not Request.QueryString("StkRcvType") = "" Then
            SearchStr = SearchStr & "STKRCV.CanteenDocType = '" & Request.QueryString("StkRcvType") & "' AND "
        Else
            SearchStr = SearchStr & "STKRCV.CanteenDocType LIKE '%' AND "
        End If
        ...
        If Not SearchStr = "" Or Not itemSelectStr = "" Then
            ...
            If Not Request.QueryString("ItemCode") = "" Or Not Request.QueryString("ProdType") = "" Or Not Request.QueryString("ProdBrand") = "" Or Not _
                ...
                strParam = strUserLoc & "|" & strDDLAccMth & "|" & strDDLAccYr & "|" & itemSelectStr & "AND ITM.ItemType = '" & objCTSetup.EnumInventoryItemType.CanteenItem & "'|" & itemSearchStr & "|" & NormStr & SearchStr
            Else
                strParam = strUserLoc & "|" & strDDLAccMth & "|" & strDDLAccYr & "|" & itemSelectStr & "AND ITM.ItemType = '" & objCTSetup.EnumInventoryItemType.CanteenItem & "'|" & itemSearchStr & "|" & WildStr & SearchStr
            End If

        End If

        Try
            intErrNo = objCT.mtdGetReport_StockReceiveList(strOpCdStkRcv, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_CTRECEIVE_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        ...
        For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
            If Not IsDBNull(objRptDs.Tables(0).Rows(intCnt).Item("ItemCode")) Then
                ...
                strParamItm = "WHERE ITM.ItemCode = '" & strItemCode & "' AND ITM.ItemType = '" & objCTSetup.EnumInventoryItemType.CanteenItem & "'"
                Try
                    intErrNo = objCT.mtdGetItem(strOpCdItem_GET, _
                                                ...
                                                objItem)
                Catch Exp As System.Exception
                    Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_CTRECEIVE_ITEM_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
                End Try
        ...
        If Not Request.QueryString("ItemCode") = "" Or Not Request.QueryString("ProdType") = "" Or Not Request.QueryString("ProdBrand") = "" Or Not _
            ...
            If objRptDs.Tables(0).Rows.Count > 0 Then
                Do
                    If IsDBNull(objRptDs.Tables(0).Rows(intCntRem).Item("CanteenRcvLnID")) Then
        ...
        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\CT_StdRpt_StkReceiveList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\CT_StdRpt_StkReceiveList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/CT_StdRpt_StkReceiveList.pdf"">")
    End Sub

    Sub PassParam()
        Dim strStkRcvTypeText As String
        ...
        If Trim(Session("SS_StkRcvType")) = "" Then
            strStkRcvTypeText = "All"
        Else
            strStkRcvTypeText = objCTTrx.mtdGetCanteenReceiveDocType(CInt(Trim(Session("SS_StkRcvType"))))
        End If
        strStatusText = objCTTrx.mtdGetCanteenReceiveStatus(CInt(Trim(Session("SS_Status"))))
        ...
        ParamDiscreteValue4.Value = Session("SS_StkRcvIDFrom")
        ParamDiscreteValue5.Value = Session("SS_StkRcvIDTo")
        ParamDiscreteValue6.Value = strStkRcvTypeText
