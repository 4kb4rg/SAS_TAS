...
Public Class CB_StdRpt_DailyGiroPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCode As String = "CB_STDRPT_DAILYGIRO_HANDOVER_GET"
        Dim strFileName As String = "CB_StdRpt_DailyGiro_Handover_Report"

        Dim strParam As String
        Dim searchStr As String
        Dim objFTPFolder As String

        strParamName = "TRXDATE|LOCCODE|STRSEARCH"
        If strAccCode = "" Then
            'strParamValue = strDate & "|" & strLocation & "|" & " "
			strParamValue = strDate & "|" & strLocation & "|" & " AND A.BankAccNo IN (SELECT BankAccNo FROM SH_LOCATION_BANK WHERE LocCode = '" & strlocation & "') "
        Else
            strParamValue = strDate & "|" & strLocation & "|" & " AND A.BankAccNo IN (SELECT BankAccNo FROM SH_LOCATION_BANK WHERE LocCode = '" & strlocation & "' AND AccCode = '" & strAccCode & "') "
        End If
