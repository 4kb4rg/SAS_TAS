...
Public Class BD_StdRpt_FFBProductionSumPreview : inherits page
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd2 As String = "BD_STDRPT_FFB_PRODUCTION_BUDGET_SUMMARY"
        ...
        strParam =  strLocation & "|" & strYear   
        Try
            intErrNo = objBD.mtdGetReport_FFBProductionBudgetSummary(strOpCd2, strParam, objRptDs4, objMapPath)
        ...
        objRptDs.Tables(0).TableName = "BD_StdRpt_FFBProductionSumInt"
        objRptDs.Tables.Add(objRptDs2.Tables(0).Copy())
        objRptDs.Tables(1).TableName = "BD_StdRpt_FFBProductionSumAss"
        objRptDs.Tables.Add(objRptDs3.Tables(0).Copy())
        objRptDs.Tables(2).TableName = "BD_StdRpt_FFBProductionSumExt"
        objRptDs.Tables.Add(objRptDs4.Tables(0).Copy())
        objRptDs.Tables(3).TableName = "BD_StdRpt_FFBProductionSumExtRate"
        
        For I = 0 To 2
            If objRptDs.Tables(I).Rows.Count = 0 Then
                newRow = objRptDs.Tables(I).NewRow()
                newRow.Item(0) = ""
                newRow.Item(1) = ""
                For K = 2 To 13
                    newRow.Item(K) = 0.0
                Next
                objRptDs.Tables(I).Rows.Add(newRow)
            Else
            ...
            End If
        Next
        For J = 0 To objRptDs.Tables(3).Rows.Count - 1
            For K = 1 To 24
                If IsDBNull(objRptDs.Tables(3).Rows(J).Item(K)) Then
                    objRptDs.Tables(3).Rows(J).Item(K) = 0.0
                End If
            Next
        Next


        rdCrystalViewer.Load(objMapPath & "Web\" & strLangCode & "\Reports\Crystal\BD_StdRpt_FFBProductionSum.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\BD_StdRpt_FFBProductionSum.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/BD_StdRpt_FFBProductionSum.pdf"">")
        End If
