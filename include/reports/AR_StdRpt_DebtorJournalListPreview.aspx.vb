...
Public Class AR_StdRpt_DebtorJournalList_Preview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strFileName = "AR_StdRpt_DebtorJournalListing"
        ...
        Session("SS_DJIDFrom") = Request.QueryString("DJIDFrom")
        Session("SS_DJIDTo") = Request.QueryString("DJIDTo")
        Session("SS_lblBillParty") = Request.QueryString("lblBillParty")
        Session("SS_BillParty") = Request.QueryString("BillParty")
        Session("SS_JournalType") = objARTrx.mtdGetDebtorJournalType(Trim(Request.QueryString("JournalType")))
        Session("SS_Status") = objARTrx.mtdGetDebtorJournalStatus(Trim(Request.QueryString("Status")))
        Session("SS_lblCOA") = Request.QueryString("lblCOA")
        Session("SS_lblBlkCode") = Request.QueryString("lblBlk")
    ...
    Sub BindReport()
        ...
        Dim strOpCd_DebtorJournalList_GET As String = "AR_STDRPT_DEBTORJOURNAL_LIST_GET"
        Dim SearchStr As String
        Dim SQLStr As String

        Dim intCnt As Integer
        SearchStr = ""
        If Not (Request.QueryString("DJIDFrom") = "" AND Request.QueryString("DJIDTo") = "") Then 
            SearchStr = SearchStr & " AND DJ.DebtorJrnID >='" & Replace(Request.QueryString("DJIDFrom"), "'", "''") & "' AND DJ.DebtorJrnID <='" & Replace(Request.QueryString("DJIDTo"), "'", "''") & "' "
        End If
        If Not Request.QueryString("BillParty") = "" Then 
            SearchStr = SearchStr & " AND DJ.BillPartyCode LIKE '" & Replace(Request.QueryString("BillParty"), "'", "''") & "' "
        End If
        If Not Request.QueryString("JournalType") = "0" Then 
            SearchStr = SearchStr & " AND DJ.JrnType LIKE '" & Request.QueryString("JournalType") & "' "
        End If
        If Not Request.QueryString("Status") = "0" Then 
            SearchStr = SearchStr & " AND DJ.Status LIKE '" & Request.QueryString("Status") & "' "
        End If


        strParam =  strUserLoc & "|" & strDDLAccMth & "|" & strDDLAccYr & "|" & SearchStr & "|" & _
                    objGlobal.EnumDocType.BICreditNote & "|" & objGlobal.EnumDocType.BIDebitNote & "|" & objGlobal.EnumDocType.ContractInvoice
        Try
            intErrNo = objAR.mtdGetReport_DebtorJournalList(strOpCd_DebtorJournalList_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=AR_STDRPT_InvoiceList&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("JrnType") = objArTrx.mtdGetDebtorJournalType(Trim(objRptDs.Tables(0).Rows(intCnt).Item("JrnType")))
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objArTrx.mtdGetDebtorJournalStatus(Trim(objRptDs.Tables(0).Rows(intCnt).Item("Status")))
            If IsDBNull(objRptDs.Tables(0).Rows(intCnt).Item("DocType")) = False Then
                If Trim(objRptDs.Tables(0).Rows(intCnt).Item("DocType")) <> "" Then
...
Sub PassParam()
        ...
        paramField9 = paramFields.Item("ParamDJIDFrom")
        paramField10 = paramFields.Item("ParamDJIDTo")
        paramField11 = paramFields.Item("ParamBillParty")
        paramField12 = paramFields.Item("ParamJournalType")
        paramField13 = paramFields.Item("ParamStatus")
        paramField14 = paramFields.Item("lblBillParty")
        paramField15 = paramFields.Item("lblCOACode")
        paramField16 = paramFields.Item("lblVehicle")
        paramField17 = paramFields.Item("lblVehExpCode")
        paramField18 = paramFields.Item("lblBlkCode")
        ...
        ParamDiscreteValue9.Value = Session("SS_DJIDFrom")
        ParamDiscreteValue10.Value = Session("SS_DJIDTo")
        ParamDiscreteValue11.Value = Session("SS_BillParty")
        ParamDiscreteValue12.Value = Session("SS_JournalType")
        ParamDiscreteValue13.Value = Session("SS_Status")
        ParamDiscreteValue14.Value = Session("SS_lblBillParty")
        ParamDiscreteValue15.Value = Session("SS_lblCOA")
        ParamDiscreteValue16.Value = Session("SS_lblVehicle")
        ParamDiscreteValue17.Value = Session("SS_lblVehicleExpCode")
        ParamDiscreteValue18.Value = Session("SS_lblBlkCode")
        ParamDiscreteValue19.Value = Session("SS_lblLocation")
