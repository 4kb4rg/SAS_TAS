...
Public Class PU_StdRpt_PORealizationPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR_GET As String = "PU_STDRPT_POREALIZATION"

        Dim WildStr As String = "PO.POID *= POLN.POID "
        Dim NormStr As String = "PO.POID = POLN.POID "

        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = "AND (DateDiff(Day, '" & Request.QueryString("DateFrom") & "', PO.CreateDate) >= 0) And (DateDiff(Day, '" & Request.QueryString("DateTo") & "', PO.CreateDate) <= 0) "
        End If
		
		 ' If Not (Request.QueryString("PRNoFrom") = "" And Request.QueryString("PRNoTo") = "") Then
            ' SearchStr = SearchStr & "AND POLN.PRID IN (SELECT SUBPR.PRID FROM PU_POLN SUBPR WHERE SUBPR.PRID >= '" & Request.QueryString("PRNoFrom") & _
                        ' "' AND SUBPR.PRID <= '" & Request.QueryString("PRNoTo") & "') "
        ' End If
		
        If Not (Request.QueryString("PRNoFrom") = "" And Request.QueryString("PRNoTo") = "") Then
            SearchStr = SearchStr & "AND PO.POID >= '" & Request.QueryString("PRNoFrom") & _
                        "' AND PO.POID <= '" & Request.QueryString("PRNoTo") & "' "
        End If

        If Request.QueryString("PRType") = "All" Then
            SearchStr = SearchStr & "AND PO.POType IN ('" & objPUTrx.EnumPOType.Stock & "','" & objPUTrx.EnumPOType.DirectCharge & "','" & objPUTrx.EnumPOType.Workshop & "','" & objPUTrx.EnumPOType.FixedAsset & "','" & objPUTrx.EnumPOType.Nursery & "') "
        ElseIf Request.QueryString("PRType") = objPUTrx.EnumPOType.Stock Then
            SearchStr = SearchStr & "AND PO.POType = '" & objPUTrx.EnumPOType.Stock & "' "
        ElseIf Request.QueryString("PRType") = objPUTrx.EnumPOType.DirectCharge Then
            SearchStr = SearchStr & "AND PO.POType = '" & objPUTrx.EnumPOType.DirectCharge & "' "
        ElseIf Request.QueryString("PRType") = objPUTrx.EnumPOType.Workshop Then
            SearchStr = SearchStr & "AND PO.POType = '" & objPUTrx.EnumPOType.Workshop & "' "
        ElseIf Request.QueryString("PRType") = objPUTrx.EnumPOType.FixedAsset Then
            SearchStr = SearchStr & "AND PO.POType = '" & objPUTrx.EnumPOType.FixedAsset & "' "
        ElseIf Request.QueryString("PRType") = objPUTrx.EnumPOType.Nursery Then
            SearchStr = SearchStr & "AND PO.POType = '" & objPUTrx.EnumPOType.Nursery & "' "
        End If

        If Request.QueryString("PRStatus") = "All" Then
            SearchStr = SearchStr & "AND PO.Status LIKE '%' "
        ElseIf Request.QueryString("PRStatus") = "Closed" Then
            SearchStr = SearchStr & "AND PO.Status = '" & objPUTrx.EnumPOStatus.Closed & "' "
        ElseIf Request.QueryString("PRStatus") = "Outstanding" Then
            SearchStr = SearchStr & "AND PO.Status = '" & objPUTrx.EnumPOStatus.Confirmed & "' "
        End If
        ...
        If Not Request.QueryString("ItemCode") = "" Then
            SearchStr = SearchStr & "AND POLN.ItemCode LIKE '" & Request.QueryString("ItemCode") & "' "
        End If

        If Not Request.QueryString("RefLocCode") = "" Then
            SearchStr = SearchStr & "AND SUBSTRING(POLN.PRID,PATINDEX('%/%',POLN.PRID)+5,3) = '" & Request.QueryString("RefLocCode") & "' "
        End If
        ...
        strParam = strUserLoc & "|" & itmSearchStr & "|" & SearchStr

        Try
            intErrNo = objIN.mtdGetReport_PRRealization(strOpCdPR_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_PU_PO_REALIZATION_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\PU_StdRpt_PRRealization.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        If strExportToExcel = "0" Then
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\PU_StdRpt_PORealization.pdf"
        Else
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\PU_StdRpt_PORealization.xls"
        End If
        ...
        If strExportToExcel = "0" Then
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/PU_StdRpt_PORealization.pdf"">")
        Else
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/PU_StdRpt_PORealization.xls"">")
        End If
    End Sub
    ...
    Sub PassParam()
        ...
        paramField11 = paramFields.Item("ParamRptID")
        paramField12 = paramFields.Item("ParamRptName")
        paramField13 = paramFields.Item("ParamPRType")
        ...
        paramField26 = paramFields.Item("ParamPRStatus")
        ...
        ParamDiscreteValue11.Value = Request.QueryString("RptID")
        ParamDiscreteValue12.Value = Request.QueryString("RptName")
        ParamDiscreteValue13.Value = Request.QueryString("PRType")
        ...
        ParamDiscreteValue26.Value = Request.QueryString("PRStatus")
