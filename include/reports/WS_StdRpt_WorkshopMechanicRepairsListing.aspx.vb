...
Public Class WS_StdRpt_WorkshopMechanicRepairsListing : Inherits Page
    Protected RptSelect As UserControl
    
    Protected WithEvents lblLocation As Label
    Protected WithEvents lblWork As Label
    Protected WithEvents lblBillParty As Label
    Protected WithEvents lblWorkingDateFromErr As Label
    Protected WithEvents lblWorkingDateToErr As Label
    
    Protected WithEvents txtMechanicEmpCode As TextBox
    Protected WithEvents txtWorkingDateFrom As TextBox
    Protected WithEvents txtWorkingDateTo As TextBox
    ...
    Protected WithEvents imgWorkingDateFrom As Image
    Protected WithEvents imgWorkingDateTo As Image
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            lblWorkingDateFromErr.Visible = False
            lblWorkingDateToErr.Visible = False
    ...
    Function GetLanguageCaptionDS() As DataSet
        ...
        Catch ex As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=WS_STDRPT_WORKSHOP_MECHANIC_REPAIRS_LISTING_GET_LANGCAP&errmesg=" & Server.UrlEncode(ex.Message) & "&redirect=WS/trx/WS_Trx_Job_List.aspx")
        End Try
    ...
    Sub ibPrintPreview_OnClick(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Dim strMechanicEmpCode As String
        Dim strWorkingDateFrom As String
        Dim strWorkingDateTo As String
        ...
        strMechanicEmpCode = Server.UrlEncode(Trim(txtMechanicEmpCode.Text))
        strWorkingDateFrom = Trim(txtWorkingDateFrom.Text)
        strWorkingDateTo = Trim(txtWorkingDateTo.Text)
        ...
        If strWorkingDateFrom <> "" Then
            strWorkingDateFrom = GetValidDate(strWorkingDateFrom, strErrMsg)
            If strWorkingDateFrom = "" Then
                lblWorkingDateFromErr.Text = strErrMsg
                lblWorkingDateFromErr.Visible = True
                blnErrMsg = True
            End If
        End If
        If strWorkingDateTo <> "" Then
            strWorkingDateTo = GetValidDate(strWorkingDateTo, strErrMsg)
            If strWorkingDateTo = "" Then
                lblWorkingDateToErr.Text = strErrMsg
                lblWorkingDateToErr.Visible = True
                blnErrMsg = True
            End If
        End If
        ...
        Response.Write("<Script Language=""JavaScript"">window.open(""WS_StdRpt_WorkshopMechanicRepairsListingPreview.aspx?CompName=" & strCompany & "&RptID=" & strRptID & "&RptName=" & strRptName & "&RptLocation=" & strRptLocation & "&RptAccMonth=" & strRptAccMonth & "&RptAccYear=" & strRptAccYear & "&Decimal=" & strDecimal & _
                        "&lblLocation=" & lblLocation.Text & "&lblWork=" & lblWork.Text & "&lblBillParty=" & lblBillParty.Text & _
                        "&WorkingDateFrom=" & strWorkingDateFrom & "&WorkingDateTo=" & strWorkingDateTo & _
                        "&JobType=" & strJobType & _
                        "&BillPartyCode=" & strBillPartyCode & "&EmpCode=" & strEmpCode & _
                        "&MechanicEmpCode=" & strMechanicEmpCode & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
    End Sub
