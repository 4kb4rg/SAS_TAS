...
Public Class CT_StdRpt_StkRetAdvList_Preview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim WildStr As String = "AND IRA.ItemRetAdvID *= IRALn.ItemRetAdvID AND "
        Dim NormStr As String = "AND IRA.ItemRetAdvID = IRALn.ItemRetAdvID AND "
        ...
        If Request.QueryString("ProdType") = "" And Request.QueryString("ProdBrand") = "" And Request.QueryString("ProdModel") = "" And Request.QueryString("ProdCat") = "" And Request.QueryString("ProdMat") = "" And Request.QueryString("StkAna") = "" And Request.QueryString("ItemCode") = "" Then
            itemSearchStr = "OR ( IRALN.ItemCode = '' )"
        End If
        ...
        If Not SearchStr = "" Or Not itemSelectStr = "" Then
            ...
            If Not Request.QueryString("ItemCode") = "" Or Not Request.QueryString("ProdType") = "" Or Not Request.QueryString("ProdBrand") = "" Or Not _
                ...
                strParam = strUserLoc & "|" & strDDLAccMth & "|" & strDDLAccYr & "|" & objGlobal.EnumDocType.CanteenReturnAdvice & "|" & itemSelectStr & "AND ITM.ItemType = '" & objCTSetup.EnumInventoryItemType.CanteenItem & "'|" & itemSearchStr & "|" & NormStr & SearchStr
            Else
                strParam = strUserLoc & "|" & strDDLAccMth & "|" & strDDLAccYr & "|" & objGlobal.EnumDocType.CanteenReturnAdvice & "|" & itemSelectStr & "AND ITM.ItemType = '" & objCTSetup.EnumInventoryItemType.CanteenItem & "'|" & itemSearchStr & "|" & WildStr & SearchStr
            End If

        End If

        Try
            intErrNo = objCT.mtdGetReport_StkRetAdvList(strOpCdItemRetAdv_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_CT_STKRETADV_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
            ...
            If Not IsDBNull(objRptDs.Tables(0).Rows(intCnt).Item("ItemCode")) Then
                ...
                Try
                    intErrNo = objCT.mtdGetItem(strOpCdItem_GET, _
                                                ...
                                                objItem)
                Catch Exp As System.Exception
                    Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_ITEM_FOR_CT_STKRETADV_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
                End Try

                If objItem.Tables(0).Rows.Count > 0 Then
                    objRptDs.Tables(0).Rows(intCnt).Item("Item") = Trim(objRptDs.Tables(0).Rows(intCnt).Item("ItemCode")) & " (" & _
        ...
        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\CT_StdRpt_StkRetAdvList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\CT_StdRpt_StkRetAdvList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/CT_StdRpt_StkRetAdvList.pdf"">")
    End Sub
