...
Public Class PU_StdRpt_OutstandingPO : Inherits Page
    ...
    Protected WithEvents lstGR As DropDownList
    Protected WithEvents lstInvoice As DropDownList
    Protected WithEvents lstPayment As DropDownList
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        If Not (strDocDateFrom = "" And strDocDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDocDateFrom, objDateFormat, objDocDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDocDateTo, objDateFormat, objDocDateTo) = True Then
                Response.Write("<Script Language=""JavaScript"">window.open(""PU_StdRpt_OutstandingPOPreview.aspx?CompName=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                               "&DDLAccMth=" & strddlAccMth & "&DDLAccYr=" & strddlAccYr & _
                               "&lblLocation=" & lblLocation.Text & "&Supplier=" & strSupplier & _
                               "&POID=" & strPOID & "&DocDateFrom=" & objDocDateFrom & "&DocDateTo=" & objDocDateTo & "&ProdType=" & strProdType & "&ProdBrand=" & strProdBrand & _
                               ...
                               "&LnStatus=" & strLnStatus & "&lstGR=" & Trim(lstGR.SelectedItem.Value) & "&lstInvoice=" & Trim(lstInvoice.SelectedItem.Value) & "&lstPayment=" & Trim(lstPayment.SelectedItem.Value) & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
            Else
