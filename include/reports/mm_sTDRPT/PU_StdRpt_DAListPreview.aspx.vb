...
Public Class PU_StdRpt_DAList_Preview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        response.write(Session("SS_DOCDATEFROM"))
    ...
    Sub BindReport()
        ...
        Dim strOpCd_DA_BlkGrp_GET As String = "PU_STDRPT_DA_AND_DALN_BLKGRP_GET"
        Dim strOpCd_DA_Blk_GET As String = "PU_STDRPT_DA_AND_DALN_BLK_GET"
        Dim strOpCd_DA_SubBlk_GET As String = "PU_STDRPT_DA_AND_DALN_SUBBLK_GET"
        Dim strOpCdItem_GET As String = "PU_STDRPT_ITEM_GET"
        Dim strOpCd_DA_POID_GET As String = "PU_STDRPT_DA_POID_GET"
        ...
        'WildStr = " FROM PU_DISPADV DA left outer join PU_DISPADVLN DALN on DA.DispAdvID = DALN.DispAdvID "
        'WildStr = WildStr & " inner join SH_USER USR on USR.UserID = DA.UpdateID "
        'WildStr = WildStr & " inner join pu_goodsrcvln gr on gr.goodsrcvlnid = daln.goodsrcvlnid "
        'WildStr = WildStr & " inner join pu_poln poln on poln.polnid = gr.polnid and poln.itemcode = gr.itemcode "
        'WildStr = WildStr & " left join in_pr pr on pr.prid = poln.prid "

        'NormStr = " FROM PU_DISPADV DA inner join PU_DISPADVLN DALN on DA.DispAdvID = DALN.DispAdvID "
        'NormStr = NormStr & " inner join SH_USER USR on USR.UserID = DA.UpdateID "
        'NormStr = NormStr & " inner join pu_goodsrcvln gr on gr.goodsrcvlnid = daln.goodsrcvlnid "
        'NormStr = NormStr & " inner join pu_poln poln on poln.polnid = gr.polnid and poln.itemcode = gr.itemcode "
        ...
        If strBlkType = "BlkGrp" Then
            ...
            strOpCd_DA_GET = strOpCd_DA_BlkGrp_GET
        ElseIf strBlkType = "BlkCode" Then
            If Not Request.QueryString("BlkCode") = "" Then
                blkSelectStr = "DALN.BlkCode LIKE '" & Request.QueryString("BlkCode") & "' "
            Else
                blkSelectStr = "DALN.BlkCode LIKE '%' "
                blkSearchStr = "OR ( DALN.BlkCode = '' )"
            End If
            Session("SS_BLKHEADER") = "BlkCode"
            strOpCd_DA_GET = strOpCd_DA_Blk_GET

        ElseIf strBlkType = "SubBlkCode" Then
            If Not Request.QueryString("SubBlkCode") = "" Then
                blkSelectStr = "DALN.BlkCode LIKE '" & Request.QueryString("SubBlkCode") & "' "
            Else
                blkSelectStr = "DALN.BlkCode LIKE '%' "
                blkSearchStr = "OR ( DALN.BlkCode = '' )"
            End If
            Session("SS_SUBBLKHEADER") = "SubBlkCode"
            strOpCd_DA_GET = strOpCd_DA_SubBlk_GET
        End If

        If Not (Request.QueryString("DocDateFrom") = "" And Request.QueryString("DocDateTo") = "") Then
            SearchStr = "AND (DateDiff(Day, '" & Request.QueryString("DocDateFrom") & "', DA.CreateDate) >= 0) And (DateDiff(Day, '" & Request.QueryString("DocDateTo") & "', DA.CreateDate) <= 0) "
        End If


        If Not (Request.QueryString("DocNoFrom") = "" And Request.QueryString("DocNoTo") = "") Then
            SearchStr = SearchStr & "AND DA.DispAdvID IN (SELECT SUBDA.DispAdvID FROM PU_DISPADV SUBDA WHERE SUBDA.DispAdvID >= '" & Request.QueryString("DocNoFrom") & _
                        "' AND SUBDA.DispAdvID <= '" & Request.QueryString("DocNoTo") & "') "
        End If

        If Not Request.QueryString("Supplier") = "" Then
            'SearchStr = SearchStr & "AND DA.SupplierCode LIKE '" & Request.QueryString("Supplier") & "' "
            SearchStr = SearchStr & "AND (DA.SupplierCode LIKE '%" & Request.QueryString("Supplier") & "%' OR SU.Name LIKE '%" & Request.QueryString("Supplier") & "%')  "
        Else
            SearchStr = SearchStr & "AND DA.SupplierCode LIKE '%' "
        End If
        ...
        If Request.QueryString("ProdType") = "" And Request.QueryString("ProdBrand") = "" And Request.QueryString("ProdModel") = "" And Request.QueryString("ProdCat") = "" And Request.QueryString("ProdMat") = "" And Request.QueryString("StkAna") = "" And Request.QueryString("ItemCode") = "" Then
            itemSearchStr = "OR ( DALN.ItemCode = '' )"
        End If

        If Not Request.QueryString("AccCode") = "" Then
            SearchStr = SearchStr & "AND DALN.AccCode LIKE '" & Request.QueryString("AccCode") & "' "
        Else
            SearchStr = SearchStr & "AND DALN.AccCode LIKE '%' "
        End If

        If Not Request.QueryString("VehCode") = "" Then
            vehSelectStr = "VEH.VehCode LIKE '" & Request.QueryString("VehCode") & "' "
        Else
            vehSelectStr = "VEH.VehCode LIKE '%' "
        End If
        ...
        Else
            vehSelectStr = vehSelectStr & "AND VEH.VehTypeCode LIKE '%' "
        End If

        If Request.QueryString("VehCode") = "" And Request.QueryString("VehTypeCode") = "" Then
            vehSearchStr = "OR ( DALN.VehCode = '' )"
        End If

        If Not Request.QueryString("VehExpCode") = "" Then
            SearchStr = SearchStr & "AND DALN.VehExpenseCode LIKE '" & Request.QueryString("VehExpCode") & "' "
        Else
            SearchStr = SearchStr & "AND DALN.VehExpenseCode LIKE '%' "
        End If

        If Not Request.QueryString("DAType") = "" Then
            If Not Request.QueryString("DAType") = objPUTrx.EnumDAType.All Then
                SearchStr = SearchStr & "AND DA.DispAdvType = '" & Request.QueryString("DAType") & "' "
            Else
                SearchStr = SearchStr & "AND DA.DispAdvType LIKE '%' "
            End If
        End If

        If Not Request.QueryString("Transporter") = "" Then
            SearchStr = SearchStr & "AND DA.Transporter LIKE '%" & Request.QueryString("Remark") & "%' "
        Else
            SearchStr = SearchStr & "AND DA.Transporter LIKE '%' "
        End If

        If Not Request.QueryString("Status") = "" Then
            If Not Request.QueryString("Status") = objPUTrx.EnumPOStatus.All Then
                SearchStr = SearchStr & "AND DA.Status = '" & Request.QueryString("Status") & "' "
            Else
                SearchStr = SearchStr & "AND DA.Status LIKE '%' "
            End If
        End If
        ...
        '        strUserLoc1 = " WHERE DA.LocCode IN ('" & strUserLoc & "') "   
        '    end if
        ...
        Try
            intErrNo = objPU.mtdGetReport_DAList(strOpCd_DA_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PU_STDRPT_DA_LIST&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        ...
        For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
            If Not IsDBNull(objRptDs.Tables(0).Rows(intCnt).Item("ItemCode")) Then
                ...
                strGoodsRcvID = Trim(objRptDs.Tables(0).Rows(intCnt).Item("GoodsRcvID"))
                ...
                Try
                    intErrNo = objPU.mtdGetItem(strOpCdItem_GET, _
                                                ...
                                                objItem)
                Catch Exp As System.Exception
                    Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_ITEM_FOR_DALIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
                End Try
            ...
            strParam = "|||||||||" & "WHERE GoodsRcvID = '" & strGoodsRcvID & "'" & "|"
            Try
                intErrNo = objPU.mtdGetReport_DAList(strOpCd_DA_POID_GET, strParam, objPOID, objMapPath)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PU_STDRPT_DA_POID_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
            End Try

            If objPOID.Tables(0).Rows.Count > 0 Then
                objRptDs.Tables(0).Rows(intCnt).Item("POID") = Trim(objPOID.Tables(0).Rows(0).Item("POID"))
            End If
        ...
        If Not Request.QueryString("AccCode") = "" Or Not Request.QueryString("VehTypeCode") = "" Or Not Request.QueryString("VehCode") = "" Or Not Request.QueryString("VehExpCode") = "" Or Not Request.QueryString("BlkGrp") = "" Or Not _
            ...
            If objRptDs.Tables(0).Rows.Count > 0 Then
                Do
                    If IsDBNull(objRptDs.Tables(0).Rows(intCntRem).Item("DispAdvLnID")) Then
        ...
        Select Case strGroupBy
            Case "1"
                strRptPrefix = "PU_StdRpt_DAList"
            Case "2"
                strRptPrefix = "PU_StdRpt_DAListByItem"
            Case "3"
                strRptPrefix = "PU_StdRpt_DAListByToLocCode"
            Case "4"
                strRptPrefix = "PU_StdRpt_DAListByTransporter"
        End Select
    ...
    Sub PassParam()
        ...
        paramField21 = paramFields.Item("ParamDAType")
        ...
        ParamDiscreteValue21.Value = Request.QueryString("DAType")
