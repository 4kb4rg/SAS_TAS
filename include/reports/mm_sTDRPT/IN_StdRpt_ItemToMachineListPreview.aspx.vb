...
Public Class IN_StdRpt_ItemToMachineListPreview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Session("SS_LBLLOCATION") = Request.QueryString("Location")
        ...
        Session("SS_InstallDateFrom") = Request.QueryString("InstallDateFrom")
        Session("SS_InstallDateTo") = Request.QueryString("InstallDateTo")
    ...
    Sub BindReport()
        ...
        Dim strOpCdItem_GET As String = "IN_STDRPT_ITEMTOMACHINE_GET"
        ...
        If Not Request.QueryString("BlkCode") = "" Then
            SearchStr = SearchStr & "AND ITML.BlkCode LIKE '" & Request.QueryString("BlkCode") & "' AND "
        Else
            SearchStr = SearchStr & "AND ITML.BlkCode LIKE '%' AND "
        End If

        If Not Request.QueryString("SubBlkCode") = "" Then
            SearchStr = SearchStr & "ITML.SubBlkCode LIKE '" & Request.QueryString("SubBlkCode") & "' AND "
        Else
            SearchStr = SearchStr & "ITML.SubBlkCode LIKE '%' AND "
        End If

        If Not Request.QueryString("ItemCode") = "" Then
            SearchStr = SearchStr & "ITML.ItemCode LIKE '" & Request.QueryString("ItemCode") & "' AND "
        Else
            SearchStr = SearchStr & "ITML.ItemCode LIKE '%' AND "
        End If

        If Not Request.QueryString("InstallDate") = "" Then
            SearchStr = SearchStr & "(DateDiff(Day, '" & Request.QueryString("InstallDate") & "', ITML.InstallDate) = 0) AND "
        End If

        If Not Request.QueryString("ReplDate") = "" Then
            SearchStr = SearchStr & "(DateDiff(Day, '" & Request.QueryString("ReplDate") & "', ITML.ReplaceDate) = 0) AND "
        End If
        ...
        If Not SearchStr = "" Then
            ...
            strParam = strUserLoc & "|" & objINSetup.EnumStockItemStatus.Active & "|" & _
                       SearchStr 
        End If

        Try
            intErrNo = objIN.mtdGetReport_ItemToMachineList(strOpCdItem_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_ITEMTOMACHINE_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\IN_StdRpt_ItemToMachineList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        If strExportToExcel = "0" Then
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\IN_StdRpt_ItemToMachineList.pdf"
        Else
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\IN_StdRpt_ItemToMachineList.xls"
        End If
        ...
        If strExportToExcel = "0" Then
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/IN_StdRpt_ItemToMachineList.pdf"">")
        Else
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/IN_StdRpt_ItemToMachineList.xls"">")
        End If
    End Sub
    ...
    Sub PassParam()
        ...
        paramField9 = paramFields.Item("ParamInstallDateFrom")
        paramField10 = paramFields.Item("ParamInstallDateTo")
        paramField11 = paramFields.Item("ParamReplDateFrom")
        paramField12 = paramFields.Item("ParamReplDateTo")
        paramField13 = paramFields.Item("lblLocation")
        paramField14 = paramFields.Item("lblBlkTag")
        paramField15 = paramFields.Item("lblSubBlkTag")
        ...
        ParamDiscreteValue9.Value = Session("SS_INSTALLDATEFROM")
        ParamDiscreteValue10.Value = Session("SS_INSTALLDATETO")
        ParamDiscreteValue11.Value = Session("SS_REPLDATEFROM")
        ParamDiscreteValue12.Value = Session("SS_REPLDATETO")
        ParamDiscreteValue13.Value = Session("SS_LBLLOCATION")
        ParamDiscreteValue14.Value = Session("SS_LBLBLKTAG")
        ParamDiscreteValue15.Value = Session("SS_LBLSUBBLKTAG")
