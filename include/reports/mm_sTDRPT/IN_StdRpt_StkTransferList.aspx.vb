...
Public Class IN_StdRpt_StkTransferList : Inherits Page
    ...
    Protected WithEvents lblToLocation As Label
    ...
    Protected WithEvents txtStkTransferIdFrom As TextBox
    Protected WithEvents txtStkTransferIdTo As TextBox    
    Protected WithEvents txtToLocation As TextBox    
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblToLocation.Text = GetCaption(objLangCap.EnumLangCap.Location) & " Code :"
    ...
    Sub BindStatus()
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockTransferStatus(objINTrx.EnumStockTransferStatus.All), objINTrx.EnumStockTransferStatus.All))
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockTransferStatus(objINTrx.EnumStockTransferStatus.Active), objINTrx.EnumStockTransferStatus.Active))
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockTransferStatus(objINTrx.EnumStockTransferStatus.Confirmed), objINTrx.EnumStockTransferStatus.Confirmed))
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockTransferStatus(objINTrx.EnumStockTransferStatus.DbNote), objINTrx.EnumStockTransferStatus.DbNote))
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockTransferStatus(objINTrx.EnumStockTransferStatus.Deleted), objINTrx.EnumStockTransferStatus.Deleted))
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockTransferStatus(objINTrx.EnumStockTransferStatus.Closed), objINTrx.EnumStockTransferStatus.Closed))
        lstStatus.SelectedIndex = 2
    End Sub

    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        Dim strStkTransferIdFrom As String
        Dim strStkTransferIdTo As String
        ...
        If txtStkTransferIdFrom.Text = "" Then
            strStkTransferIdFrom = ""
        Else
            strStkTransferIdFrom = Trim(txtStkTransferIdFrom.Text)
        End If

        If txtStkTransferIdTo.Text = "" Then
            strStkTransferIdTo = ""
        Else
            strStkTransferIdTo = Trim(txtStkTransferIdTo.Text)
        End If

        If txtToLocation.Text = "" Then
            strToLocation = ""
        Else
            strToLocation = Trim(txtToLocation.Text)
        End If
        ...
        strStkTransferIdFrom = Server.UrlEncode(strStkTransferIdFrom)
        strStkTransferIdTo = Server.UrlEncode(strStkTransferIdTo)
        ...
        Try
            intErrNo = objSysCfg.mtdGetConfigInfo(strParam, _
                                                  ...
                                                  objSysCfgDs)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=IN_STDRPT_STKTRANSFER_GET_CONFIG_DATE&errmesg=" & Exp.ToString() & "&redirect=../en/reports/IN_StdRpt_Selection.aspx")
        End Try
        ...
        If Not (strDateFrom = "" And strDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDateFrom, objDateFormat, objDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDateTo, objDateFormat, objDateTo) = True Then
                Response.Write("<Script Language=""JavaScript"">window.open(""IN_StdRpt_StkTransferListPreview.aspx?Type=Print&CompName=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & _
                            "&Decimal=" & strDec & "&lblLocation=" & lblLocation.Text & "&DateFrom=" & objDateFrom & "&DateTo=" & objDateTo & "&DDLAccMth=" & strddlAccMth & "&DDLAccYr=" & strddlAccYr & _
                            ...
                            "&lblStkAnaCode=" & lblStkAnaCode.Text & "&StkTransferIDFrom=" & strStkTransferIdFrom & "&StkTransferIDTo=" & strStkTransferIdTo & "&ToLocation=" & strToLocation & _
