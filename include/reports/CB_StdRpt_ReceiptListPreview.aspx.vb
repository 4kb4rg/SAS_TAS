...
Public Class CB_StdRpt_ReceiptList_Preview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strFileName = "CB_StdRpt_ReceiptListing"
        ...
        Session("SS_Status") = objCBTrx.mtdGetReceiptStatus(Trim(Request.QueryString("Status")))
    ...
    Sub BindReport()
        ...
        Dim strOpCd_ReceiptList_GET As String = "CB_STDRPT_RECEIPT_LIST_GET"
        ...
        Try
            intErrNo = objCB.mtdGetReport_ReceiptList(strOpCd_ReceiptList_GET, strParam, objRptDs, objMapPath, objFTPFolder)
        Catch Exp As Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=CB_STDRPT_ReceiptList&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        
        For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
            If LCase(TypeName(objRptDs.Tables(0).Rows(intCnt).Item("DocType"))) = "string" Then
                IF Trim(objRptDs.Tables(0).Rows(intCnt).Item("DocType")) = "106"
                    objRptDs.Tables(0).Rows(intCnt).Item("DocType") = "Advance Receipt"
                Else
                    objRptDs.Tables(0).Rows(intCnt).Item("DocType") = objGlobal.mtdGetDocName(Trim(objRptDs.Tables(0).Rows(intCnt).Item("DocType")))
                End If
            End If
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objCBTrx.mtdGetReceiptStatus(Trim(objRptDs.Tables(0).Rows(intCnt).Item("Status")))
        Next
