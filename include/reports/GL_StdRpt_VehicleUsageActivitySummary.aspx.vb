...
Public Class GL_StdRpt_VehicleUsageActivitySummary : Inherits Page
...
#Region " -- Form Properties -- "
    ...
    Protected WithEvents ddlVehAct As DropDownList
#End Region
    ...
    Sub BindVehAct()
        ...
        Dim strOpCdAccCode_Get As String = "GL_CLSSETUP_ACCOUNTCODE_LIST_GET_FOR_VEHUSAGE" '"GL_CLSSETUP_ACCOUNTCODE_LIST_GET" '"GL_CLSTRX_ACCOUNTCODE_LIST_GET"
        Dim dsForDropDown As DataSet
        Dim SelectedIndex As Integer = -1
        Dim intCnt As Integer
        Dim strFieldCheck As String
        'Dim strParam As New StringBuilder("")
        Dim strParam As String = "Order By VEH.VehActCode ASC|And ACC.Status = '" & objGLSetup.EnumAccountCodeStatus.Active & "'"
        ...
        strParam = strParam & IIf(Session("SS_COACENTRALIZED") = "1", "", " AND ACC.LocCode = '" & Trim(strLocation) & "' AND ACC.LocCode = VEH.LocCode")
        ...
        For intCnt = 0 To dsForDropDown.Tables(0).Rows.Count - 1
            ...
            'dsForDropDown.Tables(0).Rows(intCnt).Item(1) = Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("VehActCode")) & " - " & _
            '                                              Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("VehActDesc"))
            'dsForDropDown.Tables(0).Rows(intCnt).Item(0) = Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("AccCode"))
            dsForDropDown.Tables(0).Rows(intCnt).Item(0) = Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("VehActCode"))
            dsForDropDown.Tables(0).Rows(intCnt).Item(1) = Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("VehActCode")) & " - " & _
                                                         Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("VehActDesc")) & ", " & _
                                                         Trim(dsForDropDown.Tables(0).Rows(intCnt).Item("AccCode"))
        ...
        drinsert(0) = " "
        drinsert(1) = "Please select" & GetCaption(objLangCap.EnumLangCap.Activity) & " " & GetCaption(objLangCap.EnumLangCap.Vehicle) & lblCode.Text
        dsForDropDown.Tables(0).Rows.InsertAt(drinsert, 0)

        ddlVehAct.DataSource = dsForDropDown.Tables(0)
        ddlVehAct.DataValueField = "VehActCode"
        ddlVehAct.DataTextField = "Description"
        ddlVehAct.DataBind()
        ddlVehAct.SelectedIndex = intSelectedIndex
