...
Public Class CT_StdRpt_MthlyGoodsMoveList_Preview : Inherits Page
    ...
    Dim strDDLAccMthBF As Integer
    Dim strDDLAccYrBF As Integer
    Dim strDDLAccMth As String
    Dim strDDLAccYr As String
    Dim strCanteenItemType As String

    Dim tblGoodsMove As DataTable = New DataTable("tblGoodsMove")
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strCanteenItemType = Trim(Request.QueryString("CanteenItemType"))
    ...
    Sub BindReport()
        Dim objRptDsGoodsMove As New DataSet()
        ...
        Dim strOpCdMthlyGoodsMove_GET As String = "CT_STDRPT_MONTHLY_GOODSMOVE_GET_SP"


        SearchStr = "AND ITM.ItemType = '" & strCanteenItemType & "' "
        ...
        For intCntLocCode = 0 To arrLocCode.GetUpperBound(0)
            ...
            strParam = arrLocCode(intCntLocCode) & "|" & strDDLAccMthBF & "|" & strDDLAccYrBF & "|" & strDDLAccMth & "|" & strDDLAccYr & "|''" & _
                    ...
                    Convert.ToString(objGlobal.EnumDocType.CanteenReturnAdvice) & "|" & Replace(SearchStr, "'", "''") & "|''" & _
                    Convert.ToString(objPUTrx.EnumGRStatus.Confirmed) & "'',''" & Convert.ToString(objPUTrx.EnumGRStatus.Closed) & "''|''" & _
                    Convert.ToString(objPUTrx.EnumGRNStatus.Confirmed) & "'',''" & Convert.ToString(objPUTrx.EnumGRNStatus.Closed) & "''|''" & _
                    Convert.ToString(objPUTrx.EnumDAStatus.Confirmed) & "'',''" & Convert.ToString(objPUTrx.EnumDAStatus.Closed) & "''"
            Try
                intErrNo = objCT.mtdGetReport_MthCTGoodsMovement(strOpCdMthlyGoodsMove_GET, strParam, dsBalBF, objMapPath)
            Catch Exp As System.Exception
                Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_MTHLYGOODSMOVE_BALBF_LIST&errmesg=" & lblErrMessage.Text & "&redirect=")
            End Try

            If dsBalBF.Tables(0).Rows.Count > 0 Then
                For intCnt = 0 To dsBalBF.Tables(0).Rows.Count - 1
                    dr = tblGoodsMove.NewRow()
                    ...
                    dr("OrderQty") = dsBalBF.Tables(0).Rows(intCnt).Item("OrderQty")
                    dr("OrderCost") = dsBalBF.Tables(0).Rows(intCnt).Item("OrderCost")
                    dr("IssQty") = dsBalBF.Tables(0).Rows(intCnt).Item("IssQty")
                    dr("IssCost") = dsBalBF.Tables(0).Rows(intCnt).Item("IssCost")
                    dr("AdjQty") = dsBalBF.Tables(0).Rows(intCnt).Item("AdjQty")
                    dr("AdjCost") = dsBalBF.Tables(0).Rows(intCnt).Item("AdjCost")
                    dr("BalCFQty") = dsBalBF.Tables(0).Rows(intCnt).Item("BalCFQty")
                    dr("BalCFCost") = dsBalBF.Tables(0).Rows(intCnt).Item("BalCFCost")
                    tblGoodsMove.Rows.Add(dr)
                Next
            End If
        Next
        objRptDsGoodsMove.Tables.Add(tblGoodsMove)


        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\CT_StdRpt_MthlyGoodsMoveList.rpt", OpenReportMethod.OpenReportByTempCopy)
        rdCrystalViewer.SetDataSource(objRptDsGoodsMove.Tables(0))
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\CT_StdRpt_MthlyGoodsMoveList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/CT_StdRpt_MthlyGoodsMoveList.pdf"">")

        dsBalBF = Nothing
        objMapPath = Nothing
        objRptDsGoodsMove = Nothing
    End Sub
    ...
    Sub CreateTables()
        ...
        Col6.DefaultValue = 0
        tblGoodsMove.Columns.Add(Col6)

        Col7.DataType = System.Type.GetType("System.Decimal")
        Col7.AllowDBNull = True
        Col7.Caption = "OrderQty"
        Col7.ColumnName = "OrderQty"
        Col7.DefaultValue = 0
        tblGoodsMove.Columns.Add(Col7)

        Col8.DataType = System.Type.GetType("System.Decimal")
        Col8.AllowDBNull = True
        Col8.Caption = "OrderCost"
        Col8.ColumnName = "OrderCost"
        Col8.DefaultValue = 0
        tblGoodsMove.Columns.Add(Col8)

        Col9.DataType = System.Type.GetType("System.Decimal")
        Col9.AllowDBNull = True
        Col9.Caption = "IssQty"
        Col9.ColumnName = "IssQty"
        Col9.DefaultValue = 0
        tblGoodsMove.Columns.Add(Col9)

        Col10.DataType = System.Type.GetType("System.Decimal")
        Col10.AllowDBNull = True
        Col10.Caption = "IssCost"
        Col10.ColumnName = "IssCost"
        Col10.DefaultValue = 0
        tblGoodsMove.Columns.Add(Col10)
        ...
        tblGoodsMove.Columns.Add(Col11)

        Col12.DataType = System.Type.GetType("System.Decimal")
        Col12.AllowDBNull = True
        Col12.Caption = "AdjCost"
        Col12.ColumnName = "AdjCost"
        Col12.DefaultValue = 0
        tblGoodsMove.Columns.Add(Col12)
        ...
        tblGoodsMove.Columns.Add(Col13)
        ...
        tblGoodsMove.Columns.Add(Col14)
