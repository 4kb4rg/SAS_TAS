...
Public Class PM_StdRpt_OilQualityTransactionListPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR_GET As String = "PM_CLSRPT_OIL_QUALITY_GET"

        SearchStr = " AND PMOQ.LocCode IN('" & strUserLoc & "') "

        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = SearchStr & " AND (DateDiff(Day, '" & Session("SS_DATEFROM") & "', PMOQ.TransDate) >= 0) And (DateDiff(Day, '" & Session("SS_DATETO") & "', PMOQ.TransDate) <= 0) "
        End If
        ...
        Try
            intErrNo = objPM.mtdGetOilQuality(strOpCdPR_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_PM_OIL_QUALITY_GET_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try



        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\PM_StdRpt_OilQualityTransactionList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\PM_StdRpt_OilQualityTransactionList.pdf"
 	...
 	rdCrystalViewer.Close()
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/PM_StdRpt_OilQualityTransactionList.pdf"">")
    End Sub
