...
Public Class IN_StdRpt_StkIssueList_Preview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Session("SS_lblBatchNo") = Request.QueryString("lblBatchNo")
        Session("SS_BatchNo") = Request.QueryString("BatchNo")
        ...
        Session("SS_StkIssueIDFrom") = Request.QueryString("StkIssueIDFrom")
        Session("SS_StkIssueIDTo") = Request.QueryString("StkIssueIDTo")
        Session("SS_StkIssueType") = Request.QueryString("StkIssueType")
    ...
    Sub BindReport()
        ...
        Dim strOpCdStkIssue_BlkGrp_GET As String = "IN_STDRPT_STKISSUE_AND_STKISSUELN_BLKGRP_GET"
        Dim strOpCdStkIssue_Blk_GET As String = "IN_STDRPT_STKISSUE_AND_STKISSUELN_BLK_GET"
        Dim strOpCdStkIssue_SubBlk_GET As String = "IN_STDRPT_STKISSUE_AND_STKISSUELN_SUBBLK_GET"
        Dim strOpCdStkIssueListGet As String = "IN_STDRPT_STKISSUE_LISTING_GET"
        ...
        Dim WildStr As String = "AND STKISS.StockIssueID *= STKISSLn.StockIssueID AND "
        Dim NormStr As String = "AND STKISS.StockIssueID = STKISSLn.StockIssueID AND "

        SearchStr = ""
        
        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = SearchStr & "  AND (DateDiff(Day, '" & Session("SS_DATEFROM") & "', STKISS.StockIssueDate) >= 0) AND (DateDiff(Day, '" & Session("SS_DATETO") & "', STKISS.StockIssueDate) <= 0)" & vbCrLf
        End If

        If Not (Request.QueryString("StkIssueIDFrom") = "" And Request.QueryString("StkIssueIDTo") = "") Then
            SearchStr = SearchStr & "  AND STKISS.StockIssueID >= '" & Request.QueryString("StkIssueIDFrom") & "' AND STKISS.StockIssueID <= '" & Request.QueryString("StkIssueIDTo") & "'" & vbCrLf
        End If

        If Not Request.QueryString("StkIssueType") = objINTrx.mtdGetStockIssueType(objINTrx.EnumStockIssueType.All) Then
            SearchStr = SearchStr & "  AND STKISS.IssueType = '" & Request.QueryString("StkIssueType") & "'" & vbCrLf
        End If
        
        If Not Request.QueryString("Status") = "" Then
            If Not Request.QueryString("Status") = objINTrx.EnumStockIssueStatus.All Then
                SearchStr = SearchStr & "  AND STKISS.Status = '" & Request.QueryString("Status") & "'" & vbCrLf
            End If
        End If

        If Not Request.QueryString("AccCode") = "" Then
            SearchStr = SearchStr & "  AND STKISSLN.AccCode LIKE '%" & Request.QueryString("AccCode") & "%'" & vbCrLf
        End If
        
        If strBlkType = "BlkGrp" Then
            If Not Request.QueryString("BlkGrp") = "" Then
                SearchStr = SearchStr & "  AND STKISSLN.BlkCode IN (SELECT BlkCode FROM GL_BLOCK WHERE BlkGrpCode LIKE '%" & Request.QueryString("BlkGrp") & "%')" & vbCrLf
            End If
            Session("SS_BLKGRPHEADER") = "BlkGrp"
        ElseIf strBlkType = "BlkCode" Then
            If Not Request.QueryString("BlkCode") = "" Then
                SearchStr = SearchStr & "  AND BLK.BlkCode LIKE '%" & Request.QueryString("BlkCode") & "%'" & vbCrLf
            End If
            Session("SS_BLKHEADER") = "BlkCode"
        ElseIf strBlkType = "SubBlkCode" Then
            If Not Request.QueryString("SubBlkCode") = "" Then
                SearchStr = SearchStr & "  AND STKISSLN.BlkCode LIKE '%" & Request.QueryString("SubBlkCode") & "%'" & vbCrLf
            End If
            Session("SS_SUBBLKHEADER") = "SubBlkCode"
        End If
        
        If Not (Request.QueryString("VehCode") = "" And Request.QueryString("VehTypeCode") = "") Then
            SearchStr = SearchStr & "  AND STKISSLN.VehCode IN (SELECT VEH.VehCode FROM GL_VEHICLE VEH WHERE "
        ...
        If Not Request.QueryString("VehExpCode") = "" Then
            SearchStr = SearchStr & "  AND STKISSLN.VehExpCode LIKE '%" & Request.QueryString("VehExpCode") & "%'" & vbCrLf
        End If
        
        If Not Request.QueryString("BatchNo") = "" Then
            SearchStr = SearchStr & "  AND STKISSLN.BatchNo LIKE '%" & Request.QueryString("BatchNo") & "%'" & vbCrLf
        End If
        ...
        strOpCdStkIssue = strOpCdStkIssueListGet

        Try
            intErrNo = objIN.mtdGetReport_StockIssueList(strOpCdStkIssue, strParam, objRptDs, objMapPath)
        ...
        If Not Request.QueryString("AccCode") = "" Or Not Request.QueryString("VehTypeCode") = "" Or Not Request.QueryString("VehCode") = "" Or Not Request.QueryString("VehExpCode") = "" Or Not Request.QueryString("BlkGrp") = "" Or Not _
            ...
            If objRptDs.Tables(0).Rows.Count > 0 Then
                Do
                    If IsDBNull(objRptDs.Tables(0).Rows(intCntRem).Item("StockIssueLnId")) Then
        ...
        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\IN_StdRpt_StkIssueList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        'crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\IN_StdRpt_StkIssueList.pdf"
        ...
        If strExportToExcel = "0" Then
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\IN_StdRpt_StkIssueList.pdf"
        Else
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\IN_StdRpt_StkIssueList.xls"
        End If
        crExportOptions = rdCrystalViewer.ExportOptions
        With crExportOptions
            .DestinationOptions = crDiskFileDestinationOptions
        ...
        'Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/IN_StdRpt_StkIssueList.pdf"">")
        If strExportToExcel = "0" Then
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/IN_StdRpt_StkIssueList.pdf"">")
        Else
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/IN_StdRpt_StkIssueList.xls"">")
        End If
    End Sub
    ...
    Sub PassParam()
        ...
        paramField46 = paramFields.Item("ParamInterEstateEnabled")
        paramField49 = paramFields.Item("ParamUser")
        ...
        ParamDiscreteValue4.Value = Session("SS_StkIssueIDFrom")
        ParamDiscreteValue5.Value = Session("SS_StkIssueIDTo")
        ParamDiscreteValue6.Value = Session("SS_StkIssueType")
        ParamDiscreteValue7.Value = Session("SS_AccCode")
        ParamDiscreteValue8.Value = Session("SS_VehCode")
        ParamDiscreteValue9.Value = Session("SS_VehExpCode")
        ParamDiscreteValue10.Value = Session("SS_BlkCode")
        ParamDiscreteValue11.Value = Session("SS_SubBlkCode")
        ...
        ParamDiscreteValue21.Value = Session("SS_LBLACCCODE")
        ParamDiscreteValue22.Value = Session("SS_LBLVEHCODE")
        ParamDiscreteValue23.Value = Session("SS_LBLVEHEXPCODE")
        ParamDiscreteValue24.Value = Session("SS_LBLBLKCODE")
        ParamDiscreteValue25.Value = Session("SS_LBLSUBBLKCODE")
        ParamDiscreteValue26.Value = Session("SS_ITEMCODE")
        ParamDiscreteValue27.Value = Session("SS_LBLLOCATION")
        ParamDiscreteValue28.Value = Session("SS_BLKTYPE")
        ParamDiscreteValue29.Value = Session("SS_BLKGRP")
        ParamDiscreteValue30.Value = Session("SS_LBLBLKGRP")
        ParamDiscreteValue31.Value = Session("SS_PRODTYPE")
        ParamDiscreteValue32.Value = Session("SS_PRODBRAND")
        ParamDiscreteValue33.Value = Session("SS_PRODMODEL")
        ParamDiscreteValue34.Value = Session("SS_PRODCAT")
        ParamDiscreteValue35.Value = Session("SS_PRODMAT")
        ParamDiscreteValue36.Value = Session("SS_STKANA")
        ParamDiscreteValue37.Value = Session("SS_LBLPRODTYPECODE")
        ParamDiscreteValue38.Value = Session("SS_LBLPRODBRANDCODE")
        ParamDiscreteValue39.Value = Session("SS_LBLPRODMODELCODE")
        ParamDiscreteValue40.Value = Session("SS_LBLPRODCATCODE")
        ParamDiscreteValue41.Value = Session("SS_LBLPRODMATCODE")
        ParamDiscreteValue42.Value = Session("SS_LBLSTKANACODE")
        ParamDiscreteValue43.Value = Session("SS_LBLVEHTYPECODE")
        ParamDiscreteValue44.Value = Session("SS_VEHTYPECODE")
        ParamDiscreteValue45.Value = Session("SS_lblBillPartyCode")
        ParamDiscreteValue46.Value = IIf(Session("SS_INTER_ESTATE_CHARGING"), "true", "false")
        ParamDiscreteValue47.Value = Session("SS_lblBatchNo")
        ParamDiscreteValue48.Value = Session("SS_BatchNo")
        ParamDiscreteValue49.Value = strUserId
