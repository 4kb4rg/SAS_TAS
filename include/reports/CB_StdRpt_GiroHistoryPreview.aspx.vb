...
Public Class CB_StdRpt_GiroHistoryPreview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strDocType = Trim(Request.QueryString("DocType"))
        strUsedGiro = Trim(Request.QueryString("UsedGiro"))
    ...
    Sub BindReport()
        ...
        If strUsedGiro = 0 Then
            searchStr = " AND COALESCE(DocID,'') = '' "
            strOpCode = "CB_STDRPT_GIROHISTORY_NOTUSED_GET"
            strFileName = "CB_StdRpt_GiroHistoryNotUsed_Report"
        Else
            searchStr = " AND COALESCE(DocID,'') <> '' "
            strOpCode = "CB_STDRPT_GIROHISTORY_GET"
            strFileName = "CB_StdRpt_GiroHistory_Report"
        End If

        strParamName = "TRXDATE|TRXDATETO|ACCCODE|LOCCODE|DOCTYPE|STRSEARCH"
        strParamValue = strDate & "|" & strDateTo & "|" & strAccCode & "|" & strLocation & "|" & IIf(strDocType = "9", "0','3", strDocType) & "|" & searchStr

        Try
            intErrNo = objGLRpt.mtdGetReport_BiayaProduksi(strOpCode, strParamName, strParamValue, objRptDs, objMapPath, objFTPFolder)

        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=CB_STDRPT_GIROHISTORY_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
