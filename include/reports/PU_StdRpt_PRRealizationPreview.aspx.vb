...
Public Class PU_StdRpt_PRRealizationPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR_GET As String = "IN_STDRPT_PRREALIZATION"

        Dim WildStr As String = "PR.PRID *= PRLn.PRID "
        Dim NormStr As String = "PR.PRID = PRLn.PRID "

        If UCase(Request.QueryString("PRStatus")) = "OUTSTANDING" Then
            strOpCdPR_GET = "IN_STDRPT_PRREALIZATION_OUTSTANDING"
        ElseIf UCase(Request.QueryString("PRStatus")) = "OUTSTANDING" Then
            strOpCdPR_GET = "IN_STDRPT_PRREALIZATION_CLOSED"
        Else
            strOpCdPR_GET = "IN_STDRPT_PRREALIZATION"
        End If
        ...
        If Not (Request.QueryString("PRNoFrom") = "" And Request.QueryString("PRNoTo") = "") Then
            SearchStr = SearchStr & "AND PR.PRID IN (SELECT SUBPR.PRID FROM IN_PR SUBPR WHERE SUBPR.PRID >= '" & Request.QueryString("PRNoFrom") & _
                        "' AND SUBPR.PRID <= '" & IIf(Request.QueryString("PRNoTo") = "", Request.QueryString("PRNoFrom"), Request.QueryString("PRNoTo")) & "') "
        End If

        If Request.QueryString("PRType") = "All" Then
            SearchStr = SearchStr & "AND PR.PRType IN ('" & objINTrx.EnumPurReqDocType.StockPR & "','" & objINTrx.EnumPurReqDocType.DirectChargePR & "','" & objINTrx.EnumPurReqDocType.WorkshopPR & "','" & objINTrx.EnumPurReqDocType.FixedAssetPR & "','" & objINTrx.EnumPurReqDocType.NurseryPR & "') "
        ...
        ElseIf Request.QueryString("PRType") = objINTrx.EnumPurReqDocType.FixedAssetPR Then
            SearchStr = SearchStr & "AND PR.PRType = '" & objINTrx.EnumPurReqDocType.FixedAssetPR & "' "  
        ElseIf Request.QueryString("PRType") = objINTrx.EnumPurReqDocType.NurseryPR Then
            SearchStr = SearchStr & "AND PR.PRType = '" & objINTrx.EnumPurReqDocType.NurseryPR & "' "        
        End If

        If Request.QueryString("PRStatus") = "All" Then
            SearchStr = SearchStr & "AND PR.Status LIKE '%' "
        ...
        If Not Request.QueryString("RefLocCode") = "" Then
            SearchStr = SearchStr & "AND SUBSTRING(PR.PRID,PATINDEX('%/%',PR.PRID)+5,3) = '" & Request.QueryString("RefLocCode") & "' "
        End If
        ...
        strParam = strUserLoc & "|" & itmSearchStr & "|" & NormStr & SearchStr
        ...
        Else
            crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\PU_StdRpt_PRRealization.xls"
        End If
        ...
        Else
            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/PU_StdRpt_PRRealization.xls"">")
        End If
