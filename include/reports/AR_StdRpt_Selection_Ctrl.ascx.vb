...
Public Class AR_STDRPT_SELECTION_CTRL : Inherits UserControl
    ...
    Sub BindReportNameList()
        ...
        strParam = " WHERE ReportType = '" & Convert.ToString(objGlobal.EnumStdRptType.AccountReceivables) & "'   AND Reportid IN (Select ReportID From SH_REPORT_USER Where UserId='" & strUserId & "') AND Status = '" & Convert.ToString(objGlobal.EnumStdRptStatus.Active) & "' ORDER BY RptName"
        Try
    ...
    Sub CheckRptName(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        ElseIf strSelectedIndex = "rptar1000015" Then
            Response.Redirect("../../" & strLangCode & "/reports/AR_StdRpt_Customer_AgeingList.aspx?RptName=" & strRptname & "&SelIndex=" & Convert.ToString(intSelectedIndex) & "&UserLoc=" & strUserLoc)
		...
		ElseIf strSelectedIndex = "rptar1000020" Then
            Response.Redirect("../../" & strLangCode & "/reports/AR_StdRpt_MutasiAccountReceivable.aspx?RptName=" & strRptname & "&SelIndex=" & Convert.ToString(intSelectedIndex) & "&UserLoc=" & strUserLoc)        
		End If
    End Sub
End Class
