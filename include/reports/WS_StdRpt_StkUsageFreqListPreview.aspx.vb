...
Public Class WS_StdRpt_StkUsageFreqListPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCdItem_GET As String = "WS_STDRPT_STKUSAGEFREQ_ITEM_GET" 
        Dim strOpCdPRLn_GET As String = "WS_STDRPT_STKUSAGEFREQ_PRLN_GET" 
        ...
        If Not SearchStr = "" Then
            ...
            strParam = objINSetup.EnumInventoryItemType.WorkshopItem & "|" & strUserLoc & "|" & objINSetup.EnumStockItemStatus.Active & "|" & _
                       SearchStr & "|" & Request.QueryString("SortBy") & "|" & Request.QueryString("OrderBy")
        End If

        Try
			
			intErrNo = objWSReport.mtdGetReport_StkUsageFreqList(strOpCdItem_GET, strParam, objRptDs, objMapPath)
        ...
        For intItemCode = 0 To objRptDs.Tables(0).Rows.Count - 1
            ...
            If objRptDs.Tables(0).Rows.Count > 0 Then
                strParamItemCode = objINTrx.EnumPurReqDocType.StockPR & "|" & strItemCode & "|"
                Try
					
					intErrNo = objWSReport.mtdGetStkUsageFreqPurchaseRequestLn(strOpCdPRLn_GET, strParamItemCode, objRptPRLn)
                Catch Exp As System.Exception
                    Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_STKUSGFREQ_PRLN_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
                End Try
            End If
            objRptDs.Tables(0).Rows(intItemCode).Item("QtyOutstanding") = CDbl(objRptPRLn.Tables(0).Rows(0).Item("QtyOutstanding"))
        Next intItemCode


        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\WS_StdRpt_StkUsageFreqList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\WS_StdRpt_StkUsageFreqList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/WS_StdRpt_StkUsageFreqList.pdf"">")
    End Sub
