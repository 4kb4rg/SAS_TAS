...
Public Class PU_StdRpt_POList : Inherits Page
    ...
    Protected WithEvents rbYes As RadioButton
    Protected WithEvents rbNo As RadioButton
    Protected WithEvents lstPRRefLoc As DropDownList
    ...
    Sub BindLoc(ByVal pv_strPRId As String)
        ...
        Try
            intErrNo = objPUTrx.mtdGetLoc(strOpCd, strParam, objLocDs)
        Catch Exp As System.Exception
        ...
        dr("LocCode") = ""
        dr("Description") = "Please select " & lblLocation.Text
        objLocDs.Tables(0).Rows.InsertAt(dr, 0)

        lstPRRefLoc.DataSource = objLocDs.Tables(0)
        lstPRRefLoc.DataValueField = "LocCode"
        lstPRRefLoc.DataTextField = "Description"
        lstPRRefLoc.DataBind()
        lstPRRefLoc.SelectedIndex = intSelectedIndex
    End Sub
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        strPRRefLoc = Trim(lstPRRefLoc.SelectedItem.Value)
        ...
        If Not (strDocDateFrom = "" And strDocDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDocDateFrom, objDateFormat, objDocDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDocDateTo, objDateFormat, objDocDateTo) = True Then
                Response.Write("<Script Language=""JavaScript"">window.open(""PU_StdRpt_POListPreview.aspx?CompName=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                               ...
                               "&LnStatus=" & strLnStatus & "&strView=" & strView & "&strPRRefLoc=" & strPRRefLoc & "&ExportToExcel=" & strExportToExcel & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
            Else
