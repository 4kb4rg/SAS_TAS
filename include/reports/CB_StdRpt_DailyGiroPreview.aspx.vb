...
Public Class CB_StdRpt_DailyGiroPreview : Inherits Page
    ...
    Dim strGrpAmount As Decimal
    Dim strGrpAmountTo As Decimal
    Dim strTransIdFr As String
    Dim strTransidTo As String
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strGrpAmount = Trim(Request.QueryString("GrpAmount"))
        strGrpAmountTo = Trim(Request.QueryString("GrpAmountTo"))
        strTransIDFr = Trim(Request.QueryString("transidFr"))
        strTransidTo = Trim(Request.QueryString("transidTo"))
    ...
    Sub BindReport()
        ...
        Dim strOpCode As String = "CB_STDRPT_DAILYGIRO_GET"
        Dim strFileName As String = "CB_StdRpt_DailyGiro_Report"
        ...
        If Trim(strTransIdFr) <> "" And Trim(strTransidTo) <> "" Then
            sSQLKriteria = "Where DocID Between '" & strTransIdFr & "' And '" & strTransidTo & "'"
        End If

        strParamName = "TRXDATE|LOCCODE|STRSEARCH|GROUPAMOUNT|GROUPAMOUNTTO|STRSEARCH2"
        If strAccCode = "" Then
            strParamValue = strDate & "|" & strLocation & "|" & " " & "|" & strGrpAmount & "|" & strGrpAmountTo & "|" & sSQLKriteria
        Else
            strParamValue = strDate & "|" & strLocation & "|" & " AND A.BankAccNo IN (SELECT BankAccNo FROM SH_LOCATION_BANK WHERE LocCode = '" & strLocation & "' AND AccCode = '" & strAccCode & "')" & "|" & strGrpAmount & "|" & strGrpAmountTo & "|" & sSQLKriteria
        End If

        Try
            intErrNo = objGLRpt.mtdGetReport_BiayaProduksi(strOpCode, strParamName, strParamValue, objRptDs, objMapPath, objFTPFolder)

            objRptDs.Tables(0).TableName = "CB_StdRpt_DailyGiro_Report"
            objRptDs.Tables(1).TableName = "CB_StdRpt_DailyGiroLast_Report"

        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=CB_STDRPT_DAILYGIRO_RPT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        objRptDs1.Tables.Add(objRptDs.Tables(0).Copy())
        objRptDs1.Tables(0).TableName = "CB_StdRpt_DailyGiro_Report"
        objRptDs1.Tables.Add(objRptDs.Tables(1).Copy())
        objRptDs1.Tables(1).TableName = "CB_StdRpt_DailyGiroLast_Report"
