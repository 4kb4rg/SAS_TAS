...
Public Class AR_StdRpt_ContractDebitNotePreview : Inherits Page
    ...
    Dim strFormDebitNoteDate As String
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            strFromDebitNoteId = Trim(Request.QueryString("FromDNId"))
            strToDebitNoteId = Trim(Request.QueryString("ToDNId"))
            strFormDebitNoteDate = Trim(Request.QueryString("FromDNDate"))
            strToDebitNoteDate = Trim(Request.QueryString("ToDNDate"))
            strBillPartyCode = Trim(Request.QueryString("BillPartyCode"))
            strDebitNoteType = Trim(Request.QueryString("DNType"))
            BillPartyTag = Trim(Request.QueryString("BillPartyTag"))
            strBankAccNo = Trim(Request.QueryString("BankAccNo"))
            strBankName = Trim(Request.QueryString("BankName"))
            ...
            strSearchExp = "and dn.LocCode = '" & strUserLoc & "' " & _
                            "and dn.DocType = '" & strDebitNoteType & "' " & _
                            "and dn.Status = '" & objBITrx.EnumDebitNoteStatus.Confirmed & "' "
            ...
            If Trim(strFromDebitNoteId) <> "" Then
                strSearchExp = strSearchExp & "and dn.DebitNoteId >= '" & strFromDebitNoteId & "' "
            End If

            If Trim(strToDebitNoteId) <> "" Then
                strSearchExp = strSearchExp & "and dn.DebitNoteId <= '" & strToDebitNoteId & "' "
            End If

            If Trim(strFormDebitNoteDate) <> "" Then
                strSearchExp = strSearchExp & "and datediff(d, '" & strFormDebitNoteDate & "', dn.CreateDate) >= 0 "
            End If
            
            If Trim(strToDebitNoteDate) <> "" Then
                strSearchExp = strSearchExp & "and datediff(d, '" & strToDebitNoteDate & "', dn.CreateDate) <= 0 "
            End If
    ...
    Sub BindReport()
        ...
        Select Case CInt(strDebitNoteType)
            Case objBITrx.EnumDebitNoteDocType.Manual 
                strRptPrefix = "AR_StdRpt_ManualDebitNote"
                strOpCd = "AR_STDRPT_MANUAL_DEBITNOTE_GET_SP" & "|" & "AR_DEBIT"
            Case objBITrx.EnumDebitNoteDocType.Manual_Millware
                strRptPrefix = "AR_StdRpt_ManualMillDebitNote"
                strOpCd = "AR_STDRPT_MANUALMILL_DEBITNOTE_GET_SP" & "|" & "AR_DEBIT"
            Case objBITrx.EnumDebitNoteDocType.Auto_Millware
                strRptPrefix = "AR_StdRpt_AutoMillDebitNote"
                strOpCd = "AR_STDRPT_AUTOMILL_DEBITNOTE_GET_SP" & "|" & "AR_DEBIT"
            Case objBITrx.EnumDebitNoteDocType.Auto_INIssue_Staff, objBITrx.EnumDebitNoteDocType.Auto_CTIssue_Staff, objBITrx.EnumDebitNoteDocType.Auto_INFuelIssue_Staff, objBITrx.EnumDebitNoteDocType.Auto_WSJob_Staff
                strRptPrefix = "AR_StdRpt_AutoStaffDebitNote"
                strOpCd = "AR_STDRPT_AUTOSTAFF_DEBITNOTE_GET_SP" & "|" & "AR_DEBIT"
            Case objBITrx.EnumDebitNoteDocType.Auto_INIssue_External, objBITrx.EnumDebitNoteDocType.Auto_CTIssue_External, objBITrx.EnumDebitNoteDocType.Auto_INFuelIssue_External, objBITrx.EnumDebitNoteDocType.Auto_WSJob_External
                strRptPrefix = "AR_StdRpt_AutoExternalDebitNote"
                strOpCd = "AR_STDRPT_AUTOEXTERNAL_DEBITNOTE_GET_SP" & "|" & "AR_DEBIT"               
            Case objBITrx.EnumDebitNoteDocType.Auto_INTransfer, objBITrx.EnumDebitNoteDocType.Auto_CTTransfer
                strRptPrefix = "AR_StdRpt_AutoTransferDebitNote"
                strOpCd = "AR_STDRPT_AUTOTRANSFER_DEBITNOTE_GET_SP" & "|" & "AR_DEBIT"
        End Select        

        strParam = strSelAccMonth & "|" & _
                   strSelAccYear & "|" & _
                   strSearchExp & "|" & _
                   objBITrx.EnumDebitNoteStatus.Active & "|" & _
                   objBITrx.EnumDebitNoteStatus.Confirmed


        Try
            intErrNo = objARRpt.mtdGetReport_DebitNote(strOpCd, _
                                                       strCompany, _
                                                       strLocation, _
                                                       strUserId, _
                                                       strAccMonth, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=AR_STDRPT_DEBITNOTE_GET&errmesg=" & Exp.ToString() & "&redirect=")
        End Try
    ...
    Sub PassParam()
        ...
        ParamDiscreteValue2.Value = strBankAccNo
        ParamDiscreteValue3.Value = strBankName
        ParamDiscreteValue4.Value = strBankBranch
        ...
        if NOT CInt(strDebitNoteType) = objBITrx.EnumDebitNoteDocType.Manual_Millware AND NOT CInt(strDebitNoteType) =objBITrx.EnumDebitNoteDocType.Auto_Millware then
