...
Public Class NU_StdRpt_WorkAccDetMiPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCd_GET As String = "NU_STDRPT_NUWRKACCDET_MI_GET_SP"
        Dim strAccMonth As String
        Dim strAccYear As String
        
        strWhere = " ST.LocCode IN('" & strUserLoc & "')" & vbCrLf


        If Not Request.QueryString("BatchNo") = "" Then
            strWhere = strWhere & " AND ST_LN.BatchNo = '" & Replace(Request.QueryString("BatchNo"), "'", "''") & "'" & vbCrLf
        End If
        ...
        If strBlkType = "BlkCode" Then
            ...
            If Not Request.QueryString("BlkCode") = "" Then
                If blnCostAtBlock = True Then
                    strWhere = strWhere & " AND ST_LN.BlkCode LIKE '" & Request.QueryString("BlkCode") & "'" & vbCrLf
                Else
                    strWhere = strWhere & " AND ST_LN.BlkCode IN (SELECT DISTINCT SubBlkCode FROM GL_SUBBLK WHERE BlkCode LIKE '" & Request.QueryString("BlkCode") & "')" & vbCrLf
                End If
            End If
        ElseIf strBlkType = "SubBlkCode" Then
            ...
            If Not Request.QueryString("SubBlkCode") = "" Then
                If blnCostAtBlock = True Then
                    strWhere = strWhere & " AND ST_LN.BlkCode IN (SELECT DISTINCT BlkCode FROM GL_SUBBLK WHERE SubBlkCode LIKE '" & Request.QueryString("SubBlkCode") & "')" & vbCrLf
                Else
                    strWhere = strWhere & " AND ST_LN.BlkCode LIKE '" & Request.QueryString("SubBlkCode") & "'" & vbCrLf
                End If
            End If
        End If
           
        If Not Request.QueryString("AccClass") = "" Then
            strWhere = strWhere & " AND GL.AccCode = '" & Replace(Request.QueryString("AccClass"), "'", "''") & "'" & vbCrLf
        End If

        strWhereSub = replace(strWhereSub, "'", "''")        
        strWhere = replace(strWhere, "'", "''") 
        strAccMonth = intDDLAccMthFrom
        strAccYear = intDDLAccYrFrom
        Try
            intErrNo = objNU.mtdGetReport_NuWrkAccDetMi(strOpCd_GET, _    
                                                            strLocation, _                                                        
                                                            strAccMonth, _
                                                            strAccYear, _
                                                            strWhere, _
            ...
            objRptDs.Tables(0).TableName = "NU_StdRpt_WorkAccDet"
        ...
        objRptDs.Tables(0).TableName = "NU_StdRpt_WorkAccDet"
        objRptDs.Tables(1).TableName = "NU_StdRpt_WorkAccDetSub"

        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\NU_StdRpt_WorkAccDet.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\NU_StdRpt_WorkAccDet.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/NU_StdRpt_WorkAccDet.pdf"">")
    End Sub
    ...
    Sub PassParam()
        ...
        paramField14 = paramFields.Item("lblAcc")
        ...
        ParamDiscreteValue8.Value = lblBlockCode    
        ...
        ParamDiscreteValue14.Value = Request.QueryString("AccClass")
