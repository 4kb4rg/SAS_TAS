...
Public Class NU_StdRpt_SeedReceiveListPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCd_GET As String = "NU_STDRPT_SEEDRECEIVELIST_GET"


        If Not (Request.QueryString("TxIDFrom") = "" And Request.QueryString("TxIDTo") = "") Then
            SearchStr = SearchStr & " AND NU.ReceiveID >= '" & Request.QueryString("TxIDFrom") & "' AND NU.ReceiveID <= '" & Request.QueryString("TxIDTo") & "'"
        End If

        If Not (Request.QueryString("TxDateFrom") = "" And Request.QueryString("TxDateTo") = "") Then
            SearchStr = SearchStr & " AND (DateDiff(Day, '" & Request.QueryString("TxDateFrom") & "', NU.ReceiveDate) >= 0) And (DateDiff(Day, '" & Request.QueryString("TxDateTo") & "', NU.ReceiveDate) <= 0) "
        End If
        ...
        If Not Request.QueryString("Status") = "" Then
            If Not Request.QueryString("Status") = objNUTrx.EnumSeedReceiveStatus.All Then
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=NU_STDRPT_SEEDRECEIVE_GET_CHECKACCPERIOD&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        SearchStr = SearchStr & strAccPeriod
        SearchStr = SearchStr & " ORDER BY NU.ReceiveID ASC "

        strParam = strLocation & "|" & _
                    SearchStr

        Try
            intErrNo = objNU.mtdGetReport_SeedReceiveList(strOpCd_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_NU_SEEDRECEIVELIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try


        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\NU_StdRpt_SeedReceiveList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\NU_StdRpt_SeedReceiveList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/NU_StdRpt_SeedReceiveList.pdf"">")
    End Sub
