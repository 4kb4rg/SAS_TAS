...
Public Class PU_StdRpt_DAList : Inherits Page
    ...
    Protected WithEvents lstDAType As DropDownList
    ...
    Protected WithEvents rbToLocCode As RadioButton
    Protected WithEvents rbTrx As RadioButton
    Protected WithEvents txtTransporter As TextBox
    Protected WithEvents rbTransporter As RadioButton
    ...
    Sub BindDAType()
        ...
        lstDAType.Items.Add(New ListItem(objPUTrx.mtdGetDAType(objPUTrx.EnumDAType.All), objPUTrx.EnumDAType.All))
        lstDAType.Items.Add(New ListItem(objPUTrx.mtdGetDAType(objPUTrx.EnumDAType.Canteen), objPUTrx.EnumDAType.Canteen))
        lstDAType.Items.Add(New ListItem(objPUTrx.mtdGetDAType(objPUTrx.EnumDAType.DirectCharge), objPUTrx.EnumDAType.DirectCharge))
        lstDAType.Items.Add(New ListItem(objPUTrx.mtdGetDAType(objPUTrx.EnumDAType.FixedAsset), objPUTrx.EnumDAType.FixedAsset))
        lstDAType.Items.Add(New ListItem("Stock / Workshop", objPUTrx.EnumDAType.Stock)) 
    ...
    Sub BindStatus()
        ...
        lstStatus.Items.Add(New ListItem(objPUTrx.mtdGetDAStatus(objPUTrx.EnumDAStatus.All), objPUTrx.EnumDAStatus.All))
        lstStatus.Items.Add(New ListItem(objPUTrx.mtdGetDAStatus(objPUTrx.EnumDAStatus.Active), objPUTrx.EnumDAStatus.Active))
        lstStatus.Items.Add(New ListItem(objPUTrx.mtdGetDAStatus(objPUTrx.EnumDAStatus.Cancelled), objPUTrx.EnumDAStatus.Cancelled))
        lstStatus.Items.Add(New ListItem(objPUTrx.mtdGetDAStatus(objPUTrx.EnumDAStatus.Closed), objPUTrx.EnumDAStatus.Closed))
        lstStatus.Items.Add(New ListItem(objPUTrx.mtdGetDAStatus(objPUTrx.EnumDAStatus.Confirmed), objPUTrx.EnumDAStatus.Confirmed))
        lstStatus.Items.Add(New ListItem(objPUTrx.mtdGetDAStatus(objPUTrx.EnumDAStatus.Deleted), objPUTrx.EnumDAStatus.Deleted))
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        If txtTransporter.Text = "" Then
            strTransporter = ""
        Else
            strTransporter = Trim(txtTransporter.Text)
        End If

        strDAType = Trim(lstDAType.SelectedItem.Value)
        ...
        strTransporter = Server.UrlEncode(strTransporter)
       
        If rbTrx.Checked = True Then
            strGroupBy = "1"
        ElseIf rbItem.Checked = True Then
            strGroupBy = "2"
        ElseIf rbToLocCode.Checked = True Then
            strGroupBy = "3"
        ...
        Try
            intErrNo = objSysCfg.mtdGetConfigInfo(strParam, _
                                                  ...
                                                  objSysCfgDs)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PU_STDRPT_DALIST_GET_CONFIG_DATE&errmesg=" & lblErrMessage.Text & "&redirect=../../en/reports/PU_StdRpt_Selection.aspx")
        End Try
        ...
        If Not (strDocDateFrom = "" And strDocDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDocDateFrom, objDateFormat, objDocDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDocDateTo, objDateFormat, objDocDateTo) = True Then
                Response.Write("<Script Language=""JavaScript"">window.open(""PU_StdRpt_DAListPreview.aspx?CompName=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                               ...
                               "&DAType=" & strDAType & "&Status=" & strStatus & "&GroupBy=" & strGroupBy & "&Transporter=" & strTransporter & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
            Else
