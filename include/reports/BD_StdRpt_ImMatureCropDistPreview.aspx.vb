...
Public Class BD_StdRpt_ImMatureCropDistPreview : Inherits Page
    ...
    Sub CountAccPeriod()
        Dim strOppCd_ImMatureCropDist_AccPeriod_GET As String = "BD_STDRPT_UNMATURECROPDIST_ACCPERIOD_GET"
        Dim strParam As String

        strParam = "|" & strLocation & "|" & Request.QueryString("DDLPeriodID").Trim & "||||"
        Try
            intErrNo = objBDTrx.mtdGetUnMatureCropDist(strOppCd_ImMatureCropDist_AccPeriod_GET, strParam, dsAccPeriod)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_STDRPT_IMMATURECROPDIST_YEAR_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
    ...
    Sub BindReport()
        ...
        Dim strOpCd_ImMatureCropDist_Format_GET As String = "BD_STDRPT_UNMATURECROPDIST_FORMAT_GET"

        If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
            If Not Request.QueryString("BlkCode") = "" Then
                SearchStr = " AND UMCD.BlkCode LIKE '" & Request.QueryString("BlkCode") & "' "
            Else
                SearchStr = " AND UMCD.BlkCode LIKE '%' "
            End If
        Else
            If Not Request.QueryString("SubBlkCode") = "" Then
                SearchStr = " AND UMCD.BlkCode LIKE '" & Request.QueryString("SubBlkCode") & "' "
            Else
                SearchStr = " AND UMCD.BlkCode LIKE '%' "
            End If
        End If
        ...
        Try
            intErrNo = objBD.mtdGetReport_UnMatureCropDist(strOpCd_ImMatureCropDist_Format_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=BD_IMMATURECROPDIST_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        objRptDs.Tables(0).TableName = "BD_IMMATURECROPDIST_FORMAT"
        objRptDs.Tables.Add(dsAccPeriod.Tables(0).Copy())
        objRptDs.Tables(1).TableName = "BD_IMMATURECROPDIST_PERIOD"


        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\BD_StdRpt_ImMatureCropDist.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\BD_StdRpt_ImMatureCropDist.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/BD_StdRpt_ImMatureCropDist.pdf"">")
    End Sub
    ...
    Sub PassParam()
        ...
        paramField14 = paramFields.Item("ParamLocationTag")
        ...
        End If
        ParamDiscreteValue14.Value = UCase(strLocationTag)
    ...
    Sub GetEntireLangCap()
        ...
        Try
            intErrNo = objLangCap.mtdGetBussTerm(strOpCode_BussTerm, _
                                                 ...
                                                 strParam)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_STDRPT_IMMATURECROP_LANGCAP_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
