...
Public Class WM_StdRpt_TransporterMasterListPreview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Session("SS_TransporterCode") = Request.QueryString("TransporterCode")
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR_GET As String = "WM_STDRPT_TRANSPORTER_MASTER_LIST_GET"


        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = searchStr & " AND (DateDiff(Day, '" & Session("SS_DATEFROM") & "', WMT.UpdateDate) >= 0) And (DateDiff(Day, '" & Session("SS_DATETO") & "', WMT.UpdateDate) <= 0) "
        End If


        If Request.QueryString("Status") = "All" Then
            SearchStr = SearchStr & "AND WMT.Status LIKE '%' "
        ElseIf Request.QueryString("Status") = "Active" Then
            SearchStr = SearchStr & "AND WMT.Status = '" & objWMSetup.EnumTransporterStatus.Active & "'"
        ElseIf Request.QueryString("Status") = "Deleted" Then
            SearchStr = SearchStr & "AND WMT.Status = '" & objWMSetup.EnumTransporterStatus.Deleted & "'"
        End If

        If Not Request.QueryString("TransporterCode") = "" Then
            SearchStr = SearchStr & " AND WMT.TransporterCode LIKE '" & Request.QueryString("TransporterCode") & "'"
        Else
            SearchStr = SearchStr & " AND WMT.TransporterCode LIKE '%' "
        End If

        If Not Request.QueryString("Name") = "" Then
            SearchStr = SearchStr & " AND WMT.Name LIKE '" & Request.QueryString("Name") & "'"
        Else
            SearchStr = SearchStr & " AND WMT.Name LIKE '%' "
        End If

        If Not Request.QueryString("Telephone") = "" Then
            SearchStr = SearchStr & " AND WMT.TelNo LIKE '" & Request.QueryString("Telephone") & "'"
        Else
            SearchStr = SearchStr & " AND WMT.TelNo LIKE '%' "
        End If

        If Not Request.QueryString("UpdatedBy") = "" Then
            SearchStr = SearchStr & " AND WMT.UpdateID LIKE '" & Request.QueryString("UpdatedBy") & "'"
        Else
            SearchStr = SearchStr & " AND WMT.UpdateID LIKE '%' "
        End If
        ...
        Try
            intErrNo = objWM.mtdGetReport_TransporterMasterList(strOpCdPR_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_WM_TRANSPORTER_MASTER_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try



        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\WM_StdRpt_TransporterMasterList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\WM_StdRpt_TransporterMasterList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/WM_StdRpt_TransporterMasterList.pdf"">")
    End Sub
    ...
    Sub PassParam()
        ...
        paramField9 = paramFields.Item("ParamTransporterCode")
        ...
        ParamDiscreteValue9.Value = Session("SS_TRANSPORTERCODE")
