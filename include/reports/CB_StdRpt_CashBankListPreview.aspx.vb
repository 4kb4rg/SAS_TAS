...
Public Class CB_StdRpt_CashBankList_Preview : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Session("SS_CashBankIDFrom") = Request.QueryString("CashBankIDFrom")
        Session("SS_CashBankIDTo") = Request.QueryString("CashBankIDTo")
        ...
        Session("SS_PrintFor") = Request.QueryString("PrintFor")
    ...
    Sub BindReport()
        ...
        Dim strOpCd_CashBank_BlkGrp_And_Blk_GET As String = "CB_STDRPT_CASHBANK_AND_CASHBANKLN_BLKGRP_AND_BLK_GET"
        Dim strOpCd_CashBank_SubBlk_GET As String = "CB_STDRPT_CASHBANK_AND_CASHBANKLN_SUBBLK_GET"

        Dim WildStr As String = "AND CB.CashBankID *= CBLN.CashBankID AND "
        Dim NormStr As String = "AND CB.CashBankID = CBLN.CashBankID AND "
        ...
        If strBlkType = "BlkGrp" Then
            ...
            Else
                ...
                blkSearchStr = "OR ( CBLN.BlkCode = '' )"
            End If
            Session("SS_BLKGRPHEADER") = "BlkGrp"
            strOpCdCashBank = strOpCd_CashBank_BlkGrp_And_Blk_GET

        ElseIf strBlkType = "BlkCode" Then
            If Not Request.QueryString("BlkCode") = "" Then
                blkSelectStr = blkSelectStr & "CBLN.BlkCode LIKE '" & Request.QueryString("BlkCode") & "' AND "
            Else
                blkSelectStr = blkSelectStr & "CBLN.BlkCode LIKE '%' AND "
                blkSearchStr = "OR ( CBLN.BlkCode = '' )"
            End If
        ...
        ElseIf strBlkType = "SubBlkCode" Then
            If Not Request.QueryString("SubBlkCode") = "" Then
                blkSelectStr = blkSelectStr & "CBLN.BlkCode LIKE '" & Request.QueryString("SubBlkCode") & "' AND "
            Else
                blkSelectStr = blkSelectStr & "CBLN.BlkCode LIKE '%' AND "
                blkSearchStr = "OR ( CBLN.BlkCode = '' )"
            End If
            Session("SS_SUBBLKHEADER") = "SubBlkCode"
            strOpCdCashBank = strOpCd_CashBank_SubBlk_GET
        End If

        If Not (Request.QueryString("CashBankIDFrom") = "" And Request.QueryString("CashBankIDTo") = "") Then
            SearchStr = SearchStr & "CB.CashBankID IN (SELECT SUBCB.CashBankID FROM CB_CASHBANK SUBCB WHERE SUBCB.CashBankID >= '" & Request.QueryString("CashBankIDFrom") & _
                        "' AND SUBCB.CashBankID <= '" & Request.QueryString("CashBankIDTo") & "') AND "
        End If

        'Add Date Selection
        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = SearchStr & "CB.CreateDate BETWEEN '" & Request.QueryString("DateFrom") & "' AND '" & Request.QueryString("DateTo") & "' AND "
        End If

        If Not Request.QueryString("TransType") = objCBTrx.EnumCashBankType.All Then
            SearchStr = SearchStr & "CB.CashBankType = '" & Request.QueryString("TransType") & "' AND "
        Else
            SearchStr = SearchStr & "CB.CashBankType LIKE '%' AND "
        End If

        If Not Request.QueryString("UpdatedBy") = "" Then
            SearchStr = SearchStr & "CB.UpdateID LIKE '" & Request.QueryString("UpdatedBy") & "' AND "
        Else
            SearchStr = SearchStr & "CB.UpdateID LIKE '%' AND "
        End If

        If Not Request.QueryString("AccCode") = "" Then
            SearchStr = SearchStr & "CBLN.AccCode LIKE '" & Request.QueryString("AccCode") & "' AND "
        Else
            SearchStr = SearchStr & "CBLN.AccCode LIKE '%' AND "
        End If
        ...
        If Request.QueryString("VehCode") = "" And Request.QueryString("VehTypeCode") = "" Then
            vehSearchStr = "OR ( CBLN.VehCode = '' )"
        End If

        If Not Request.QueryString("VehCode") = "" Then
            SearchStr = SearchStr & "CBLN.VehCode LIKE '" & Request.QueryString("VehCode") & "' AND "
        Else
            SearchStr = SearchStr & "CBLN.VehCode LIKE '%' AND "
        End If

        If Not Request.QueryString("VehExpCode") = "" Then
            SearchStr = SearchStr & "CBLN.VehExpCode LIKE '" & Request.QueryString("VehExpCode") & "' AND "
        Else
            SearchStr = SearchStr & "CBLN.VehExpCode LIKE '%' AND "
        End If

        If Not Request.QueryString("Status") = "" Then
            If Not Request.QueryString("Status") = objCBTrx.EnumCashBankStatus.All Then
                SearchStr = SearchStr & "CB.Status = '" & Request.QueryString("Status") & "' AND "
            Else
                SearchStr = SearchStr & "CB.Status IN('1', '2', '5')  AND "
            End If
        End If

        If Request.QueryString("Status") = objCBTrx.EnumCashBankStatus.Verified Then
            If Request.QueryString("VerifiedBy") = "ACC" Then
                SearchStr = SearchStr & " Left(CB.CashBankID,3) = 'XXX'  AND "
            Else
                SearchStr = SearchStr & " Left(CB.CashBankID,3) <> 'XXX'  AND "
            End If
        End If
        ...
        If Request.QueryString("PrintFor") = "LIST" Then
            strFileName = "CB_StdRpt_CashBankList"
        Else
            strFileName = "CB_StdRpt_CashBankListSerahTerima"
            If Request.QueryString("PrintFor") = "PJK" Then
                'SearchStr = SearchStr & "AND CBLN.AccCode IN ('33.3.01.003','33.3.01.007')  "
                SearchStr = SearchStr & "AND CBLN.AccCode IN (SELECT AccCode FROM TX_TaxObjectRate WHERE Status='1' AND ExpiredStatus='0')  "
                strParam = strUserLoc & "|" & strAccMonth & "|" & strAccYear & "|" & vehSelectStr & "|" & vehSearchStr & "|" & blkSelectStr & "|" & blkSearchStr & "|" & NormStr & SearchStr
            ElseIf Request.QueryString("PrintFor") = "FIN" Then
                SearchStr = SearchStr & "AND CB.CashBankID NOT IN (SELECT CashBankID FROM CB_CashBankLn WHERE AccCode IN (SELECT AccCode FROM TX_TaxObjectRate WHERE Status='1' AND ExpiredStatus='0'))"
                strParam = strUserLoc & "|" & strAccMonth & "|" & strAccYear & "|" & vehSelectStr & "|" & vehSearchStr & "|" & blkSelectStr & "|" & blkSearchStr & "|" & NormStr & SearchStr
            End If
        End If

        Try
            intErrNo = objCB.mtdGetReport_CashBankList(strOpCdCashBank, strParam, objRptDs, objMapPath, objFTPFolder)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_CASHBANK_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
    ...
    Sub PassParam()
        ...
        paramField9 = paramFields.Item("ParamCashBankIDFrom")
        paramField10 = paramFields.Item("ParamCashBankIDTo")
        paramField11 = paramFields.Item("ParamTransType")
        paramField12 = paramFields.Item("ParamUpdatedBy")
        paramField13 = paramFields.Item("ParamStatus")
        paramField14 = paramFields.Item("ParamAccCode")
        paramField15 = paramFields.Item("ParamVehCode")
        paramField16 = paramFields.Item("ParamVehExpCode")
        paramField17 = paramFields.Item("ParamBlkCode")
        paramField18 = paramFields.Item("ParamSubBlkCode")
        paramField19 = paramFields.Item("ParamBlkOrSubBlk")
        ...
        paramField25 = paramFields.Item("lblLocation")
        paramField26 = paramFields.Item("ParamBlkType")
        paramField27 = paramFields.Item("ParamBlkGrp")
        paramField28 = paramFields.Item("lblBlkGrp")
        paramField29 = paramFields.Item("lblVehTypeCode")
        paramField30 = paramFields.Item("ParamVehTypeCode")
        paramField31 = paramFields.Item("ParamInterEstateEnabled")
        ...
        ParamDiscreteValue9.Value = Session("SS_CASHBANKIDFROM")
        ParamDiscreteValue10.Value = Session("SS_CASHBANKIDTO")
        ParamDiscreteValue11.Value = Session("SS_TRANSTYPE")
        ParamDiscreteValue12.Value = Session("SS_UPDATEDBY")
        ParamDiscreteValue13.Value = Session("SS_STATUS")
        ParamDiscreteValue14.Value = Session("SS_ACCCODE")
        ParamDiscreteValue15.Value = Session("SS_VEHCODE")
        ParamDiscreteValue16.Value = Session("SS_VEHEXPCODE")
        ParamDiscreteValue17.Value = Session("SS_BLKCODE")
        ParamDiscreteValue18.Value = Session("SS_SUBBLKCODE")

        If strBlkType = "BlkGrp" Then
            ParamDiscreteValue19.Value = Session("SS_BLKGRPHEADER")
        ElseIf strBlkType = "BlkCode" Then
            ParamDiscreteValue19.Value = Session("SS_BLKHEADER")
        ElseIf strBlkType = "SubBlkCode" Then
            ParamDiscreteValue19.Value = Session("SS_SUBBLKHEADER")
        End If
        ...
        ParamDiscreteValue25.Value = Session("SS_LBLLOCATION")
        ParamDiscreteValue26.Value = Session("SS_BLKTYPE")
        ParamDiscreteValue27.Value = Session("SS_BLKGRP")
        ParamDiscreteValue28.Value = Session("SS_LBLBLKGRP")
        ParamDiscreteValue29.Value = Session("SS_LBLVEHTYPECODE")
        ParamDiscreteValue30.Value = Session("SS_VEHTYPECODE")
