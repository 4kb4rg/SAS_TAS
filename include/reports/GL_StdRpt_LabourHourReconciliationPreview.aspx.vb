...
Public Class GL_StdRpt_LabourHourReconciliationPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        strRptPrefix = "GL_StdRpt_LabourHoursReconciliation"

        strOpCd = "GL_STDRPT_LABOUR_HOURS_RECONCILIATION_REPORT_GET_SP" & "|GL_LABHRSRECON"
        colOpCode.Add("GL_STDRPT_LABOUR_HOURS_RECONCILIATION_REPORT_GET_SP", "LABOUR_HOURS_RECON")  


        colParam.Add(strCompany, "COMPANY")
        colParam.Add(strLocation, "LOCATION")        
        colParam.Add(strUserId, "USERID")        
        colParam.Add("'" & strSelLocation & "'", "LOCCODE")
        colParam.Add(strSelAccMonth, "ACCMONTH")
        colParam.Add(strSelAccYear, "ACCYEAR")
                
        Try
            intErrNo = objGLRpt.mtdGetReport_LabourHoursRecon(colOpCode, _
                                                         colParam, _
                                                         dtReport, _
                                                         strMapPath, objFTPFolder)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GL_STDRPT_LABOUR_HOURS_RECONCILIATION_REPORT_GET_SP&errmesg=" & Exp.ToString() & "&redirect=")
        End Try
        ...
        strRptPrefix = strRptPrefix & GetRandomNum()
        ...
        For Each myTable In rdCrystalViewer.Database.Tables
            ...
            If myTable.Name = "GL_LABOURHRSRECON" Then
                myLogin = myTable.LogOnInfo
                Try
                    intErrNo = objGLRpt.mtdGet_DBConnObj(myLogin)
                Catch Exp As System.Exception
                    Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GL_LABOURHRSRECON_RPT_GET&errmesg=" & Exp.ToString() & "&redirect=")
                End Try

                myTable.ApplyLogOnInfo(myLogin)
                myTable.Location = myLogin.ConnectionInfo.DatabaseName & ".dbo.GL_LABOURHRSRECON_RPT"
            End If
    ...
    Private Function GetRandomNum() As String
        ...
        Randomize()
        intRandomNum = CInt(Int((10000 * Rnd()) + 1))
        Return "_" & Now.Year.ToString & Now.Month.ToString & Now.Day.ToString & "_" & intRandomNum.ToString
    End Function
