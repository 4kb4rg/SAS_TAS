...
Public Class PM_StdRpt_KernelQualityTransactionListPreview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR_GET As String = "PM_CLSRPT_KERNEL_QUALITY_GET"


        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = SearchStr & " AND (DateDiff(Day, '" & Session("SS_DATEFROM") & "', PMKQ.TransDate) >= 0) And (DateDiff(Day, '" & Session("SS_DATETO") & "', PMKQ.TransDate) <= 0) "
        End If

        If Not (strProcessingLnNo = "All") Then
            SearchStr = SearchStr & " AND PMKQ.ProcessLnNo='" & strProcessingLnNo & "' "
        End If

        strParam = strUserLoc & "|" & strAccMonth & "|" & strAccYear & "||" & SearchStr


        Try
            intErrNo = objPM.mtdGetKernelQuality(strOpCdPR_GET, strParam, objRptDs, objMapPath)

            objRptDs.Tables(0).TableName = "PM_StdRpt_KernelQualityTransactionListTitleBar"
            objRptDs.Tables(1).TableName = "PM_StdRpt_KernelQualityTransactionListMain"
            objRptDs.Tables(2).TableName = "PM_StdRpt_KernelQualityTransactionListDetail"
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_PM_OIL_Loss_GET_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try


        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\PM_StdRpt_KernelQualityTransactionList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\PM_StdRpt_KernelQualityTransactionList.pdf"
 	...
 	rdCrystalViewer.Close()
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/PM_StdRpt_KernelQualityTransactionList.pdf"">")
    End Sub
    ...
    Sub PassParam()
        ...
        paramField13 = paramFields.Item("ParamProcessingLnNo")
        paramField14 = paramFields.Item("ParamLblLocation")
        ...
        ParamDiscreteValue13.Value = strProcessingLnNo
        ParamDiscreteValue14.Value = strLblLocation
