...
Public Class PR_StdRpt_DaftarAbsensi : Inherits Page
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Response.Write("<Script Language=""JavaScript"">window.open(""PR_StdRpt_DaftarAbsensiPreview_Estate.aspx?CompCode=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                       ...
                       "&EmpType=" & strEmpType & "&Divisi=" & strDivisi & "&Mandor=" & strMandor & "&Pekerjaan=" & strPekerjaan & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
    End Sub
    ...
    Sub BindEmpType()
        ...
        If objDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objDs.Tables(0).Rows.Count - 1
                ...
                objDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objDs.Tables(0).Rows(intCnt).Item("EmpTyCode")) & " (" & Trim(objDs.Tables(0).Rows(intCnt).Item("Description")) & ")"
            Next
        End If
    ...
    Sub BindMandoran(ByVal strdivcode As String)
        ...
        Dim dr As DataRowDim strAM as String
        ...
        strParamValue = strLocation & "|" & strAM & "|" & ddlyear.SelectedItem.Value.Trim & "|AND A.Status='1' AND A.LocCode = '" & strLocation & "' AND C.IDDiv like '%" & strdivcode & "%' AND (C.isMandor='1')|ORDER BY A.EmpName"
