...
Public Class RPT_ClaimBenefit_Listing_Preview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strBenefit = Trim(Request.QueryString("strBenefit"))
    ...
    Sub BindReport()
        ...
        strOpCode = "REPORTS_CLAIM_BENEFIT"
        If strListingType = "1" Then
            strFileName = "RPT_ClaimBenefit_Listing_Airline"
        ElseIf strListingType = "2" Then
            strFileName = "RPT_ClaimBenefit_Listing_Provider"
        Else
            strFileName = "RPT_ClaimBenefit_Listing_Benefit"
        End If

        If strFlightNo = "" And strAirline = "" And strProvider = "" And strBenefit = "" Then
            strSearch = ""
        Else
            If strFlightNo <> "" Then
            ...
            ElseIf strBenefit <> "" Then
                strSearch = strSearch + " AND BenefitCode = '" & strBenefit & "'"
            End If
        End If

        strParamName = "SDATE|EDATE|AIRLINECODE|PROVIDERCODE"
        strParamValue = strDateFrom & "|" & strDateTo & "|" & strAirline & "|" & strProvider

        Try
            intErrNo = objGLRpt.mtdGetReportDataCommon(strOpCode, strParamName, strParamValue, objRptDs, objMapPath, objFTPFolder)

        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=RPT_ClaimBenefit_Listing&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        objRptDs.Tables(0).TableName = "RPT_CLAIM_BENEFIT"
        objRptDs.Tables(1).TableName = "RPT_CLAIM_BENEFITLN"
        objRptDs.Tables(2).TableName = "RPT_PROVIDER"
