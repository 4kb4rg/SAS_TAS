...
Public Class CB_STDRPT_SELECTION_CTRL : Inherits UserControl
    ...
    Sub BindReportNameList()
        ...
        strParam = " WHERE ReportType = '" & Convert.ToString(objGlobal.EnumStdRptType.CashAndBank) & "' AND Status = '" & Convert.ToString(objGlobal.EnumStdRptStatus.Active) & "'   AND Reportid IN (Select ReportID From SH_REPORT_USER Where UserId='" & strUserId & "') ORDER BY RptName"
        Try
    ...
    Sub CheckRptName(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        ElseIf strSelectedIndex = "rptcb1000022" Then
            Response.Redirect("../../en/reports/CB_StdRpt_DailyGiro.aspx?SelIndex=" & intSelectedIndex & "&UserLoc=" & strUserLoc)
        ElseIf strSelectedIndex = "rptcb1000023" Then
            Response.Redirect("../../en/reports/CB_StdRpt_DailyGiroHandover.aspx?SelIndex=" & intSelectedIndex & "&UserLoc=" & strUserLoc)
        ElseIf strSelectedIndex = "rptcb1000024" Then
            Response.Redirect("../../en/reports/CB_StdRpt_GiroHistory.aspx?SelIndex=" & intSelectedIndex & "&UserLoc=" & strUserLoc)
