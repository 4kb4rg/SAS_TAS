...
Public Class PR_StdRpt_EAForm : Inherits Page
    ...
    Protected WithEvents txtSerialFrom As TextBox
    Protected WithEvents txtSerialTo As TextBox
    Protected WithEvents txtPeriodDateFrom As TextBox
    Protected WithEvents txtPeriodDateTo As TextBox
    ...
    Function BindAccYearList(ByVal pv_strLocation As String, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_EAFORM_ACCCFG_DIST_GET&errmesg=" & Exp.ToString() & "&redirect=")
        End Try
    ...
    Sub BindEmpCodeList()
        ...
        strParam = "|" & Convert.ToString(objHR.EnumEmpStatus.Active)

        Try
            intErrNo = objHR.mtdGetOthEmployee(strOpCd_Get, strParam, strLocation, objRptDs)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PR_STDRPT_EMPLOYEE_GET&errmesg=" & Exp.ToString() & "&redirect=../../en/reports/PR_StdRpt_Selection.aspx")
        End Try
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Dim strPeriodDateFrom As String
        Dim strPeriodDateTo as String
        ...
        If txtSerialFrom.Text = "" Then
            strSerialFrom = ""
        Else
            strSerialFrom = Trim(txtSerialFrom.Text)
        End If

        If txtSerialTo.Text = "" Then
            strSerialTo = ""
        Else
            strSerialTo = Trim(txtSerialTo.Text)
        End If
        ...
        strPeriodDateFrom = txtPeriodDateFrom.Text
        strPeriodDateTo = txtPeriodDateTo.Text
        ...
        If Not (strPeriodDateFrom = "" And strPeriodDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strPeriodDateFrom, objDateFormat, objPeriodDateFrom) = False And objGlobal.mtdValidInputDate(strDateSetting, strPeriodDateTo, objDateFormat, objPeriodDateTo) = False Then
                lblDate.Text = lblDate.Text & objDateFormat & "."
                lblDate.Visible = True
            End If
        End If

        Response.Write("<Script Language=""JavaScript"">window.open(""PR_StdRpt_EAFormPreview.aspx?Type=Print&CompName=" & strCompany & _
                       "&Location=" & strUserLoc & _
                       "&Decimal=" & strDec & _
                       "&EAYear=" & strEAYear & _
                       "&TaxBranch=" & strTaxBranch & _
                       "&EmpCodeFrom=" & strEmpCodeFrom & _
                       "&EmpCodeTo=" & strEmpCodeTo & _
                       "&GangCode=" & strGangCode & _
                       "&SerialFrom=" & strSerialFrom & _
                       "&SerialTo=" & strSerialTo & _
                       "&PeriodDateFrom=" & objPeriodDateFrom & _
                       "&PeriodDateTo=" & objPeriodDateTo & _
                       "&SignName=" & strSignName & _
                       "&SignDesignation=" & strSignDesignation & _
