...
Public Class HR_setup_EPFList : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "EPF.EPFCode"
            End If
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "HR_CLSSETUP_EPF_SEARCH"
        Dim strSrchEPFCode As String
        ...
        strSrchEPFCode = IIF(txtEPFCode.Text = "", "", txtEPFCode.Text)
        ...
        Try
            intErrNo = objHRSetup.mtdGetEPF(strOpCd_GET, strParam, objEPFDs, False)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_EPF_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        If objEPFDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objEPFDs.Tables(0).Rows.Count - 1
                objEPFDs.Tables(0).Rows(intCnt).Item("EPFCode") = Trim(objEPFDs.Tables(0).Rows(intCnt).Item("EPFCode"))
                objEPFDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objEPFDs.Tables(0).Rows(intCnt).Item("Description"))
                objEPFDs.Tables(0).Rows(intCnt).Item("Status") = Trim(objEPFDs.Tables(0).Rows(intCnt).Item("Status"))
                objEPFDs.Tables(0).Rows(intCnt).Item("UpdateID") = Trim(objEPFDs.Tables(0).Rows(intCnt).Item("UpdateID"))
                objEPFDs.Tables(0).Rows(intCnt).Item("UserName") = Trim(objEPFDs.Tables(0).Rows(intCnt).Item("UserName"))
            Next
        End If
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        Dim strOpCd As String = "HR_CLSSETUP_EPF_STATUS_UPD"
        Dim strParam As String = ""
        Dim CPCell As TableCell = e.Item.Cells(0)
        Dim strSelectedEPFCode As String 
        Dim intErrNo As Integer

        strSelectedEPFCode = CPCell.Text
        ...
        Try
            intErrNo = objHRSetup.mtdUpdEPF(strOpCd, _
                                            ...
                                            True)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_EPF_DEL&errmesg=" & lblErrMessage.Text & "&redirect=hr/setup/HR_setup_EPFList.aspx")
        End Try
    ...
    Sub NewEPFBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("HR_setup_EPFdet.aspx")
    End Sub
