...
Public Class cb_trx_PayDet : Inherits Page
    ...
    Sub AddBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        IF ddlBank.SelectedItem.Value="" Then
            UserMsgBox(Me,"Please Select Bank")
            ddlBank.focus()
            exit sub
        End if
    ...
    Sub MenuLink_Click(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)

        If E.CommandName.ToString = "Item" Then
            Dim lbl As Label
            ...
            lbl = dgLineDet.Items.Item(intIndex).FindControl("lblDocId")
            lblDocType = dgLineDet.Items.Item(intIndex).FindControl("lblEnumDocType")

            Select Case lblDocType.Text.Trim
                Case "1"
                    If Left(lbl.Text.Trim, 3) = "INV" Then
                        Response.Write("<Script Language=""JavaScript"">pop_Att=window.open(""../../AP/trx/ap_trx_invrcv_wm_det.aspx?redirect=attm&trxID=" & lbl.Text.Trim & """, null ,""'pop_Att',width=1000,height=500,top=100,left=100,status=yes, resizable=no, scrollbars=yes, toolbar=no, location=no"");pop_Att.focus();</Script>")
                    Else
                        Response.Write("<Script Language=""JavaScript"">pop_Att=window.open(""../../AP/trx/ap_trx_InvRcvDet.aspx?redirect=attm&inrid=" & lbl.Text.Trim & """, null ,""'pop_Att',width=1000,height=500,top=100,left=100,status=yes, resizable=no, scrollbars=yes, toolbar=no, location=no"");pop_Att.focus();</Script>")
                    End If
                Case "2"
                    Response.Write("<Script Language=""JavaScript"">pop_Att=window.open(""../../AP/trx/ap_trx_DNDet.aspx?redirect=attm&dbnid=" & lbl.Text.Trim & """, null ,""'pop_Att',width=1000,height=500,top=100,left=100,status=yes, resizable=no, scrollbars=yes, toolbar=no, location=no"");pop_Att.focus();</Script>")
                Case "3"
                    Response.Write("<Script Language=""JavaScript"">pop_Att=window.open(""../../AP/trx/ap_trx_CNDet.aspx?redirect=attm&cnid=" & lbl.Text.Trim & """, null ,""'pop_Att',width=1000,height=500,top=100,left=100,status=yes, resizable=no, scrollbars=yes, toolbar=no, location=no"");pop_Att.focus();</Script>")
                Case "6"
    ...
    Sub ConfirmBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        sSQLKriteria = "Select Status From CB_Payment Where PaymentID='" & lblPaymentID.text.trim & "' And Status='9'"
        strParamName = "STRSEARCH"
        ...
        If strPaymentStatus = "9" Then
            strParamName = "STRSEARCH"
            strParamValue = "UPDATE CB_PAYMENT Set Status='2' Where PaymentId='" & lblPaymentID.text & "' And LocCode='" & strLocation & "' "
    ...
    Sub EditBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        sSQLKriteria = "Select Status From CB_Payment Where PaymentID='" & lblPaymentID.text.trim & "' And Status='2'"
        strParamName = "STRSEARCH"
        strParamValue = sSQLKriteria
        ...
        If strPaymentStatus = "2" And intLevel >= 3 Then
            strParamName = "STRSEARCH"
            strParamValue = "UPDATE CB_PAYMENT Set Status='1' Where PaymentId='" & lblPaymentID.text & "' And LocCode='" & strLocation & "' "
