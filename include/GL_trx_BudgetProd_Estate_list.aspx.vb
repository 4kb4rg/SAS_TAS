...
Public Class GL_trx_BudgetProd_Estate_list : Inherits Page
    ...
    Protected WithEvents txtGroupCoa As TextBox
	...
	Sub OnDataBound(ByVal sender As Object, ByVal e As DataGridItemEventArgs)
        Const target = "GL_trx_BudgetProd_Estate_Det.aspx?AccMonth={0}&GroupCoa={1}"
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                ...
                lb = CType(e.Item.FindControl("lblAccYear"), Label)
                ps = lb.Text.Trim()

                lb = CType(e.Item.FindControl("lblGroupCoa"), Label)
                pe = lb.Text.Trim()

                Dim row = CType(e.Item.DataItem, DataRowView)
                If ctrl IsNot Nothing Then

                    Dim rw_ps = row("AccYear")
                    Dim rw_pe = row("GroupCoa")
                    ctrl.NavigateUrl = String.Format(target, rw_ps, rw_pe)
                End If
        End Select
    End Sub
    ...
    Protected Function LoadData() As DataSet
        ...
        Dim strOpCd As String = "GL_CLSTRX_BUDGET_PROD_ESTATE_SEARCH"
        ...
        strGroupCoa = IIf(Trim(txtGroupCoa.Text) = "", "", " And GroupCoa = '" & txtGroupCoa.Text & "'")

        strSearch = " WHERE LOCCODE = '" & strLocation & "'" & strSrchAccYear & strGroupCoa
        ...
        For intCnt = 0 To dsResult.Tables(0).Rows.Count - 1
            ...
            dsResult.Tables(0).Rows(intCnt).Item("GroupCoa") = Trim(dsResult.Tables(0).Rows(intCnt).Item("GroupCoa"))
        Next
    ...
    Sub NewBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("GL_trx_BudgetProd_Estate_Det.aspx")
    End Sub
