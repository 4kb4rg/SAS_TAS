...
Public Class PR_StdRpt_GajiBesar_Harian_Estate : Inherits Page
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
		...
		strAM = month(Date_Validation(txtDateStart.Text.Trim, False))
		IF len(Trim(strAM))  = 1 Then
		strAM = "0" + strAM 
		end if
		strAY = year(Date_Validation(txtDateStart.Text.Trim, False))
        ...
        strEmpType = ddlemptype.SelectedItem.Value.Trim()
        ...
        strDateStart = Date_Validation(txtDateStart.Text.Trim, False)
		strDateEnd = Date_Validation(txtDateEnd.Text.Trim, False)

        Response.Write("<Script Language=""JavaScript"">window.open(""PR_StdRpt_GajiBesar_Harian_Preview_Estate.aspx?CompCode=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                       "&lblLocation=" & lblLocation.Text & "&AccMonth=" & strAM & "&AccYear=" & strAY & _
                       "&EmpCode=" & strEmpCode & _
                       "&Divisi=" & strDivisi & _
                       "&Mandor=" & strMandor & _
                       "&EmpType=" & strEmpType & _
					   "&DateStart=" & strDateStart & _
					   "&DateEnd=" & strDateEnd & _
    ...
    Sub BindEmployee(ByVal strdivcode As String, ByVal MandorCode As String)
        ...
        Else
            tmpParam = ""
        End If
        strParamValue = strLocation & "|" & strAM & "|" & strAY & "|" & tmpParam & " AND A.Status='1' AND A.LocCode = '" & strLocation & "' AND C.IDDiv like '%" & strdivcode & "%' |ORDER BY A.EmpName"
