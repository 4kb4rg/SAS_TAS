...
Public Class NU_trx_SeedReceiveList : Inherits Page
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDRCV_GET_LANGCAP&errmesg=" & Exp.ToString() & "&redirect=NU/Trx/NU_trx_SeedReceiveList.aspx")
        End Try
    ...
    Sub BindSearchList()
        srchStatusList.Items.Add(New ListItem(objNUTrx.mtdGetSeedReceiveStatus(objNUTrx.EnumSeedReceiveStatus.All), objNUTrx.EnumSeedReceiveStatus.All))
        srchStatusList.Items.Add(New ListItem(objNUTrx.mtdGetSeedReceiveStatus(objNUTrx.EnumSeedReceiveStatus.Closed), objNUTrx.EnumSeedReceiveStatus.Closed))
        srchStatusList.Items.Add(New ListItem(objNUTrx.mtdGetSeedReceiveStatus(objNUTrx.EnumSeedReceiveStatus.Confirmed), objNUTrx.EnumSeedReceiveStatus.Confirmed))
        srchStatusList.Items.Add(New ListItem(objNUTrx.mtdGetSeedReceiveStatus(objNUTrx.EnumSeedReceiveStatus.Deleted), objNUTrx.EnumSeedReceiveStatus.Deleted))
    ...
    Protected Function LoadData() As DataSet
        Dim strOppCode_Get As String = "NU_CLSTRX_SEEDRECEIVE_GET" '"NU_CLSTRX_SEEDRECEIVE_GET_SR" '"NU_CLSTRX_SEEDRECEIVE_GET"
        ...
        '                                          objTxDs)
        'Catch Exp As System.Exception
        '    Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_CLSTRX_SEEDRECEIVELIST_GET&errmesg=" & Exp.ToString() & "&redirect=NU/trx/NU_trx_SeedReceiveList.aspx")
        'End Try
		
		strParamName = "LOCCODE|ACCMONTH|ACCYEAR|SEARCHSTR"
        strParamValue = strlocation + "|" + strAccMonth + "|" + strAccYear + "| AND SR.ReceiveID is Not NULL"

        Try
            intErrNo = Objok.mtdGetDataCommon(strOppCode_Get, strParamName, strParamValue, objTxDs)
        Catch Exp As System.Exception
        ...
        For intCnt = 0 To objTxDs.Tables(0).Rows.Count - 1
            objTxDs.Tables(0).Rows(intCnt).Item("ReceiveID") = objTxDs.Tables(0).Rows(intCnt).Item("ReceiveID").Trim()
    ...
    Sub NewBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("NU_trx_SeedReceiveDetails.aspx")
        'Response.Redirect("../../IN/Trx/IN_Trx_StockReceive_Details.aspx?strModule=IN")
    End Sub
