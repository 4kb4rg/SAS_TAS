...
Public Class HR_setup_SocsoList : Inherits Page
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
           If SortExpression.Text = "" Then
                SortExpression.Text = "S.SocsoCode"
            End If
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "HR_CLSSETUP_SOCSO_SEARCH"
        Dim strSrchSocsoCode As String
        ...
        strSrchSocsoCode = IIF(txtSocsoCode.Text = "", "", txtSocsoCode.Text)
        ...
        strParam = strSrchSocsoCode & "|" & _
                   ...
                   SortCol.Text & "|"
        Try
            intErrNo = objHRSetup.mtdGetSocso(strOpCd_GET, strParam, objSocsoDs, False)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_SOCSO_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        If objSocsoDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objSocsoDs.Tables(0).Rows.Count - 1
                objSocsoDs.Tables(0).Rows(intCnt).Item("SocsoCode") = Trim(objSocsoDs.Tables(0).Rows(intCnt).Item("SocsoCode"))
                objSocsoDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objSocsoDs.Tables(0).Rows(intCnt).Item("Description"))
                objSocsoDs.Tables(0).Rows(intCnt).Item("Status") = Trim(objSocsoDs.Tables(0).Rows(intCnt).Item("Status"))
                objSocsoDs.Tables(0).Rows(intCnt).Item("UpdateID") = Trim(objSocsoDs.Tables(0).Rows(intCnt).Item("UpdateID"))
                objSocsoDs.Tables(0).Rows(intCnt).Item("UserName") = Trim(objSocsoDs.Tables(0).Rows(intCnt).Item("UserName"))
            Next
        End If
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        Dim strOpCd As String = "HR_CLSSETUP_SOCSO_STATUS_UPD"
        Dim strParam As String = ""
        Dim CPCell As TableCell = e.Item.Cells(0)
        Dim strSelectedSocsoCode As String 
        Dim intErrNo As Integer

        strSelectedSocsoCode = CPCell.Text
        ...
        Try
            intErrNo = objHRSetup.mtdUpdSocso(strOpCd, _
                                              ...
                                              True)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_SOCSO_DEL&errmesg=" & lblErrMessage.Text & "&redirect=hr/setup/HR_setup_SocsoList.aspx")
        End Try
    ...
    Sub NewSocsoBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("HR_setup_SocsoDet.aspx")
    End Sub
