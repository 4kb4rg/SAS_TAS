...
Public Class PR_StdRpt_AveEarningListPreview : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strStatusText = Request.QueryString("StatusText")

        Session("SS_LBLLOCATION") = Request.QueryString("lblLocation")
        Session("SS_ACCMTH") = strSelAccMonth
        Session("SS_ACCYR") = strSelAccYear
        Session("SS_DECIMAL") = strSelDecimal
        Session("SS_EMPIDFROM") = strFromEmp
        Session("SS_EMPIDTO") = strToEmp

        Session("SS_STATUS") = strStatus
        
        strSelLocTag = UCase(Session("SS_LBLLOCATION"))
        strLocTag = Session("SS_LBLLOCATION")
    ...
    Sub BindReport()
        ...
        Try
            strRptPrefix = "PR_StdRpt_AveEarningList"

            strOpCd = "PR_STDRPT_AVEEARNINGLIST_EMP_GET_SP" & "|" & objPRRpt.mtdGetPRReportTable(objPRRpt.EnumPRReportTable.AveEarning) & Chr(9) & _
                        "GET_REPORT_INFO_BY_REPORTID" & "|" & "SH_REPORT"

            strParam = strSelAccMonth & "|" & _
                       strSelAccYear & "|" & _
                       Replace(strUserLoc, "'", "''") & "|" & _
            ...
            intErrNo = objPRRpt.mtdGetReport_AveEarning_SP(strOpCd, _
                                                            strCompany, _
                                                            strLocation, _
                                                            strUserId, _
                                                            strAccMonth, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=PR_STDRPT_AVEEARNINGLIST_GET&errmesg=" & Exp.ToString() & "&redirect=")
        End Try

        For intCnt=0 To objRptDs.Tables("PR_AVEEARNINGLIST").Rows.Count - 1
            objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("EmpCode") = Trim(objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("EmpCode"))
            objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("EmpName") = Trim(objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("EmpName"))
            objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("LocCode") = Trim(objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("LocCode"))
            objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("TerminateDate") = Trim(objRptDs.Tables("PR_AVEEARNINGLIST").Rows(intCnt).Item("TerminateDate"))
        Next
    ...
    Sub PassParam()
        ...
        Session("SS_LOC") = Replace(Session("SS_LOC"), "','", ", ")

        ParamDiscreteValue1.Value = Session("SS_LOC")
        ParamDiscreteValue2.Value = Session("SS_AccMth")
        ParamDiscreteValue3.Value = Session("SS_ACCYR")
        ParamDiscreteValue4.Value = Session("SS_DECIMAL")
        ParamDiscreteValue5.Value = Session("SS_COMPANYNAME")
        ParamDiscreteValue6.Value = Session("SS_LOCATIONNAME")
        ParamDiscreteValue7.Value = Session("SS_USERNAME")
        ParamDiscreteValue8.Value = strSelLocTag
        ParamDiscreteValue9.Value = strLocTag
        ParamDiscreteValue10.Value = Session("SS_EMPIDFROM")
        ParamDiscreteValue11.Value = Session("SS_EMPIDTO")
        ...
        ParamFieldDef10 = ParamFieldDefs.Item("SrchEmpIDFrom")
        ParamFieldDef11 = ParamFieldDefs.Item("SrchEmpIDTo")
