...
Public Class GL_trx_BudgetPupuk : Inherits Page
    ...
    Dim strOppCd_GET As String = "GL_GL_TRX_BUDGET_PUPUK_GET"
    Dim strOppCd_ADD As String = "GL_GL_TRX_BUDGET_PUPUK_ADD"
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr = " WHERE AccYear='" & lstAccYear.SelectedItem.Value.trim() & "' "  
		
        If Not srchVehActCd.Text = "" Then
            SearchStr = SearchStr & " AND A.ItemCode like '%" & srchVehActCd.Text & "%'"
        End If
        If Not srchDescription.Text = "" Then
            SearchStr = SearchStr & " AND c.description like '%" & srchDescription.Text & "%'"
        End If
		
        SearchStr = SearchStr & " And A.LocCode= '" & strLocation & "' AND CodeBlkGrp like '" & srchDiv.SelectedItem.Value.Trim() & "%' AND CodeBlk like '" & srchTTnm.SelectedItem.Value.Trim() & "%'"

        ParamNama = "LOC|SEARCH|SORT"
        ParamValue = strLocation & "|" & SearchStr & "|Order by CodeBlkGrp,CodeBlk,A.itemcode " 
    ...
    Sub DEDR_Update(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
		...
		Dim strkg As String
		Dim strkgha As String
		
		lblgrd = E.Item.FindControl("PStart")
		PS = lblgrd.Text.Trim 
		...
		txtEditText = E.Item.FindControl("txtrotasi")
		strrot = txtEditText.Text.Trim 
		
		txtEditText = E.Item.FindControl("txtha")
		strha = txtEditText.Text.Trim 
		
		txtEditText = E.Item.FindControl("txtpokok")
		strpokok = txtEditText.Text.Trim 
		
		txtEditText = E.Item.FindControl("txtkg")
		strkg = txtEditText.Text.Trim 
		
		if strha="0" Then
		strkgha = "0"
		else
		strkgha = Math.Round(CSng(strkg) / CSng(strha), 2) 
		end if
		
		
			
		
		ParamName = "LOC|YR|DV|TT|IC|ROT|HA|PKK|KG|KGHA"
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
		...
		Dim strOppCd As String = "GL_GL_TRX_BUDGET_PUPUK_DEL"
        
        EventData.EditItemIndex = CInt(e.Item.ItemIndex)
        BindGrid()
		
		
        txtLabel = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("PStart")
    ...
    Sub DEDR_Add(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
	    ...
	    if  (srchDiv.SelectedItem.Value.Trim="" or srchTTnm.SelectedItem.Value.Trim = "")
		    lblErrMessage.text = "Silakan pilih divisi dan tahun tanam"
		...
		newRow.Item("AccYear") = lstAccYear.SelectedItem.Value.Trim
	...
	Sub BindItemDropList()
		...
		filter = "AND (ProdTypeCode in ('3110')) "           
