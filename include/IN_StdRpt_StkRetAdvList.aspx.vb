...
Public Class IN_StdRpt_StkRetAdvList : Inherits Page
    ...
    Sub BindStatus()
        ...
        lstStatus.Items.Add(New ListItem(objINTrx.mtdGetStockRetAdvStatus(objINTrx.EnumStockRetAdvStatus.Closed), objINTrx.EnumStockRetAdvStatus.Closed))
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        strDocNoFrom = Server.UrlEncode(strDocNoFrom)
        strDocNoTo = Server.UrlEncode(strDocNoTo)
        ...
        If Not (strDateFrom = "" And strDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDateFrom, objDateFormat, objDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDateTo, objDateFormat, objDateTo) = True Then
                Response.Write("<Script Language=""JavaScript"">window.open(""IN_StdRpt_StkRetAdvListPreview.aspx?Type=Print&CompName=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & _
                               "&Decimal=" & strDec & "&DateFrom=" & objDateFrom & "&DateTo=" & objDateTo & "&DDLAccMth=" & strddlAccMth & "&DDLAccYr=" & strddlAccYr & "&lblProdTypeCode=" & lblProdTypeCode.Text & _
                               "&lblProdBrandCode=" & lblProdBrandCode.Text & "&lblProdModelCode=" & lblProdModelCode.Text & "&lblProdCatCode=" & lblProdCatCode.Text & "&lblProdMatCode=" & lblProdMatCode.Text & _
                               "&lblStkAnaCode=" & lblStkAnaCode.Text & "&lblLocation=" & lblLocation.Text & "&DocNoFrom=" & strDocNoFrom & "&DocNoTo=" & strDocNoTo & "&ProdType=" & strProdType & "&ProdBrand=" & strProdBrand & _
                               "&ProdModel=" & strProdModel & "&ProdCat=" & strProdCat & "&ProdMat=" & strProdMat & "&StkAna=" & strStkAna & "&ItemCode=" & strItemCode & "&Status=" & strStatus & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
            Else
