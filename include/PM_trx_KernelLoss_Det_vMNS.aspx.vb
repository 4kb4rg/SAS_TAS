...
Public Class PM_KernelLoss_Det : Inherits Page
    ...
    Dim strOppCd_KernelLoss_GET As String = "PM_CLSTRX_KERNELLOSS_GET"
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=PM_CLSTRX_KERNEL_LOSS_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=PM/trx/pm_trx_KernelLoss_list.aspx")
        End Try
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objPMTrx.mtdGetOilLoss(strOppCd_KernelLoss_GET, strLocation, strParam, objDataSet)
        Catch Exp As System.Exception
    ...
    Sub DisplayData()
        Dim I As Integer
        Dim dsKernelLoss As DataSet = LoadData()

        If dsKernelLoss.Tables(0).Rows.Count > 0 Then
            
            txtdate.Text = objGlobal.GetShortDate(strDateFormat, Trim(dsKernelLoss.Tables(0).Rows(0).Item("TransDate")))

            Try
                ddlTestSampleCode.SelectedValue = dsKernelLoss.Tables(0).Rows(0).Item("TestSampleCode")
            Catch Exp As System.Exception
                ddlTestSampleCode.SelectedValue = ""
            End Try

            Try
                ddlStation.SelectedValue = dsKernelLoss.Tables(0).Rows(0).Item("StationCode")
            Catch Exp As System.Exception
                ddlStation.SelectedValue = ""
            End Try


            lblPeriod.Text = Trim(dsKernelLoss.Tables(0).Rows(0).Item("AccMonth")) & "/" & Trim(dsKernelLoss.Tables(0).Rows(0).Item("AccYear"))
            lblStatus.Text = objPMTrx.mtdGetKernelLossStatus(Trim(dsKernelLoss.Tables(0).Rows(0).Item("Status")))
            lblCreateDate.Text = objGlobal.GetLongDate(Trim(dsKernelLoss.Tables(0).Rows(0).Item("CreateDate")))
            lblLastUpdate.Text = objGlobal.GetLongDate(Trim(dsKernelLoss.Tables(0).Rows(0).Item("UpdateDate")))
            lblUpdateBy.Text = Trim(dsKernelLoss.Tables(0).Rows(0).Item("Username"))
                        
            PopulateControls(dsKernelLoss)

            DisableControl(Trim(dsKernelLoss.Tables(0).Rows(0).Item("Status")))
    ...
    Sub UpdateData(ByVal strAction As String)
        ...
        Dim strOppCd_KernelLoss_ADD As String = "PM_CLSTRX_KERNELLOSS_ADD"
        Dim strOppCd_KernelLoss_UPD As String = "PM_CLSTRX_KERNEL_LOSS_UPD"
        Dim strOppCd_Dtl_ADD As String = "PM_CLSTRX_KERNELLOSS_DETAIL_ADD"
        Dim strOppCd_Dtl_UPD As String = "PM_CLSTRX_KERNEL_LOSS_DETAIL_UPD"
        Dim strOpCodeDelDtl As String = "PM_CLSTRX_KERNELLOSS_DETAIL_DEL"
        ...
        Dim strTrxID As String = "KernelID" 
        ...
        strParam =  lblId.Text & "|" & _
                    ...
                    ddlMachine.SelectedValue & "|" &  objPMTrx.EnumKernelLossStatus.Active
                    
        Try

            intErrNo = objPMTrx.mtdUpdOilLoss(strOppCd_KernelLoss_ADD, _
                                              strOppCd_KernelLoss_UPD, _
                                              strOppCd_Dtl_ADD, _
                                              strOppCd_Dtl_UPD, _
                                              strOpCodeDelDtl, _
                                              strLocation, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PM_CLSTRX_KERNEL_LOSS_UPDATE&errmesg=" & lblErrMessage.Text & "&redirect=PM/trx/pm_trx_KernelLoss_list.aspx")
        End Try
    ...
    Sub btnDelete_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Dim strOppCd_DEL As String = "PM_CLSTRX_KERNELLOSS_DEL"
        ...
        Try
            intErrNo = objPMTrx.mtdPhysicalDelete(strOppCd_DEL, _
                                                  strParam, objPMTrx.EnumTransType.KernelLoss)

        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PM_CLSTRX_KERNEL_LOSS_DEL&errmesg=" & lblErrMessage.Text & "&redirect=PM/Trx/PM_trx_KernelLoss_List.aspx")
        End Try
    ...
    Sub BindStation(Byval vstrValue As String)
        ...
        Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PM_CLSTRX_KERNEL_LOSS_BINDSTATION&errmesg=" & lblErrMessage.Text & "&redirect=PM/Trx/PM_trx_KernelLoss_List.aspx")
        End Try
    ...
    Sub BindMachine(Byval vstrValue As String, Optional Byval vstrStation As String = "")
        ...
        If vstrStation <> "" Then
            ...
            Try
                intErrNo = objPMTrx.mtdGetTransactionList(strOpCd, strParam, dsList)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PM_CLSTRX_KERNEL_LOSS_BINDMACHINE&errmesg=" & lblErrMessage.Text & "&redirect=PM/Trx/PM_trx_KernelLoss_List.aspx")
            End Try
    ...
    Private Sub PopulateTemplControls(ByVal vstrMachine As String) 
           ...
           Dim hrow As HtmlTableRow
            ...
            Try
                intErrNo = objPMTrx.mtdGetTransactionList(strOpCd, strParam, dsList)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=PM_CLSTRX_KERNEL_LOSS_MACHINETEMPLATE_GET&errmesg=" & lblErrMessage.Text & "&redirect=PM/Trx/PM_trx_KernelLoss_List.aspx")
            End Try
