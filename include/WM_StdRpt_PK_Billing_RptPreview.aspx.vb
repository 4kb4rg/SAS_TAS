...
Public Class WM_StdRpt_PKBillRpt_Preview : Inherits Page
    ...
    Sub BindReport()
        ...
        Dim dsPKBilling As New DataSet()
        Dim objMapPath As String

        Dim crExportOptions As ExportOptions
        Dim crDiskFileDestinationOptions As DiskFileDestinationOptions

        Dim strOpCd_PK_Billing_GET As String = "WM_STDRPT_CPO_BILLING_REPORT_GET"
        Dim strFileName As String = "WM_StdRpt_PK_Billing_Report"

        Dim strParam As String
        Dim searchStr As String

        If Not Trim(Request.QueryString("BillPartyCode")) = "" Then
            searchStr = "AND TIC.CustomerCode LIKE '" & Trim(Request.QueryString("BillPartyCode")) & "' "
        End If

        If Not (Request.QueryString("DelDateFrom") = "" And Request.QueryString("DelDateTo") = "") Then
            searchStr = searchStr & "AND (DateDiff(Day, '" & Request.QueryString("DelDateFrom") & "', TIC.OutDate) >= 0) And (DateDiff(Day, '" & Request.QueryString("DelDateTo") & "', TIC.OutDate) <= 0) "
        End If

        strParam = strUserLoc & "|" & Request.QueryString("strddlAccMth") & "|" & _
                   Request.QueryString("strddlAccYr") & "|" & _
                   objWMTrx.EnumWeighBridgeTicketTransType.Sales & "|" & _
                   objWMTrx.EnumWeighBridgeTicketStatus.Active & "|" & _
                   objWMTrx.EnumWeighBridgeTicketProduct.PK & "|" & _
                   searchStr
        Try
            intErrNo = objWM.mtdGetReport_CPOBilling_Report(strOpCd_PK_Billing_GET, _
                                                            strParam, _
                                                            dsPKBilling, _
                                                            objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=WM_STDRPT_PK_BILLING_REPORT_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try


        dsPKBilling.Tables(0).TableName = "WM_PK_BILLING_REPORT"


        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\" & strFileName & ".rpt", OpenReportMethod.OpenReportByTempCopy)
        rdCrystalViewer.SetDataSource(dsPKBilling.Tables("WM_PK_BILLING_REPORT"))
    ...
    Sub PassParam()
        ...
        ParamDiscreteValue10.Value = Request.QueryString("BillPartyCode")
