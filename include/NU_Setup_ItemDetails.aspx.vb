...
Public Class NU_NurseryDetails : Inherits Page

    Dim dsStockItem AS Dataset

    Protected objNU As New agri.NU.clsSetup()
    ...
    Protected WithEvents NUItemCode as textBox
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumNUAccessRights.NUItem), intNUAR) = False Then
   ...
   End Sub
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=NU_SETUP_NURSERY_ITEM_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=NU/Setup/NU_Setup_Item.aspx")
        End Try
    ...
    Protected Function LoadData(ByVal pv_blnIsMaster As Boolean) As DataSet
        strParam =  ItemCode & "|" & objNU.EnumInventoryItemType.NurseryItem & "|" & Trim(strLocation) 
        ...
        Try
            intErrNo = objNU.mtdGetMasterDetail(strOppCd_GET, strParam, objDataSet)
        Catch Exp As Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_NURSERYITEM_DETAILS&errmesg=" & lblErrMessage.Text & "&redirect=NU/Setup/NU_Setup_Item.aspx")
        End Try
        Return objDataSet
    End Function
    
    Sub DisableControl()
        NUItemCode.Enabled = False
        Desc.Enabled = False
    ...
    Sub DisplayData()
        ...
        NUItemCode.Text = Trim(dsStockItem.Tables(0).Rows(0).Item("ItemCode"))
        ...
        If objMasterDs.Tables(0).Rows.Count > 0 Then
            If objMasterDs.Tables(0).Rows(0).Item("Status") = objNU.EnumStockItemStatus.Deleted And _
                dsStockItem.Tables(0).Rows(0).Item("Status") = objNU.EnumStockItemStatus.Deleted Then
                Delete.Visible = False
            End If
        End If
    End Sub
    ...
    ByVal ModuleType As Integer, ByVal Status As Integer, ByVal BindToList As DropDownList)
        ...
        Try
            Select Case ModuleType
                ...
                Case objGlobal.EnumModule.Inventory
                    intErrNo = objNU.mtdGetMasterList(Oppcode, strParam, dsForDropDown)
            End Select
        Catch Exp As Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_NURSERYITEM_DETAILS&errmesg=" & lblErrMessage.Text & "&redirect=NU/Setup/NU_Setup_Item.aspx")
        End Try
        ...
        If SelectedIndex = -1 And Not ItemCode = "" Then
            ...
            Try
                intErrNo = objNU.mtdGetMasterList(Oppcode, strParam, dsForInactiveItem)
            ...
            Catch Exp As Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=ADD_ITEMTODROPDOWN&errmesg=" & lblErrMessage.Text & "&redirect=NU/Setup/NU_Setup_Item.aspx")
            End Try
    ...
    Sub BindItemCodeDropList()
        ...
        SearchStr = "AND ItemType in ('" & objIN.EnumInventoryItemType.NurseryItem & "') AND itm.Status = '" & objIN.EnumStockItemStatus.Active & "' AND Not ItemCode In (Select ItemCode From IN_Item where ItemType in ('" & objIN.EnumInventoryItemType.NurseryItem &  "') And LocCode='" & strLocation & "' ) "
    ...
    Sub UpdateData(ByVal strAction As String)
        ...
        Select Case strAction
            Case "Save"
                strStatus = IIf(Status.Text = objNU.mtdGetStockItemStatus(objNU.EnumStockItemStatus.Deleted), _
                                    objNU.EnumStockItemStatus.Deleted, _
                                    objNU.EnumStockItemStatus.Active)
            Case "Delete"
                strStatus = IIf(Status.Text = objNU.mtdGetStockItemStatus(objNU.EnumStockItemStatus.Active), _
                                    objNU.EnumStockItemStatus.Deleted, _
                                    objNU.EnumStockItemStatus.Active)
        ...
        Else
            Select Case blnUpdate.Text
                Case True
                    Response.Redirect("NU_Setup_Item.aspx")
                Case False
                    Response.Redirect("NU_Setup_ItemDetail.aspx")
            End Select
        End If
    End Sub
    ...
    Sub btnSynchronizeData_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        strParam = Trim(NUItemCode.Text) & "|" & _
                   Trim(objIN.EnumInventoryItemType.NurseryItem)
        Try
            intErrNo = objIN.mtdSynchronizeItemData(strOpCode_GetMaster, _
                                                    ...
                                                    strLocation)
        Catch Exp As Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=UPDATE_NURSERYITEM&errmesg=" & lblErrMessage.Text & "&redirect=NU/Setup/NU_Setup_Item.aspx")
        End Try
