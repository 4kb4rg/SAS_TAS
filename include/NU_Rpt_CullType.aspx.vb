...
Public Class NU_Rpt_CullType : Inherits Page
    ...
    Public Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strCullTypeCode = Trim(Request.QueryString("strCullTypeCode"))
        ...
        CullTypeCodeTag = Trim(Request.QueryString("CullTypeCodeTag"))
        DescriptionTag = Trim(Request.QueryString("DescriptionTag"))
        NurseryStageTag = Trim(Request.QueryString("NurseryStageTag"))
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd As String = "NU_CLSSETUP_CULLTYPE_LIST_GET"
        ...
        Try
            intErrNo = objNUSetup.mtdGetCullType(strOpCd, _
                                                   strParam, _
                                                   objRptDs)
        ...
        For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("CullTypeCode") = objRptDs.Tables(0).Rows(intCnt).Item("CullTypeCode").Trim()
            objRptDs.Tables(0).Rows(intCnt).Item("Description") = objRptDs.Tables(0).Rows(intCnt).Item("Description").Trim()
            objRptDs.Tables(0).Rows(intCnt).Item("NurseryStage") = objRptDs.Tables(0).Rows(intCnt).Item("NurseryStage").Trim()
            objRptDs.Tables(0).Rows(intCnt).Item("Username") = objRptDs.Tables(0).Rows(intCnt).Item("Username").Trim()
        Next

        For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("NurseryStage") = objNUSetup.mtdGetNurseryStage(objRptDs.Tables(0).Rows(intCnt).Item("NurseryStage"))
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objNUSetup.mtdGetCullTypeStatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
            objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate") = objGlobal.GetLongDate(objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate"))
        Next

        rdCrystalViewer.Load(objMapPath & "Web\EN\NU\Reports\Crystal\NU_Rpt_CullType.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\NU_Rpt_CullType.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/NU_Rpt_CullType.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        paramField4 = paramFields.Item("CullTypeCodeTag")
        paramField5 = paramFields.Item("DescriptionTag")
        paramField6 = paramFields.Item("NurseryStageTag")
        ...
        ParamDiscreteValue4.Value = CullTypeCodeTag
        ParamDiscreteValue5.Value = DescriptionTag
        ParamDiscreteValue6.Value = NurseryStageTag
