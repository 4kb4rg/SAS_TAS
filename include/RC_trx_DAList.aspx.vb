...
Public Class RC_trx_DAList : Inherits Page

    Protected WithEvents dgLine As DataGrid
    Protected WithEvents lblTracker As Label
    Protected WithEvents lstDropList As DropDownList
    Protected WithEvents ddlStatus As DropDownList
    Protected WithEvents txtDAID As TextBox
    Protected WithEvents txtDARefID As TextBox
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        Else
            ...
            If SortExpression.Text = "" Then
                SortExpression.Text = "DA.DispAdvID"
            End If
    ...
    Sub BindGrid() 
        ...
        Dim strOpCode_GetDAList As String = "RC_CLSTRX_DISPATCHADVICE_LIST_GET"
        Dim lbButton As LinkButton
        Dim strSrchDAID as string
        Dim strSrchDARefNo as string
        ...
        strSrchDAID = IIf(txtDAID.Text = "", "", txtDAID.Text)
        strSrchDARefNo = IIf(txtDARefID.Text = "", "", txtDARefID.Text)
        ...
        Try
            intErrNo = objRCTrx.mtdGetDispatchAdvice(strOpCode_GetDAList, _
                                                    strCompany, _
                                                    strLocation, _
                                                    strUserId, _
                                                    strAccMonth, _
                                                    ...
                                                    objRCDs)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=RC_DALIST_GET_LIST&errmesg=" & Exp.ToString() & "&redirect=")
        End Try

        For intCnt = 0 To objRCDs.Tables(0).Rows.Count - 1
            objRCDs.Tables(0).Rows(intCnt).Item("DispAdvID") = Trim(objRCDs.Tables(0).Rows(intCnt).Item("DispAdvID"))
            objRCDs.Tables(0).Rows(intCnt).Item("DispAdvRefNo") = Trim(objRCDs.Tables(0).Rows(intCnt).Item("DispAdvRefNo"))
        ...
        For intCnt = 0 To dgLine.Items.Count - 1
            Select Case CInt(objRCDs.Tables(0).Rows(intCnt).Item("Status"))
                ...
                Case objRCTrx.EnumDispatchAdviceStatus.Confirmed, objRCTrx.EnumDispatchAdviceStatus.Cancelled
                        lbButton = dgLine.Items.Item(intCnt).FindControl("lbDelete")
                        lbButton.Visible = False
                Case objRCTrx.EnumDispatchAdviceStatus.Deleted
                        lbButton = dgLine.Items.Item(intCnt).FindControl("lbDelete")
                        lbButton.Visible = False
            End Select
        Next
    ...
    Sub Update_Status(ByVal pv_strDAID As String, _
        ...
        Dim strOpCode_DA As String = "RC_CLSTRX_DISPATCHADVICE_STATUS_UPD"
        Dim strParam As String = pv_strDAID & "|" & pv_intDASts
        Dim intCnt As Integer
        Dim intErrNo As Integer

        Try
            intErrNo = objRCTrx.mtdUpdDispatchAdviceStatus(strOpCode_DA, _
                                                            strCompany, _
                                                            strLocation, _
                                                            strUserId, _
                                                            strAccMonth, _
                                                            ...
                                                            strParam)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=RC_DISPATCHADVICELIST_UPD_STATUS&errmesg=" & Exp.ToString() & "&redirect=RC/trx/RC_trx_DAList.aspx")
        End Try
    End Sub
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Dim strDispatchAdviceID As String

        dgLine.EditItemIndex = CInt(E.Item.ItemIndex)
        lblDelText = dgLine.Items.Item(CInt(E.Item.ItemIndex)).FindControl("idDAId")
        strDispatchAdviceID = lblDelText.Text

        Update_Status(strDispatchAdviceID, objRCTrx.EnumDispatchAdviceStatus.Deleted)
    ...
    Sub NewDABtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("RC_trx_DADet.aspx")
    End Sub
