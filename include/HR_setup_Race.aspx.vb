...
Public Class HR_Setup_Race : Inherits Page
    ...
    Protected WithEvents srchRaceCode as TextBox
    ...
    Dim strOppCd_GET As String = "HR_CLSSETUP_RACE_LIST_GET"
    Dim strOppCd_ADD As String = "HR_CLSSETUP_RACE_LIST_ADD"
    Dim strOppCd_UPD As String = "HR_CLSSETUP_RACE_LIST_UPD"
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumHRAccessRights.HRRace), intHRAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")

        Else

            If SortExpression.text = "" Then
                SortExpression.text = "RaceCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objHR.mtdGetRaceStatus(objHR.EnumRaceStatus.Active), objHR.EnumRaceStatus.Active))
        StatusList.Items.Add(New ListItem(objHR.mtdGetRaceStatus(objHR.EnumRaceStatus.Deleted), objHR.EnumRaceStatus.Deleted))

    End Sub 

    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objHR.mtdGetRaceStatus(objHR.EnumRaceStatus.All), objHR.EnumRaceStatus.All))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetRaceStatus(objHR.EnumRaceStatus.Active), objHR.EnumRaceStatus.Active))
        srchStatusList.Items.Add(New ListItem(objHR.mtdGetRaceStatus(objHR.EnumRaceStatus.Deleted), objHR.EnumRaceStatus.Deleted))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND RA.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objHR.EnumRaceStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchRaceCode.text = "" Then
            SearchStr =  SearchStr & " AND RA.RaceCode like '" & srchRaceCode.text &"%' "
        End If
        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND RA.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objHR.mtdGetMasterList(strOppCd_GET, strParam, objHR.EnumHRMasterType.Race, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_RACE_GET_LIST&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
        Return objDataSet
    End Function
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objHR.EnumRaceStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("RaceCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("RaceCode")
        RaceCode = EditText.Text
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          strUserId, _
                                          strParam, _
                                          objHR.EnumHRMasterType.Race, _
                                          blnDupKey, _
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_UPDATE_RACE&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Race.aspx")
        End Try
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objHR.EnumRaceStatus.Active, _
                                    objHR.EnumRaceStatus.Deleted, _
                                    objHR.EnumRaceStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  RaceCode & "|" & _
                    Description & "|" & _
        ...
        intErrNo = objHR.mtdUpdMasterList(strOppCd_ADD, _
                                          ...
                                          blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_DELETE_RACE&errmesg=" & lblErrMessage.Text & "&redirect=HR/Setup/HR_setup_Race.aspx")
        End Try
