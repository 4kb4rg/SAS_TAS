...
Public Class PopUpFindSupplier_New : Inherits Page
    ...
    Protected WithEvents txtSupname As System.Web.UI.WebControls.TextBox
    Protected WithEvents txtCreditTerm As System.Web.UI.WebControls.TextBox
    Protected WithEvents ddlPPN As System.Web.UI.WebControls.DropDownList
    Protected WithEvents txtPPNInit As System.Web.UI.WebControls.TextBox
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        strCreditTerm = Request.QueryString("inCreditTerm")
        strPPNInit = Request.QueryString("inPPNInit")
        strPPNInit2 = Request.QueryString("inPPNInit2")

        txtSupname.Text = strCreditTerm
    ...
    Protected Function LoadData() As DataSet
        ...
        If txtItemCode.Text = "" Then
            strParamValue = "" & "|" & IIf(Session("SS_COACENTRALIZED") = "1", "", " AccCode in (SELECT AccCode FROM GL_Account WHERE LocCode='" & Trim(strLocCode) & "') AND ") & "|" & strSortExp
        Else
            If strFormAction = "" Then
                strParamValue = "%" & txtItemCode.Text & "%|" & IIf(Session("SS_COACENTRALIZED") = "1", "", " AccCode in (SELECT AccCode FROM GL_Account WHERE LocCode='" & Trim(strLocCode) & "') AND ") & "|" & strSortExp
            Else
                strParamValue = "%" & txtItemCode.Text & "%||" & strSortExp
            End If
        End If
    ...
    Sub OnSelectItem(ByVal Sender As Object, ByVal e As DataGridCommandEventArgs)
        ...
        If Left(strINItem, 3) = "txt" Then
            strCode = CPCellCode.Text
            strName = CPCellName.Text
            strTerm = CPCellTerm.Text
            strPPN = CPCellPPN.Text
            'strPPN2 = CPCellPPN2.Text


            txtItemCode.Text = Trim(strCode)
            txtSupname.Text = Trim(strName)
            txtCreditTerm.Text = Trim(strTerm)
            'ddlPPN.SelectedItem.Value = Trim(strPPN)
            txtPPNInit.Text = Trim(strPPN)

            opener_findtextbox()
            opener_dropdownval(strINItem, txtItemCode.Text)
            opener_dropdownval(strINItemName, txtSupname.Text)
            opener_dropdownval(strCreditTerm, txtCreditTerm.Text)
            opener_dropdownval(strPPNInit, Trim(strPPN))
            opener_dropdownval(strPPNInit2, txtPPNInit.Text)
        ...
        If Left(strINItem, 3) = "txt" Then
            ...
            onclick_reload_txt(strINItemName, txtSupname.Text, False, blnPostBack)
            onclick_reload_txt(strCreditTerm, txtCreditTerm.Text, False, blnPostBack)
            onclick_reload_txt(strPPNInit, Trim(strPPN), False, blnPostBack)
            onclick_reload_txt(strPPNInit2, txtPPNInit.Text, False, blnPostBack)
        Else
            onclick_reload(strINItem, txtItemCode.Text, False, blnPostBack)
        End If

        'response.write(strPPNInit2)
        'response.write(txtPPNInit.Text)
        close_script()
        print_script()
