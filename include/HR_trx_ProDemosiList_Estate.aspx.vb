...
Public Class HR_trx_ProDemosiList_Estate : Inherits Page
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "HR_HR_TRX_EMPHIST_PRODEMOSI_LIST_GET"
        Dim strST As String = "%"
        Dim strEC As String = "%"
        Dim strEN As String = "%"
        Dim strUI As String = "%"
        ...
        strST = IIf(Not ddlStatus.SelectedItem.Value = "0", ddlStatus.SelectedItem.Value, "%") & " "
 
        If Not txtEmpCode.Text = "" Then
            strEC =  "%" & txtEmpCode.Text & "%"
        End If

        If Not txtEmpName.Text = "" Then
            strEN = "%" &  txtEmpName.Text & "%"
        End If

        If Not txtLastUpdate.Text = "" Then
            strUI = "&" &  txtLastUpdate.Text & "%"
        End If

        	
        sortitem = "ORDER BY " & SortExpression.Text & " " & SortCol.Text
        ParamNama = "ST|EC|EN|UI|LOC|SORT"
        ParamValue = strST & "|" & strEC & "|" & strEN & "|" & strUI & "|" & strLocation & "|" & sortitem

        Try
            intErrNo = ObjOK.mtdGetDataCommon(strOpCd_GET, ParamNama, ParamValue, objMdr)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_HR_TRX_EMPHIST_PRODEMOSI_LIST_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try
    ...
    Sub NewBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("HR_Trx_ProDemosiDet_Estate.aspx")
    End Sub
