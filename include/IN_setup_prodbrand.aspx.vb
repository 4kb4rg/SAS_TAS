...
Public Class IN_ProdBrand : Inherits Page
    ...
    Protected WithEvents srchProdBrandCode as TextBox
    ...
    Dim strOppCd_GET As String = "IN_CLSSETUP_PRODBRAND_LIST_GET"
    Dim strOppCd_ADD As String = "IN_CLSSETUP_PRODBRAND_LIST_ADD"
    Dim strOppCd_UPD As String = "IN_CLSSETUP_PRODBRAND_LIST_UPD"
    ...
    Dim ProdBrandCodeTag As String
    Dim ProdBrandDescTag As String
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumINAccessRights.INProductBrand), intINAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")

        Else
            onload_GetLangCap()
            If SortExpression.text = "" Then
                SortExpression.text = "ProdBrandCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.text = UCase(GetCaption(objLangCap.EnumLangCap.ProdBrand))
        lblProdBrandCode.text = GetCaption(objLangCap.EnumLangCap.ProdBrand) & lblCode.text
        lblDescription.text = GetCaption(objLangCap.EnumLangCap.ProdBrandDesc)

        strValidateCode = lblPleaseEnter.text & lblProdBrandCode.text & "."
        strValidateDesc = lblPleaseEnter.text & lblDescription.text & "."

        EventData.Columns(0).HeaderText = lblProdBrandCode.text
        EventData.Columns(1).HeaderText = lblDescription.text

        DocTitleTag = lblTitle.text & lblList.text
        ProdBrandCodeTag = lblProdBrandCode.text
        ProdBrandDescTag = lblDescription.text
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_SETUP_PRODBRAND_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodbrand.aspx")
        End Try
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductBrandStatus(objIN.EnumProductBrandStatus.Active), objIN.EnumProductBrandStatus.Active))
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductBrandStatus(objIN.EnumProductBrandStatus.Deleted), objIN.EnumProductBrandStatus.Deleted))

    End Sub 
    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductBrandStatus(objIN.EnumProductBrandStatus.Active), objIN.EnumProductBrandStatus.Active))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductBrandStatus(objIN.EnumProductBrandStatus.Deleted), objIN.EnumProductBrandStatus.Deleted))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductBrandStatus(objIN.EnumProductBrandStatus.All), objIN.EnumProductBrandStatus.All))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND PBrand.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductBrandStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchProdBrandCode.text = "" Then
            SearchStr = SearchStr & " AND PBrand.ProdBrandCode like '" & srchProdBrandCode.text &"%' "
        End If

        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND PBrand.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objIN.mtdGetMasterList(strOppCd_GET, strParam, objIN.EnumInventoryMasterType.ProductBrand, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_PRODBRAND&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodbrand.aspx")
        End Try

        If objDataSet.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objDataSet.Tables(0).Rows.Count - 1
                objDataSet.Tables(0).Rows(intCnt).Item("ProdBrandCode") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("ProdBrandCode"))
...
Sub btnPreview_Click (sender As Object, e As ImageClickEventArgs)
    ...
    Dim strProdBrandCode As String
    Dim strDescription As String
    Dim strUpdateBy As String
    Dim strSortExp As String
    Dim strSortCol As String

    strStatus = IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductBrandStatus.All, srchStatusList.selectedItem.Value, "")
    strProdBrandCode = srchProdBrandCode.text
    strDescription = srchDesc.text
    strUpdateBy =  srchUpdateBy.text
    strSortExp = sortexpression.text
    strSortCol = sortcol.text

    Response.Write("<Script Language=""JavaScript"">window.open(""../reports/IN_Rpt_ProdBrand.aspx?strStatus=" & strStatus & _
                   "&strProdBrandCode=" & strProdBrandCode & _
                   ...
                   "&ProdBrandCodeTag=" & ProdBrandCodeTag & _
                   "&ProdBrandDescTag=" & ProdBrandDescTag & _
...
End Sub
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objIN.EnumProductBrandStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("ProdBrandCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("ProdBrandCode")
        ProdBrandCode = EditText.Text
        ...
        intErrNo = objIN.mtdUpdMasterList(strOppCd_ADD, _
                                                ...
                                                strUserId, _
                                                strParam, _
                                                objIN.EnumInventoryMasterType.ProductBrand, _
                                                blnDupKey, _
                                                blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=UPDATE_PRODBRAND&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodbrand.aspx")
        End Try
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objIN.EnumProductBrandStatus.Active, _
                        objIN.EnumProductBrandStatus.Deleted, _
                        objIN.EnumProductBrandStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  ProdBrandCode & "|" & _
                    Description & "|" & _
