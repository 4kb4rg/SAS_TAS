...
Public Class TX_trx_PPNMasukanList : Inherits Page
    ...
    Protected Function LoadData() As DataSet
        ...
        If hidTrxStatus.Value <> objCBTrx.EnumTaxStatus.Closed Then
            ...
            strOpCd_GET = "TX_CLSTRX_PPNMASUKAN_LIST"
        ...
        Else
            ...
            strOpCd_GET = "TX_CLSTRX_PPNMASUKAN_POSTED_LIST"
    ...
    Sub GetPPNPosted()
        Dim strOpCd_GET As String = "TX_CLSTRX_PPNMASUKAN_POSTED_LIST"
        Dim intErrNo As Integer
        Dim strParamName As String
        Dim strParamValue As String
    ...
    Sub PostingBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Dim strOpCdGen As String = "TX_CLSTRX_PPNMASUKAN_POSTED"
        Dim strParamName As String
        Dim strParamValue As String
        Dim intErrNo As Integer
    ...
    Sub PrintDoc_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        Dim strOpCd_GET As String = "TX_CLSTRX_PPNMASUKAN_UPDATE"
        Dim intErrNo As Integer
        Dim strParamName As String
        Dim strParamValue As String
        ...
        Else
            ...
            strOpCd = "TX_STDRPT_GET_CSV_PPNMASUKAN"
            strSearch = IIf(Trim(srchFPNo.Text) = "", "", "FPNo Like '%" & Trim(srchFPNo.Text) & "%' AND ")
            strSearch = strSearch + IIf(Trim(srchDocID.Text) = "", "", "DocID LIKE '%" & Trim(srchDocID.Text) & "%' AND ")
            strSearch = strSearch + IIf(Trim(srchSupplier.Text) = "", "", "(SupplierCode LIKE '%" & Trim(srchSupplier.Text) & "%' OR SupplierName LIKE '%" & Trim(srchSupplier.Text) & "%') AND ")
            ...
            Dim MyCSVFileK As String = objFTPFolder & "VAT_IN-" & Trim(strCompany) & Trim(strPeriod) & ".csv"
            ...
            dataToWriteK.WriteLine("FM;Kode Jenis Transaksi;FG PENGGANTI;No Faktur;Masa Pajak;Tahun Pajak;Tgl Faktur;Npwp;Nama;Alamat;Jumlah_Dpp;Jumlah_Ppn;Jumlah_PpnBM;Is Creditable")

            If objRptDs.Tables(0).Rows.Count > 0 Then
                For intCnt = 0 To objRptDs.Tables(0).Rows.Count - 1
                    dataToWriteK.WriteLine("""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("FM")) & """;""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("JnsTrx")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("FPPengganti")) & """;""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("FPNo")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("MasaPajak")) & """;""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("TahunPajak")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("FPDate")) & """;""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("SPLNPWP")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("SPLName")) & """;""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("SPLAddress")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("FPDPPAmount")) & """;""" & Trim(objRptDs.Tables(0).Rows(intCnt).Item("FPAmount")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("PPNBM")) & """;""" & _
                    Trim(objRptDs.Tables(0).Rows(intCnt).Item("IsCredited")) & """")
                Next
                ...
                LinkDownload.Text = "Download file VAT_IN-" & Trim(strCompany) & Trim(strPeriod) & ".csv"
                LinkDownload.NavigateUrl = "../../../" & strUrl & "VAT_IN-" & Trim(strCompany) & Trim(strPeriod) & ".csv"
            End If
            dataToWriteK.Close()
        End If
        'BindGrid()
    ...
    Sub CancelBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Dim strOpCdGen As String = "TX_CLSTRX_PPNMASUKAN_UNPOSTED"
        Dim strParamName As String
        Dim strParamValue As String
        Dim intErrNo As Integer
        Dim postAccMonth As String
