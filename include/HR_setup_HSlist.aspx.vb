...
Public Class HR_setup_HSList : Inherits Page
    ...
    Protected Function LoadData() As DataSet
        ...
        strSrchHSCode = IIF(txtHSCode.Text = "", "", txtHSCode.Text)
        ...
        strParam = strSrchHSCode & "|" & _
                   ...
                   SortCol.Text & "|"
        Try
            intErrNo = objHRSetup.mtdGetHolidaySchedule(strOpCd_GET, strParam, objHSDs, False)

        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_HS_GET&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        If objHSDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objHSDs.Tables(0).Rows.Count - 1
                objHSDs.Tables(0).Rows(intCnt).Item("HSCode") = Trim(objHSDs.Tables(0).Rows(intCnt).Item("HSCode"))
                objHSDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objHSDs.Tables(0).Rows(intCnt).Item("Description"))
                objHSDs.Tables(0).Rows(intCnt).Item("Status") = Trim(objHSDs.Tables(0).Rows(intCnt).Item("Status"))
                objHSDs.Tables(0).Rows(intCnt).Item("UpdateID") = Trim(objHSDs.Tables(0).Rows(intCnt).Item("UpdateID"))
                objHSDs.Tables(0).Rows(intCnt).Item("UserName") = Trim(objHSDs.Tables(0).Rows(intCnt).Item("UserName"))
            Next
        End If
    ...
    Sub DEDR_Delete(Sender As Object, e As DataGridCommandEventArgs)
        ...
        strSelectedHSCode = CPCell.Text
        strParam = strSelectedHSCode & "||" & objHRSetup.EnumHSStatus.Deleted
        ...
        Try
            intErrNo = objHRSetup.mtdUpdHolidaySchedule(strOpCd_ADD, _
                                                        ...
                                                        True)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_SETUP_HS_DEL&errmesg=" & lblErrMessage.Text & "&redirect=hr/setup/HR_setup_HSList.aspx")
        End Try
    ...
    Sub NewHSBtn_Click(Sender As Object, E As ImageClickEventArgs)
        Response.Redirect("HR_setup_HSdet.aspx")
    End Sub
