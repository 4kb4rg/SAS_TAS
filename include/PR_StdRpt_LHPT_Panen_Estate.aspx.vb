...
Public Class PR_StdRpt_LHPT_Panen_Estate : Inherits Page
	...
	Protected WithEvents chktoday As CheckBox
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Response.Write("<Script Language=""JavaScript"">window.open(""PR_StdRpt_LHPT_PanenPreview_Estate.aspx?CompCode=" & strCompany & "&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & strDec & _
                       "&lblLocation=" & lblLocation.Text & _
                       "&SelDate=" & strFrom & _
                       "&Divisi=" & ddldivisi.SelectedItem.Value.Trim() & _
                       "&Mandor=" & ddlmandor.SelectedItem.Value.Trim() & _
                       "&isMandor=" & chkmandor.Checked & _ 
					   "&Manager=" & Manager & _ 
					   "&Asisten=" & Asisten & _ 
					   "&excel=" & "0" & _ 
					    "&status=" & strStat & _ 
					   "&isToday=" & chktoday.Checked & """ ,""" & strRptID & """ ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
    End Sub
	...
	Sub BindManager()
        Dim strOpCd As String = "HR_PR_TRX_MANAGER_ASISTEN"
        Dim ParamName As String = ""
        Dim ParamValue As String = ""
        Dim objDs As New Object()
        Dim intErrNo As Integer
        ...
        ParamName = "LOC|DIV"
        ParamValue = strLocation & "|" & ddldivisi.SelectedItem.Value.Trim

        Try
            intErrNo = objOk.mtdGetDataCommon(strOpCd, ParamName, ParamValue, objDs)
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=HR_PR_TRX_MANAGER_ASISTEN&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try

        If objDs.Tables(0).Rows.Count > 0 Then
            Manager = Trim(objDs.Tables(0).Rows(0).Item("Manager"))
			Asisten = Trim(objDs.Tables(0).Rows(0).Item("Asisten"))
        End If
	End Sub
