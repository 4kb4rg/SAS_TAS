...
Public Class PU_SuppDet : Inherits Page
    ...
    Protected WithEvents dgBlockList As DataGrid
    ...
    Sub BtnAddBlock_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Dim strOpCd As String = "PU_CLSSETUP_SUPPLIER_FFBINV_ADD"
        Dim intErrNo As Integer
        Dim strParamName As String
        Dim strParamValue As String
        Dim intCnt As Integer = 0
        ...
        strParamName = "SPLCODE|LOCCODE"
        strParamValue = strSelectedSupp & "|" & ddlLocCode.SelectedItem.Value
    ...
    Sub BindLocation(ByVal pv_strLocCode As String)
        ...
        strParamValue = " AND LocType='1' AND LocLevel='1'"
        ...
        For intCnt = 0 To objAccYearDs.Tables(0).Rows.Count - 1
            objAccYearDs.Tables(0).Rows(intCnt).Item("LocCode") = Trim(objAccYearDs.Tables(0).Rows(intCnt).Item("LocCode"))
            objAccYearDs.Tables(0).Rows(intCnt).Item("Description") = Trim(objAccYearDs.Tables(0).Rows(intCnt).Item("Description"))
            If objAccYearDs.Tables(0).Rows(intCnt).Item("LocCode") = pv_strLocCode Then
                intSelectedIndex = intCnt + 1
            End If
        Next intCnt

        ddlLocCode.DataSource = objAccYearDs.Tables(0)
        ddlLocCode.DataValueField = "LocCode"
        ddlLocCode.DataTextField = "Description"
        ddlLocCode.DataBind()
        ddlLocCode.SelectedIndex = intSelectedIndex - 1
    End Sub

    Sub BindFFBGrid(ByVal pv_strSplCode As String)
        Dim strOpCd As String = "PU_CLSSETUP_SUPPLIER_FFBINV_GET"
        Dim intErrNo As Integer
        Dim strParamName As String
        Dim strParamValue As String
        Dim intCnt As Integer = 0
        ...
        dgBlockList.DataSource = objTaxDs.Tables(0)
        dgBlockList.DataBind()

        If objTaxDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objTaxDs.Tables(0).Rows.Count - 1
                EdtButton = dgBlockList.Items.Item(intCnt).FindControl("lbEdit")
                DelButton = dgBlockList.Items.Item(intCnt).FindControl("lbDelete")
                UpdButton = dgBlockList.Items.Item(intCnt).FindControl("lbUpdate")
                CanButton = dgBlockList.Items.Item(intCnt).FindControl("lbCancel")
                DelButton.Attributes("onclick") = "javascript:return ConfirmAction('delete');"
    ...
    Sub DEDR_DeleteFFBINV(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        
    End Sub

    Sub DEDR_CancelFFBINV(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        BindFFBGrid(strSelectedSupp)
    End Sub

    Sub DEDR_UpdateFFBINV(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        Dim StrOpCode As String = "PU_CLSSETUP_SUPPLIER_FFBINV_UPD"
        Dim strParamName As String = ""
        Dim strParamValue As String = ""
        Dim lblBlkCode As Label
        Dim lblSubBlkCode As Label
        Dim lblYearPlanting As Label
        Dim txtYearInvoice As TextBox
        Dim btn As LinkButton
        Dim intErrNo As Integer

        lblBlkCode = E.Item.FindControl("lblBlkCode")
        lblSubBlkCode = E.Item.FindControl("lblSubBlkCode")
        lblYearPlanting = E.Item.FindControl("lblYearPlanting")
        txtYearInvoice = E.Item.FindControl("TxtYearInvoice")
        
        strParamName = "LOCCODE|SPLCODE|YEARPLANTING|YEARINVOICE|BLKCODE|SUBBLKCODE"
        strParamValue = ddlLocCode.SelectedItem.Value & "|" & strSelectedSupp & "|" & lblYearPlanting.Text & "|" & txtYearInvoice.Text & "|" & _
                        lblBlkCode.Text & "|" & lblSubBlkCode.Text
        ...
        btn = E.Item.FindControl("lbUpdate")
        btn.Visible = False

        BindFFBGrid(strSelectedSupp)
    End Sub

    Sub DEDR_EditFFBINV(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        Dim lbl As Label
        Dim btn As LinkButton
        Dim txt As TextBox
        Dim strParam As String = ""
        
        dgBlockList.EditItemIndex = CInt(E.Item.ItemIndex)

        lbl = E.Item.FindControl("lblYearInvoice")
        lbl.Visible = False
        txt = E.Item.FindControl("TxtYearInvoice")
        txt.Visible = True
