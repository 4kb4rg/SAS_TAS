...
Public Class IN_ProdMaterial : Inherits Page
    ...
    Protected WithEvents srchProdMatCode as TextBox
    ...
    Dim strOppCd_GET As String = "IN_CLSSETUP_PRODMAT_LIST_GET"
    Dim strOppCd_ADD As String = "IN_CLSSETUP_PRODMAT_LIST_ADD"
    Dim strOppCd_UPD As String = "IN_CLSSETUP_PRODMAT_LIST_UPD"
    ...
    Dim ProdMatCodeTag As String
    Dim ProdMatDescTag As String
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumINAccessRights.INProductMaterial), intINAR) = False Then
            Response.Redirect("/" & strLangCode & "/include/mesg/AccessRights.aspx")

        Else
            onload_GetLangCap()
            If SortExpression.text = "" Then
                SortExpression.text = "ProdMatCode"
                sortcol.text = "ASC"
            End If
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.text = UCase(GetCaption(objLangCap.EnumLangCap.ProdMat))
        lblProdMatCode.text = GetCaption(objLangCap.EnumLangCap.ProdMat) & lblCode.text
        lblDescription.text = GetCaption(objLangCap.EnumLangCap.ProdMatDesc)

        strValidateCode = lblPleaseEnter.text & lblProdMatCode.text & "."
        strValidateDesc = lblPleaseEnter.text & lblDescription.text & "."

        EventData.Columns(0).HeaderText = lblProdMatCode.text
        EventData.Columns(1).HeaderText = lblDescription.text

        DocTitleTag = lblTitle.text & lblList.text
        ProdMatCodeTag = lblProdMatCode.text
        ProdMatDescTag = lblDescription.text
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_SETUP_PRODMATERIAL_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodmaterial.aspx")
        End Try
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductMaterialStatus(objIN.EnumProductMaterialStatus.Active), objIN.EnumProductMaterialStatus.Active))
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductMaterialStatus(objIN.EnumProductMaterialStatus.Deleted), objIN.EnumProductMaterialStatus.Deleted))

    End Sub 

    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductMaterialStatus(objIN.EnumProductMaterialStatus.Active), objIN.EnumProductMaterialStatus.Active))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductMaterialStatus(objIN.EnumProductMaterialStatus.Deleted), objIN.EnumProductMaterialStatus.Deleted))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductMaterialStatus(objIN.EnumProductMaterialStatus.All), objIN.EnumProductMaterialStatus.All))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND PMat.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductMaterialStatus.All, _
                       srchStatusList.selectedItem.Value, "%" ) &"' "
        If NOT srchProdMatCode.text = "" Then
            SearchStr = SearchStr & " AND PMat.ProdMatCode like '" & srchProdMatCode.text &"%' "
        End If

        If NOT srchDesc.text = "" Then
            SearchStr = SearchStr & " AND PMat.Description like '" & _
                        srchDesc.text &"%' "
        End If
        ...
        Try
        intErrNo = objIN.mtdGetMasterList(strOppCd_GET, strParam, objIN.EnumInventoryMasterType.ProductMaterial, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_PRODMATERIAL&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodmaterial.aspx")
        End Try

        If objDataSet.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objDataSet.Tables(0).Rows.Count - 1
                objDataSet.Tables(0).Rows(intCnt).Item("ProdMatCode") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("ProdMatCode"))
    ...
    Sub btnPreview_Click (sender As Object, e As ImageClickEventArgs)
        ...
        Dim strProdMatCode As String
        Dim strDescription As String
        Dim strUpdateBy As String
        Dim strSortExp As String
        Dim strSortCol As String

        strStatus = IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductMaterialStatus.All, srchStatusList.selectedItem.Value, "")
        strProdMatCode = srchProdMatCode.text
        strDescription = srchDesc.text
        strUpdateBy =  srchUpdateBy.text
        strSortExp = sortexpression.text
        strSortCol = sortcol.text

        Response.Write("<Script Language=""JavaScript"">window.open(""../reports/IN_Rpt_ProdMaterial.aspx?strStatus=" & strStatus & _
                       "&strProdMatCode=" & strProdMatCode & _
                       ...
                       "&ProdMatCodeTag=" & ProdMatCodeTag & _
                       "&ProdMatDescTag=" & ProdMatDescTag & _
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objIN.EnumProductMaterialStatus.Active
            Case True
                Statuslist.selectedindex = 0
                EditText = EventData.Items.Item(CInt(e.Item.ItemIndex)).FindControl("ProdMatCode")
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        EditText = E.Item.FindControl("ProdMatCode")
        ProdMatCode = EditText.Text
        ...
        intErrNo = objIN.mtdUpdMasterList(strOppCd_ADD, _
                                                ...
                                                strUserId, _
                                                strParam, _
                                                objIN.EnumInventoryMasterType.Productmaterial, _
                                                blnDupKey, _
                                                blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_PRODMATERIAL&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodmaterial.aspx")
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objIN.EnumProductMaterialStatus.Active, _
                        objIN.EnumProductMaterialStatus.Deleted, _
                        objIN.EnumProductMaterialStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  ProdMatCode & "|" & _
                    Description & "|" & _
