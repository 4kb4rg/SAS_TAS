...
Public Class IN_Rpt_FuelIssueDet : inherits page
    ...
    Public Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strFuelIssueId = Trim(Request.QueryString("strFuelIssueId"))
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Select Case CInt(strIssueType)
            Case objIN.EnumFuelIssueType.OwnUse
                strOpCd_Get = "IN_CLSTRX_FUELISSUE_DETAILS_GET_REPORT" & "|" & "FuelIssue"
                strOpCd_GetLine = "IN_CLSTRX_FUELISSUE_LINE_GET_REPORT" & "|" & "FuelIssueLn"
                strReportName = "IN_Rpt_FuelIssueDet_OwnUse.rpt"
            Case objIN.EnumFuelIssueType.StaffPayroll, objIN.EnumFuelIssueType.StaffDN
                strOpCd_Get = "IN_CLSTRX_FUELISSUE_DETAILS_GET_REPORT_STAFF" & "|" & "FuelIssue"
                strOpCd_GetLine = "IN_CLSTRX_FUELISSUE_LINE_GET_REPORT_STAFF" & "|" & "FuelIssueLn"
                strReportName = "IN_Rpt_FuelIssueDet_Staff.rpt"
            Case objIN.EnumFuelIssueType.External
                strOpCd_Get = "IN_CLSTRX_FUELISSUE_DETAILS_GET_REPORT_EXTERNAL" & "|" & "FuelIssue"
                strOpCd_GetLine = "IN_CLSTRX_FUELISSUE_LINE_GET_REPORT_EXTERNAL" & "|" & "FuelIssueLn"
                strReportName = "IN_Rpt_FuelIssueDet_External.rpt"
        End Select
        
        strOpCodes = strOpCd_Get & chr(9) & strOpCd_GetLine
        strParam = strFuelIssueId & "|" & strSortLine & "||"

        Try
            intErrNo = objIN.mtdGetFuelIssueReport(strOpCodes, _
                                                    strParam, _
                                                    strCompany, _
                                                    strLocation, _
                                                    strUserId, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_RPT_FUELISSUE_GET&errmesg=" & lblErrMesage.Text & "&redirect=")        
        End Try

        If objRptDs.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
                If batchPrint = "yes" Then
                    objRptDs.Tables(0).Rows(intCnt).Item("PrintStatus") = objIN.mtdGetFuelIssueStatus(CInt(objRptDs.Tables(0).Rows(intCnt).Item("Status")))
                ...
                If InStr(strReprintedID, "|") <> 0 Then
                    ...
                    For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
                        For intCnt2=0 To UBound(arrReprintedID)
                            If Trim(objRptDs.Tables(0).Rows(intCnt).Item("FuelIssueId")) = Trim(arrReprintedID(intCnt2)) Then
                                objRptDs.Tables(0).Rows(intCnt).Item("PrintStatus") = objIN.mtdGetFuelIssueStatus(CInt(objRptDs.Tables(0).Rows(intCnt).Item("Status"))) & " (re-printed)"                                
                            End If
                        Next
                    Next
                End If
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_RPT_FUELISSUE_GET_MAPPATH&errmesg=" & lblErrMesage.Text & "&redirect=")
        End Try
        
        rdCrystalViewer.Load(objMapPath & "Web\EN\IN\Reports\Crystal\" & strReportName, OpenReportMethod.OpenReportByTempCopy)
        rdCrystalViewer.SetDataSource(objRptDs)
        rdCrystalViewer.PrintOptions.PaperOrientation = PaperOrientation.DefaultPaperOrientation
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\IN_Rpt_FuelIssueDet.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/IN_Rpt_FuelIssueDet.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        If Trim(strIssueType) = objIN.EnumFuelIssueType.OwnUse Then
