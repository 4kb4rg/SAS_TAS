...
Public Class PR_setup_PrmBrsList_Estate : Inherits Page

    Protected WithEvents dgLine As DataGrid
    Protected WithEvents lbDelete As LinkButton
    Protected WithEvents lblTracker As Label
    Protected WithEvents lstDropList As DropDownList
    Protected WithEvents txtPrmBrsCode As TextBox
    Protected WithEvents txtTunj As TextBox
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            If SortExpression.Text = "" Then
                SortExpression.Text = "A.PMBerasCode"
            End If
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCd_GET As String = "PR_PR_STP_PREMIBERAS_GET_LIST"
        Dim strSearch As String
        Dim sortitem As String
        Dim intErrNo As Integer
        Dim intCnt As Integer

        strSearch = " AND C.LocCode='" & strLocation & "' AND C.Status='1' AND A.Status like '" & IIf(Not ddlStatus.SelectedItem.Value = "0", _
                       ddlStatus.SelectedItem.Value, "%") & "' "
        If Not txtPrmBrsCode.Text = "" Then
            strSearch = strSearch & " AND A.PMBerasCode like '%" & _
                        txtPrmBrsCode.Text & "%' "
        End If
        If Not txtTunj.Text = "" Then
            strSearch = strSearch & " AND D.Description like '%" & txtTunj.Text & "%' "
        End If
        ...
        If objBankDs.Tables(0).Rows.Count > 0 Then
            For intCnt = 0 To objBankDs.Tables(0).Rows.Count - 1
                objBankDs.Tables(0).Rows(intCnt).Item("PMBerasCode") = Trim(objBankDs.Tables(0).Rows(intCnt).Item("PMBerasCode"))
    ...
    Sub DEDR_Delete(ByVal Sender As Object, ByVal e As DataGridCommandEventArgs)
        Dim strOpCd_UPD As String = "PR_PR_STP_PREMIBERAS_LIST_DEL"
        Dim int As Integer = e.Item.ItemIndex
        Dim PMBCell As TableCell = e.Item.Cells(0)
        Dim strPMBCd As String
        Dim strStatus As String
        Dim intErrNo As Integer
        Dim EditText As Label


        strPMBCd = PMBCell.Text
        EditText = dgLine.Items.Item(int).FindControl("Status")
        strStatus = IIf(EditText.Text = "Active", "2", "1")

        ParamNama = "PMBC|ST|UD|UI|Loc"
        ParamValue = strPMBCd & "|" & strStatus & "|" & DateTime.Now & "|" & strUserId & "|" & strLocation
    ...
    Sub NewSalBtn_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        Response.Redirect("PR_setup_PrmBrsDet_Estate.aspx")
    End Sub
