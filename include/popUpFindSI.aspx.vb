...
Public Class PopUpFindSI : Inherits Page
    ...
    Protected Function LoadData() As DataSet
        Dim strOpCode As String = "IN_CLSTRX_STOCKRETURN_ITEMLIST_GET"
        ...
        If txtItemCode.Text = "" Then
            strParamValue = " And ItemType = '1' And Itm.Status = '1' And si.Status In ('2') And si.LocCode = '" & strLocCode & "'" & _
                            " And (ln.StockIssueID Like '" & txtItemCode.Text & "'" & _
                            " Or ln.ItemCode Like '" & txtItemCode.Text & "'" & _
                            " Or Itm.Description Like '" & txtItemCode.Text & "')" & _
                            "|" & "ln.StockIssueID DESC "
        Else
            strParamValue = " And ItemType = '1' And Itm.Status = '1' And si.Status In ('2') And si.LocCode = '" & strLocCode & "'" & _
                            " And (ln.StockIssueID Like '%" & txtItemCode.Text & "%'" & _
                            " Or ln.ItemCode Like '%" & txtItemCode.Text & "%'" & _
        ...
        If objItemDs.Tables(0).Rows.Count = 0 And txtItemCode.Text <> "" Then
            lblResult.Text = "Cannot find any data to be processed into Credited Invoice"
    ...
    Sub OnSelectItem(ByVal Sender As Object, ByVal e As DataGridCommandEventArgs)
        ...
        If Left(strINItem, 3) = "txt" Then
            onclick_reload_txt(strINItem, Trim(txtItemCode.Text), False, blnPostBack)
        Else
            onclick_reload(strINItem, Trim(txtItemCode.Text), False, blnPostBack)
        End If
        close_script()
        print_script()
    ...
    Sub onclick_reload(ByVal pv_strProperty As String, ByVal pv_strValue As String, ByVal pv_blnIsInstr As Boolean, ByVal pv_blnIsPostBack As Boolean)
        strJavaScript += "if (_findopenerdropdown('" + strForm + "','" + pv_strProperty + "','" + Trim(pv_strValue) + "','" & pv_blnIsInstr & "') == false) {" + Chr(10) + _
