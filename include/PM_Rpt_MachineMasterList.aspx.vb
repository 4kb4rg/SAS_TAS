...
Public Class PM_Rpt_MachineMasterList : inherits page
    ...
    Public Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strMachineCode = Trim(Request.QueryString("strMachineCode"))
        strDescription = Trim(Request.QueryString("strDescription"))
        strProcessLine = Trim(Request.QueryString("strProcessLine")) 
        ...
        MachineCodeTag = Trim(Request.QueryString("MachineCodeTag"))
    ...
    Sub Bind_ITEM(ByVal blnIsPDFFormat As Boolean)
        ...
        Dim strOpCd As String = "PM_CLSSETUP_MACHINE_GET"
        ...
        strSearch =  " AND M.Status like '" & IIF(Not strStatus = "", strStatus, "%" ) & "' AND M.LocCode='" & strLocation & "'"

        If NOT strMachineCode = "" Then
            strSearch =  strSearch & " AND M.MachineCode like '" & strMachineCode & "%' "
        End If
        
        If NOT strDescription = "" Then
            strSearch = strSearch & " AND M.Description like '" & strDescription & "%' "
        End If

        If NOT strProcessLine = "" Then
            strSearch = strSearch & " AND M.ProcessingLnNo = " & strProcessLine
        End If
        ...
        Try
            intErrNo = objPM.mtdGetMachine(strOpCd, strParam, objRptDs)
        ...
        For intCnt=0 To objRptDs.Tables(0).Rows.Count - 1
            objRptDs.Tables(0).Rows(intCnt).Item("Status") = objPM.mtdGetUllageVolumeTableStatus(objRptDs.Tables(0).Rows(intCnt).Item("Status"))
            objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate") = objGlobal.GetLongDate(objRptDs.Tables(0).Rows(intCnt).Item("UpdateDate"))
        Next

        rdCrystalViewer.Load(objMapPath & "Web\EN\PM\Reports\Crystal\PM_Rpt_MachineMasterList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        Else
            ...
            DiskOpts.DiskFileName = objMapPath & "web\ftp\PM_Rpt_MachineMasterList.pdf"
            rdCrystalViewer.ExportOptions.DestinationOptions = DiskOpts
            rdCrystalViewer.Export()

            Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../../ftp/PM_Rpt_MachineMasterList.pdf"">")
        End If
    ...
    Sub PassParam()
        ...
        paramField4 = ParamFields.Item("MachineCodeTag")
        ...
        ParamDiscreteValue4.value = MachineCodeTag
