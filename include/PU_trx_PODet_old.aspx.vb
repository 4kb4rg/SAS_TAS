...
Public Class PU_PODet : Inherits Page
    ...
    'Protected WithEvents chkVATCredited As CheckBox
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            If Not IsPostBack Then
                ...
                If strSelectedPOType = objPU.EnumPOType.Stock Then
    ...
    Sub onLoad_DisplayPO(ByVal pv_strPOId As String)
        ...
        If objPODs.Tables(0).Rows(0).Item("VatCredited") = "1" Then
            chkVATCredited.Checked = True
        Else
            chkVATCredited.Checked = False
        End If
        ...
        If (objPODs.Tables(0).Rows(0).Item("Status") = objPU.EnumPOStatus.Deleted) Or (objPODs.Tables(0).Rows(0).Item("Status") = objPU.EnumPOStatus.Cancelled) Then
            btnEdited.Visible = False
        End If
    ...
    Sub btnAdd_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        Dim IsVatCredited As String = IIf(chkVATCredited.Checked = True, "1", "0")
        ...
        If lblPOType.Text = objPU.EnumPOType.DirectCharge Then
            Select Case strItemCode
                Case "SPK"
                    ...
                    strNewIDFormat = "/" & "PO" & "/" & "SPK" & "/" & strCompany & "/" & strlocation & "/" & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/" & strAccYear
                Case "BPL"
                    'strNewIDFormat = "SPB" & "/" & strCompany & "/" & strlocation & "/" & strPOIssued & "DC/" & strAccYear & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/"                    
                    strNewIDFormat = "/" & "PO" & "/" & "SPK" & "/" & strCompany & "/" & strlocation & "/" & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/" & strAccYear
                Case "KKP"
                    'strNewIDFormat = "KKP" & "/" & strCompany & "/" & strlocation & "/" & strPOIssued & "/" & strAccYear & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/"
                    strNewIDFormat = "/" & "KKP" & "/" & "SPK" & "/" & strCompany & "/" & strlocation & "/" & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/" & strAccYear
                Case Else
                    'strNewIDFormat = "KKP" & "/" & strCompany & "/" & strlocation & "/" & strPOIssued & "/" & strAccYear & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/"
                    strNewIDFormat = "/" & "KKL" & "/" & "SPK" & "/" & strCompany & "/" & strlocation & "/" & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/" & strAccYear
            End Select
        Else
            strNewIDFormat = "/" & "PO" & "/" & strCompany & "/" & strlocation & "/" & IIf(Len(Trim(strAccMonth)) = 1, "0" & strAccMonth, strAccMonth) & "/" & strAccYear
        End If
        ...
        If lblHidStatusEdited.Text = "0" Then
            ...
            Else
                ...
                decPPNAmtTransportFee = IIf(ddlPPNTransport.SelectedItem.Value = "1", Round(decNetAmtTransportFee * (Session("SS_PPNRATE") / 100), 0), 0)
                ...
                decPBBKBAmount = Round(decQtyOrder * (decCost * (decPBBKBRate / 100) * (decPBBKB / 100)), 0)
    ...
    Sub btnSave_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        Dim strDate As String = Date_Validation(txtDateCreated.Text, False)
        Dim indDate As String = ""
        Dim StrVatCredited As String = IIf(chkVATCredited.Checked = True, "1", "0")
    ...
    Sub btnUnDelete_Click(ByVal Sender As Object, ByVal E As ImageClickEventArgs)
        ...
        If lblPOType.Text = objPU.EnumPOType.Stock Or lblPOType.Text = objPU.EnumPOType.FixedAsset Then
            ...
            strParam = lblPOId.Text & "|" & _
                        ...
                        DDLLocPenyerahan.SelectedItem.Value & "|" & _
                        strUserId & "|" & _
                        IIf(txtPPNInit.Text = "1", objPU.EnumPPN.Yes, IIf(chkSurat.Checked = True, "3", objPU.EnumPPN.No))
