...
Public Class IN_ProdCategory : Inherits Page
    ...
    Dim ProdCatCodeTag As String
    Dim ProdCatDescTag As String
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        ElseIf objAR.mtdHasAccessRights(objAR.mtdGetAccessRights(objAR.EnumINAccessRights.INProductCategory), intINAR) = False Then
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.text = UCase(GetCaption(objLangCap.EnumLangCap.ProdCat))
        lblProdCatCode.text = GetCaption(objLangCap.EnumLangCap.ProdCat) & lblCode.text
        lblDescription.text = GetCaption(objLangCap.EnumLangCap.ProdCatDesc)

        strValidateCode = lblPleaseEnter.text & lblProdCatCode.text & "."
        strValidateDesc = lblPleaseEnter.text & lblDescription.text & "."

        EventData.Columns(0).HeaderText = lblProdCatCode.text
        EventData.Columns(1).HeaderText = lblDescription.text

        DocTitleTag = lblTitle.text & lblList.text
        ProdCatCodeTag = lblProdCatCode.text
        ProdCatDescTag = lblDescription.text
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=IN_SETUP_PRODCAT_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodcategory.aspx")
        End Try
    ...
    Sub BindStatusList(index as integer) 
        ...
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductCategoryStatus(objIN.EnumProductCategoryStatus.Active), objIN.EnumProductCategoryStatus.Active))
        StatusList.Items.Add(New ListItem(objIN.mtdGetProductCategoryStatus(objIN.EnumProductCategoryStatus.Deleted), objIN.EnumProductCategoryStatus.Deleted))

    End Sub 
    Sub BindSearchList() 

        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductCategoryStatus(objIN.EnumProductCategoryStatus.Active), objIN.EnumProductCategoryStatus.Active))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductCategoryStatus(objIN.EnumProductCategoryStatus.Deleted), objIN.EnumProductCategoryStatus.Deleted))
        srchStatusList.Items.Add(New ListItem(objIN.mtdGetProductCategoryStatus(objIN.EnumProductCategoryStatus.All), objIN.EnumProductCategoryStatus.All))
    ...
    Protected Function LoadData() As DataSet
        ...
        SearchStr =  " AND PCat.Status like '" & IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductCategoryStatus.All, _
        ...
        Try
        intErrNo = objIN.mtdGetMasterList(strOppCd_GET, strParam, objIN.EnumInventoryMasterType.ProductCategory, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_PRODCATEGORY&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodcategory.aspx")
        End Try

        If objDataSet.Tables(0).Rows.Count > 0 Then
            For intCnt=0 To objDataSet.Tables(0).Rows.Count - 1
                objDataSet.Tables(0).Rows(intCnt).Item("ProdCatCode") = Trim(objDataSet.Tables(0).Rows(intCnt).Item("ProdCatCode"))
    ...
    Sub btnPreview_Click (sender As Object, e As ImageClickEventArgs)
        ...
        Dim strProdCatCode As String
        Dim strDescription As String
        Dim strUpdateBy As String
        Dim strSortExp As String
        Dim strSortCol As String

        strStatus = IIF(Not srchStatusList.selectedItem.Value = objIN.EnumProductCategoryStatus.All, srchStatusList.selectedItem.Value, "")
        strProdCatCode = srchProdCatCode.text
        strDescription = srchDesc.text
        strUpdateBy =  srchUpdateBy.text
        strSortExp = sortexpression.text
        strSortCol = sortcol.text

        Response.Write("<Script Language=""JavaScript"">window.open(""../reports/IN_Rpt_ProdCategory.aspx?strStatus=" & strStatus & _
                       "&strProdCatCode=" & strProdCatCode & _
                       ...
                       "&ProdCatCodeTag=" & ProdCatCodeTag & _
                       "&ProdCatDescTag=" & ProdCatDescTag & _
    ...
    Sub DEDR_Edit(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Select Case cint(Edittext.text) = objIN.EnumProductCategoryStatus.Active
            Case True
    ...
    Sub DEDR_Update(Sender As Object, E As DataGridCommandEventArgs)
        ...
        intErrNo = objIN.mtdUpdMasterList(strOppCd_ADD, _
                                                ...
                                                strUserId, _
                                                strParam, _
                                                objIN.EnumInventoryMasterType.ProductCategory, _
                                                blnDupKey, _
                                                blnUpdate.text)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_PRODCATEGORY&errmesg=" & lblErrMessage.Text & "&redirect=IN/Setup/IN_prodcategory.aspx")
    ...
    Sub DEDR_Delete(Sender As Object, E As DataGridCommandEventArgs)
        ...
        Status = IIF(EditText.Text = objIN.EnumProductCategoryStatus.Active, _
                        objIN.EnumProductCategoryStatus.Deleted, _
                        objIN.EnumProductCategoryStatus.Active )
        EditText = E.Item.FindControl("CreateDate")
        CreateDate = EditText.Text
        strParam =  ProdCatCode & "|" & _
                    Description & "|" & _
