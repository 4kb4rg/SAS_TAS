<%@ Control Language="VB" Inherits="WM_STDRPT_SELECTION_CTRL" src="../../../include/Reports/WM_StdRpt_Selection_Ctrl.ascx.vb" %>
	...
	<tr>
		<td>
			<table width=100% cellspacing="0" cellpadding="1" border="0" align="left" ID="tblRptSelect2" class="font9Tahoma" runat=server>
				<tr>
					<td width=40%>Report Name : </td>
					<td width=40% colspan=2><asp:DropDownList id="lstRptname" CssClass="font9Tahoma" width=80% size=1 AutoPostBack=true OnSelectedIndexchanged=CheckRptName runat=server /></td>
					<td width=20%>&nbsp;</td>
				</tr>	
				<tr id="SelLocation">
					...
					<td colspan=2>
						<table width=50% cellpadding=0 cellspadding=0>
							<tr>
								<td><asp:CheckBox text=All id="cbLocAll" OnCheckedChanged=Check_Clicked CssClass="font9Tahoma" AutoPostBack=true runat=server /></td>
								<td><asp:CheckBoxList id=cblUserLoc OnSelectedIndexChanged="LocCheckList" RepeatColumns="5" RepeatDirection="Horizontal" CssClass="font9Tahoma" AutoPostBack=True runat=server /></td>
							</tr>
						</table>
					</td>
				...
				<tr id="TrMthYr">
					...
					<td>
						<asp:DropDownList id="lstAccMonth" size=1 width=40% runat=server />
						<asp:DropDownList id="lstAccYear" size=1 width=40% autopostback=true onselectedindexchanged=OnIndexChage_FromAccPeriod runat=server />
					</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				</tr>																
