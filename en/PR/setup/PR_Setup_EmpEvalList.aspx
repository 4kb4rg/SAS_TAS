<%@ Page Language="vb" src="../../../include/PR_Setup_EmpEvalList.aspx.vb" Inherits="PR_Setup_EmpEvalList" %>
			...
			<tr>
				...
				<div class="kontenlist">
					<table cellpadding="0" cellspacing="0" style="width: 100%" class="font9Tahoma">
						...
						<tr>
							...
							<table cellpadding="4" cellspacing="0" style="width: 100%">
								...
								<td width="10%" height="26" valign=bottom>
									<asp:label id="lblYear" runat="server" /> Year:<BR>
									<asp:TextBox id=srchYear width=100% maxlength="4" runat="server"/>
								</td>
								<td width="20%" height="26" valign=bottom>
									<asp:label id="lblEmpCode" runat="server" /> Employee Code:<BR>
									<asp:DropDownList id=srchEmpCode width=100% autopostback=true onselectedindexchanged=onSelect_Code runat="server"/>
								</td>
								<td width="20%" height="26" valign=bottom>
									<asp:label id="lblEmpName" runat="server" /> Employee Name:<BR>
									<asp:DropDownList id=srchEmpName width=100% autopostback=true onselectedindexchanged=onSelect_Name runat="server"/>
								</td>
								<td width="10%" height="26" valign=bottom>
									<asp:label id="lblValue" runat="server" /> Value:<BR>
									<asp:TextBox id=srchValue width=100% maxlength="8" runat="server"/>
								</td>
							    ...
							    <table cellpadding="4" cellspacing="1" style="width: 100%">
						            ...
						            <asp:DataGrid id="EventData"
					                    ...
					                    <asp:TemplateColumn SortExpression="Year" HeaderText="Year">
						                    <ItemTemplate>
							                    <%# Trim(Container.DataItem("Year")) %>
						                    </ItemTemplate>
						                    <EditItemTemplate>
							                    <asp:TextBox id="txtYear" MaxLength="4" columns="4"
									                    Text='<%# Trim(Container.DataItem("Year")) %>'
									                    runat="server"/>							
							                    <asp:label id="lblDupMsg"  Text="<br>Employee has been <br> evaluated <br> on selected year" Visible = false forecolor=red Runat="server"/>
							                    <asp:RequiredFieldValidator id=validateYear display=dynamic runat=server 
									                    ControlToValidate=txtYear />
						                    </EditItemTemplate>
					                    </asp:TemplateColumn>
	
					                    <asp:TemplateColumn SortExpression="E.EmpCode" HeaderText="Employee Code">
						                    <ItemTemplate>
							                    <%# Trim(Container.DataItem("EmpCode")) %>
						                    </ItemTemplate>
						                    <EditItemTemplate>
							                    <asp:DropDownList id="ddlEmpCode" width=100% autopostback=true onselectedindexchanged=onSelect_Code runat="server"/>
							                    <asp:Label id="dataEmpCode" width=100% Text='<%# Trim(Container.DataItem("EmpCode")) %>' visible=false runat="server"/>																
						                    </EditItemTemplate>
					                    </asp:TemplateColumn>
					
					                    <asp:TemplateColumn SortExpression="EmpName" HeaderText="Employee Name">
						                    <ItemTemplate>
							                    <%# Trim(Container.DataItem("EmpName")) %>
						                    </ItemTemplate>
						                    <EditItemTemplate>
							                    <asp:DropDownList id="ddlEmpName" width=100% autopostback=true onselectedindexchanged=onSelect_Name runat="server"/>	
							                    <asp:Label id="dataEmpName" width=100% Text='<%# Trim(Container.DataItem("EmpName")) %>' visible=false runat="server"/>														
						                    </EditItemTemplate>
					                    </asp:TemplateColumn>
					
					                    <asp:TemplateColumn SortExpression="Value" HeaderText="Value">
						                    <ItemTemplate>
							                    <%# Trim(Container.DataItem("Value")) %>
						                    </ItemTemplate>
						                    <EditItemTemplate>
							                    <asp:TextBox id="txtValue" columns="4"
								                    Text='<%# trim(Container.DataItem("Value")) %>'
								                    runat="server"/>
							                    <asp:RequiredFieldValidator id=validateValue display=dynamic runat=server 
									                    ControlToValidate=txtValue />
							                    <asp:RegularExpressionValidator id=revAmount 
								                    ControlToValidate="txtValue"
								                    ValidationExpression="\d{1,19}\.\d{1,2}|\d{1,19}"
								                    Display="Dynamic"
								                    text = "The value must <br>whole number<br> max 2 decimal"
								                    runat="server"/>															
						                    </EditItemTemplate>
					                    </asp:TemplateColumn>
					                    <asp:TemplateColumn SortExpression="Remarks" HeaderText="Remarks">
						                    <ItemTemplate>
							                    <%# Trim(Container.DataItem("Remarks")) %>
						                    </ItemTemplate>
						                    <EditItemTemplate>
							                    <asp:TextBox id="txtRemarks" width=100%
								                    Text='<%# trim(Container.DataItem("Remarks")) %>'
								                    runat="server"/>															
						                    </EditItemTemplate>
					                    </asp:TemplateColumn>
					                    <asp:TemplateColumn HeaderText="Last Update" SortExpression="EE.UpdateDate">
						                    <ItemTemplate>
					                    ...
					                    <asp:TemplateColumn HeaderText="Status" SortExpression="EE.Status">
						                    <ItemTemplate>
						...
						<tr>
							<td>
					            <asp:ImageButton id=ibNew OnClick="DEDR_Add" imageurl="../../images/butt_new.gif" AlternateText="New Employee Evaluation" runat="server"/>
